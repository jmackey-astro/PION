...
EPOCH_FILE=${script_dir}/data/3DTest_ug_new_0000.00007040.silo
${mpi} ../pion-ug \
 ${EPOCH_FILE} outfile=3DTest_ug_new omp-nthreads=${nt} \
 redirect=pion-3d-ug || exit 1

REF_FILE=3DTest_ug_new_0000.00008704.silo
NEW_FILE=`ls 3DTest_ug_new_0000.00008704.silo | tail -n1`
../silocompare . $NEW_FILE ${script_dir}/data $REF_FILE 0 cmp 2 > tmp.txt

if grep -q "RESULTS ARE THE SAME" tmp.txt; then
  echo -e "${GREEN}*** WIND-BINARY TEST (UG RUN) HAS BEEN PASSED ***"
  tail -n10 tmp.txt
  echo -e "*** WIND-BINARY TEST (UG RUN) HAS BEEN PASSED ***${NC}"
  exit 0
else
  echo -e "${RED}*** WIND-BINARY TEST (UG RUN) HAS BEEN FAILED ***"
  tail -n10 tmp.txt
  echo -e "*** WIND-BINARY TEST (UG RUN) HAS BEEN FAILED ***${NC}"
  exit 1 
fi
