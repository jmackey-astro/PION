 ...
 * 
 * Compile with:\n
 * g++ -Wall -DSERIAL stromgensphere.cc ../../testing/global.cc ../../testing/uniformGrid.cc ../../testing/dataio.cc -lreadline -lcfitsio
 * 
 * Run with (example):\n
 * ./a.out positions ../results/pec3D_n51_recomb 0 10 6
 * stromgensphere: <executable> <Outfile-base> <file-base> <FirstO/P-Num> <O/P-Freq> <x(HII)index>
 * */
...
int main(int argc, char **argv)
{
  ...
  if (argc!=6) {
    ...
    cerr << "stromgensphere: <executable> <OutfileName> <file-base> <FirstOutput> <OutputFreq> <x(HII) var>\n";
  ...
  do {
     ...
     do {
       ...
       nions += c->P[RO]/GS.m_p()*c->P[var]*cvol; // this is number of ions in cell.
     ...
     //err += fit_data(SimPM.Ncell, dist, ifrac, grid->DX()/2., grid->Range(XX), &a, &rchisq);
     ...
     outf <<SimPM.simtime <<"\t"<<a<<"\t"<<rchisq<<"\t"<<cvol;
