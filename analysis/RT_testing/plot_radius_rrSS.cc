 ...
 * assumed to be from a simulation with radiative recombinations on, but dynamics
 * off, so that the result should follow the classical stromgen sphere analysis.
 * 
 * This is explicitly serial code, so if there are parallel outputs to multiple
 * files this code won't work.
 * 
 * Compile with:\n
 * g++ -Wall -g -DSERIAL plot_radius_rrSS.cc ../../testing/global.cc ../../testing/uniformGrid.cc ../../testing/dataio.cc -lreadline -lcfitsio
 * 
 * Run with (example):\n
...
int main(int argc, char **argv)
{
  ...
  if (argc!=6) {
    ...
    cerr << "plot_radius: <executable> <Outfile-base> <Infile-base> <First-File-Num> <File-Num-Freq> <x(HII) var>\n";
  ...
  cout <<"\n\nreading from first file "<<infilebase<<"."<<startct<<".fits\n";
  cout <<"Writing I-front to files "<<outfilebase<<"."<<startct<<".fits\n";
  cout <<"**********************************************\n";
  ...
  for (int i=0;i<SimPM.ndim;i++) srcpos[i]=RSP.position[0][i];

  // Set up and open text outfile
  temp.str(""); temp << outfilebase <<".txt"; outfile=temp.str();
  ...
  outf.precision(6);
  outf << "# Stromgen Sphere angle-averaged outputs.  First file: "<<infile<<endl;
  outf << "# Columns are [time, I-front position, max.dev., min.dev., posn from Nions, Theoretical Posn.], and should be in cgs units (s,cm).\n\n";
  ...
  do {
     ...
     // get first point, and for each point calculate distance to source (at origin), and also the number of ionised atoms.
     cell *c = grid->FirstPt();
     int i=0;
     do {
       ...
       if (SimPM.coord_sys==COORD_CYL) cvol=2.*M_PI*c->x[Rcyl]*dx*dx;
       else {cvol=1.0; for (int d=0;d<SimPM.ndim;d++) cvol *=dx;}

       nions += c->P[RO]/GS.m_p()*c->P[var]*cvol; // num. ions in cell (for pure Hydrogen).
     ...
     temp.str("");
     temp << outfilebase <<"."<< count <<".fits"; outfitsfile=temp.str();
...
{
  double del=0.0;
  if      (SimPM.ndim==2) del = 0.7*sqrt(2.0)*grid->DX()/2.0;
  else if (SimPM.ndim==3) del = 0.7*sqrt(3.0)*grid->DX()/2.0;
	     ...
	     )
{
  ...
  cout <<"\tFound "<<ct<<" cells near I-front, with mean radius "<<*a_fit;
  cout <<" and width "<<*rcsq-*rcsq2<<"\n";
  return 0;

  cout <<"********* trying new method for weighted average.************\n";
  double cct=0.0, wt=0.0;
  rad=0.0; max_rad=-1.e199; min_rad=1.e199;
  for (int i=0;i<n;i++) {
    if (f[i]>=0.1 && f[i]<=0.9) {
