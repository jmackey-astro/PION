...
int get_theoretical_position(const double, ///< density in this column
...
int main(int argc, char **argv)
{
  ...
  if (argc!=6) {
    ...
    cerr << "parallelionisation: <executable> <Outfile-base> <Infile-base> <First-File-Num> <File-Num-Freq> <x(HII) var>\n";
  ...
  outf << "# Columns are [time, I-front positions/calc.posn.], and should be in cgs units (s,cm).\n\n";
  ...
  else if (SimPM.ndim==1) outf <<c->P[RO]/GS.m_p()<<"\n\n";
  ...
  if (!fs.file_exists(infile)) rep.error("Can't find second file to open",infile);
  ...
  do {
	 ...
	 err += fit_data(SimPM.NG[XX], dist, ifrac, grid->DX()/2., grid->Range(XX), &afit, &rchisq, &rchisq2);
       //cout <<"afit="<<afit<<endl;
	 ...
	 i++;
       ...
       err += fit_data(SimPM.NG[XX], dist, ifrac, grid->DX()/2., grid->Range(XX), &afit, &rchisq, &rchisq2);
       //cout <<"afit="<<afit<<endl;
       err += get_theoretical_position(grid->FirstPt()->P[RO]/GS.m_p(), &acalc);
       outf<<afit<<"  "<<acalc<<"  ";
       afit=0.0;
       for (int v=0;v<i;v++) afit+=ifrac[v];
       outf <<afit*grid->DX()<<"\n";
     } // if 1D
...
// Given a source, a geometry, and a sim-time, we can calculate the position of the I-front.
int get_theoretical_position(const double nh, ///< number density of gas in this column
