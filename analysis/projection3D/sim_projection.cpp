...
/// - 2009-06-23 Added in Doppler Broadening for line-of-sight velocity
/// profiles.
...
enum direction axes_directions::cross_product(
    const enum direction d1, const enum direction d2)
{
  if (d1 == d2) return NO;
...
coordinate_conversion::coordinate_conversion(
    ...
    const enum direction
        up_dir,  ///< vertical direction, which stays in image plane.
...
void coordinate_conversion::set_sim_extents_in_image_coords()
{
  ...
  if (zero_angle) {
    ...
    s_xmax_img[XX] =
        sim_ncell[sa[XX]] * costheta + sim_ncell[sa[ZZ]] * sintheta;
    s_xmax_img[ZZ] =
        sim_ncell[sa[XX]] * sintheta + sim_ncell[sa[ZZ]] * costheta;
...
void coordinate_conversion::set_npix()
{
  ...
  // (cos(t)+sin(t))/2 is the projected distance from the edge cell to the
  // corner of the domain, so adding 1-(this) means we are exactly 1 cell
  // further (in projected coords) than the last one.  So taking the integer
  // part of this means the last pixel always has at least one cell in it. Note
  // we need to take the abs value of sintheta, whereas costheta>0 on [-45,45]
  //
  im_npixels = 1;
  for (int v = 0; v < 2; v++) {
    im_npix[v] = static_cast<int>(
...
)
{
  ...
  // the point in question.  Then divide by dx to get it in units of number of
  // cells, which is the image unit.
  pion_flt delta[3];
  for (int v = 0; v < 3; v++) {
    if (ss[v] > 0)
      delta[v] =
          static_cast<pion_flt>(spos[sa[v]] - sim_xminI[sa[v]]) / sim_dxI;
    else
      delta[v] =
          static_cast<pion_flt>(sim_xmaxI[sa[v]] - spos[sa[v]]) / sim_dxI;
  }
...
image::image(
    ...
    const enum direction
        perp,  ///< vertical direction, which stays in image plane.
    class GridBaseClass *ggg  ///< pointer to grid of data.
    ) :
...
image::~image()
{
  if (cell_positions_set) delete_cell_positions();

  if (pix) {
    for (int i = 0; i < im_npixels; i++)
      delete_pixel_data(&(pix[i]));
    mem.myfree(pix);
  }
...
void image::add_integration_pts_to_pixels()
{
  ...
  for (int i = 0; i < im_npixels; i++) {
    ...
    // cout <<"\t------------------- NEXT PIXEL --------------- \n";
    //
    // Now for each pixel, we have initialised its points, set npt, dx, and
    // dx_phys
    //
  }
}
    ...
    pion_flt wt[4])
{
  if (!point_in_Isim_domain(x)) {
    ...
    for (int v = 0; v < 4; v++)
      if (ngb[v] && !ngb[v]->isgd) ngb[v] = 0;
    // if no cells are leaves, then set all ngb pointers to zero.
    // int r=0;
    for (int v = 0; v < 4; v++)
      if (ngb[v] && !ngb[v]->isleaf) ngb[v] = 0;  // r++;
    ...
    //     cout <<"\twt[] = ["<<wt[0]<<", "<<wt[1]<<", "<<wt[2]<<",
    //     "<<wt[3]<<"]"; cout <<"\tsum = "<<wt[0]+wt[1]+wt[2]+wt[3]; cout
    //     <<"\t-------------------\n";
...
)
{
  ...
  //
  for (int v = 0; v < 2; v++) {
    if ((cp[v] < p->ix[v]) || (cp[v] > (p->ix[v] + 1))) inside = false;
  }
...
void image::calculate_pixel(
    ...
    const struct vel_prof_stuff
        *vps,                     ///< struct with info for velocity binning.
  ...
  if (what_to_integrate == I_D) {
    ...
    *tot_mass += ans;
    ...
    for (int v = 1; v < (npt - 1); v++) {
      wt = 6 - wt;
      ans +=
          wt * get_point_Bremsstrahlung6GHz(&(px->int_pts.p[v]), SimPM.gamma);
    }
    ...
    *tot_mass += ans;
    im[px->ipix] = ans;
      ...
      what_to_integrate == I_B_STOKESQ || what_to_integrate == I_B_STOKESU
      || what_to_integrate == I_BXabs || what_to_integrate == I_BYabs
    ...
    // cout <<"using element "<<bz<<" for LOS, and "<<bx<<" for x-component;
    // theta="<<angle<<" deg.\n";

    if (what_to_integrate == I_B_STOKESQ) {
      ans += get_point_StokesQ(
          &(px->int_pts.p[0]), SimPM.ftr, bx, by, bz, signx, signy, signz, st,
          ct);
      for (int v = 1; v < (npt - 1); v++) {
        wt = 6 - wt;
        ans += wt
      ...
      *tot_mass += ans;
      ...
      ans = get_point_RotationMeasure(
          &(px->int_pts.p[0]), bx, bz, signx, signz, st, ct);
      for (int v = 1; v < (npt - 1); v++) {
        wt = 6 - wt;
        ans += wt
    ...
    //
    // This is quite simple -- VX is just a scalar quantity at each point, but
    ...
    class point_velocity VELX(
        vx, vz, ss[XX], ss[ZZ], theta, vps->v_min, vps->v_max, Nbins);
    //
    // Add line broadening, if needed, second arg is FWHM in code's velocity
    // units. If type==1, then smooth everything with FWHM. If type==2, use
    // point temperature to do standard doppler broadening at each pt.
    //
    if (vps->smooth == 1) {
      ...
      // cout <<"Broadening cell velocities by Doppler (thermal) broadening, on
      // point-by-point basis.\n";
    }
    //
    // Get VX and density at each point.  Velocity determines which
    // bin this point contributes too; density determines the amplitude it adds
    // to that bin.  The get_point_VX_profile() function does the binning for
    // us.
    //  This is Simpson's Rule Integration.
    //
    // The function get_point_VX_profile() puts the mass in the right velocity
    // bin in the temp_profile[] array, and if smooth==2 it will smooth this
    // with a Gaussian corresponding to the Doppler broadening from the point's
    // temperature.
    //
    ...
    //
    // Smooth profile: This only does anything if smooth==1, so constant
    // smoothing. If smooth==2, then the line broadening is already done on
    ...
    // the data cube.  So we skip npixel elements for each profile element.
    //
    for (int v = 0; v < Nbins; v++)
    ...
    *tot_mass = 0.0;
    ...
    if (vps->smooth == 1) {
      ...
      // cout <<"Broadening cell velocities by "<<vps->broadening<<" (FWHM) in
      // velocity units.\n";
    }
    else if (vps->smooth == 2) {
    ...
    //
    // Get the LOS velocity and density at each point.  Velocity determines
    // which bin this point contributes too; density determines the amplitude it
    // adds to that bin.  This is Simpson's Rule Integration.
    //
    // The function get_point_v_los_profile() puts the mass in the right
    // velocity bin in the temp_profile[] array, and if smooth==2 it will smooth
    // this with a Gaussian corresponding to the Doppler broadening from the
    // point's temperature.
    //
    ...
    *tot_mass = 0.0;
    ...
    //
    for (int v = 0; v < npt; v++) {
      get_point_Halpha_params(
          &(px->int_pts.p[v]), SimPM.ftr, SimPM.gamma, &alpha, &j);
      dtau = alpha * hh;
      if (dtau < 1e-7) {
        // We can approximate the exponential by Taylor Series to avoid roundoff
        // errors.
...
)
{
  ...
  get_point_Xray_params(
      &(pix->int_pts.p[npt - 1]), ftr, index, gamma, &alpha, &j);
  ans += j;
  ans *= hh / 3.0;
  tot_mass += ans;
}
...
point_velocity::point_velocity(
    const int
        vx_comp,  ///< velocity component perp. to LOS direction (contributing
...
{
  //
  // This is close to the NR version of the FFT.  Instead of trying to convert
  // it all for zero-offset arrays, I copied Martin White, and access the
...
{
  //
  // This works with Fourier Transform methods, rather than doing the
  // convolution integral.  We remove the mean, zero pad up to a power of 2, FT,
  // multiply by the FT of the Gaussian, IFT, and add back the mean.
  //
  if (broaden != 1) {
...
)
{
  ...
  for (int v = 0; v < v_Nbins; v++)
    prof[v] = 0.0;
  if (ibin > 0 && ibin < v_Nbins) prof[ibin] = norm;
  // cout <<"vx="<<vel<<" norm="<<norm<<" prof["<<ibin<<"] =
  // "<<prof[ibin]<<endl;
  if (broaden == 2) {
...
{
  //
  // This should be a delta-function input profile, and we will convolve this
  // with (i.e. multiply by) a Gaussian to get the doppler broadening.
  //
  if (fabs(vel) < TINYVALUE && norm < TINYVALUE) return;
  ...
  double sigma2 =
      1.530e6
      * T;  // this includes the factor of 2 in the Gaussian exponential.
  double sigma = sqrt(
      M_PI
      * sigma2);  // this includes the sqrt(2pi) in the Gaussian normalisation.
  ...
  for (int v = 0; v < v_Nbins; v++) {
    ...
    prof[v] =
        norm * temp
        * v_binsize;  // normalise correctly by multiplying by interval width.
  }
...
double point_velocity::get_point_los_velocity(const struct point_4cellavg *pt)
{
  ...
  for (int v = 0; v < 4; v++) {
    if (pt->ngb[v] && pt->ngb[v]->isleaf)
      val += pt->wt[v]
             * (sx * pt->ngb[v]->P[vx] * st + sz * pt->ngb[v]->P[vz] * ct);
  }
  //  cout <<"sx="<<sx<<" sz="<<sz;
...
double point_velocity::get_point_perp_velocity(const struct point_4cellavg *pt)
{
  ...
  for (int v = 0; v < 4; v++) {
    if (pt->ngb[v] && pt->ngb[v]->isleaf)
      val += pt->wt[v]
             * (sx * pt->ngb[v]->P[vx] * ct - sz * pt->ngb[v]->P[vz] * st);
  }
...
)
{
  ...
  if (vel < v_min) {
    ...
    if (vel < v_min + ibin * v_binsize) {
      ...
      // cout.width(20);
      // cout <<" v_min="<<v_min<<"
      // v_max="<<v_max<<"\tvm+i*sze="<<v_min+ibin*v_binsize; cout.width(20);
