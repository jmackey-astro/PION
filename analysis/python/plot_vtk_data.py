...
parser = argparse.ArgumentParser(description='Process some values.',
 usage='plot_vtk_data.py <path-to-files> <base-filename> <image-path> <image-filename>')
...
# Add column density plot to upper half-plane
  ...
  norm = mpl.colors.Normalize(0, vmax=Sigma.max())
  im1.set_norm(norm) 
  plt.colorbar(im1, format="%5.2e")
