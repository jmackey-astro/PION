...
int main(int argc, char **argv)
{
  ...
  outf << "# Columns are time and I-front position (and from nions), and should be in cgs units (s,cm).\n\n";
  ...
  do {
     ...
     if (SimPM.coord_sys==COORD_CYL) 
       nions = pow(nions/(2.*M_PI/3.0 +M_PI*dx/2./pow(3.*nions/2./M_PI,1./3.)), 1./3.);
     else {
       if (SimPM.ndim==2) nions=sqrt(4.*nions/M_PI);
       if (SimPM.ndim==3) nions=pow(6.*nions/M_PI,1./3.);
     }
     cout <<"******* "<<SimPM.simtime <<"\t"<<a<<"\t"<<rchisq<<"\t"<<nions<<endl;
     outf <<SimPM.simtime <<"\t"<<a<<"\t"<<rchisq<<"\t"<<nions<<endl;
