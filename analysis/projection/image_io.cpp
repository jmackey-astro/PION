...
string image_io::get_output_filename(
    ...
    const int
        multi_opfiles,      ///< multi_opfiles: 0=single file, 1=multiple files.
  ...
  switch (op_filetype) {
    ...
    case 1:
      ...
      if (file_exists(fitsfile)) {
        spdlog::info("\tfits file exists: {} ...overwriting!", fitsfile);
        ostringstream temp;
        temp.str("");
        temp << "!" << fitsfile;
        fitsfile = temp.str();
      ...
      spdlog::info(
          "***** Created fits file: {}, file pointer={}", fitsfile,
          fmt::ptr(ff));
      break;
    ...
    case 3:
      ...
      if (fvtk)
        spdlog::error(
            "{}: {}", "Output VTK file pointer is not null!", fmt::ptr(fvtk));
...
#ifndef NDEBUG
  spdlog::debug(
      "saving data: {}, type={}.  image = {}  {}  {}  {}", f, op_filetype,
      num_pix, im_dim, npix[0], npix[1]);
  spdlog::debug(
      "  {}  {}  {}  {}  {}  {}  {}", name, xmin[0], xmin[1], dx[0], dx[1],
      time, cycle);
#endif
  ...
  switch (op_filetype) {
    ...
    case 3:
      ...
      if (!vtk_header_written) {
        ...
        if (im_dim > 1)
          ofn << static_cast<int>(npix[1] + 1) << " ";
        else
          ofn << "2 ";
        if (im_dim > 2)
          ofn << static_cast<int>(npix[2] + 1) << "\n";
        else
          ofn << "2 \n";
...
void image_io::SWAP_ENDIAN(void *x, const int nbytes)
///
...
{
  if (nbytes > 16) {
    spdlog::error("swap-endian buffer is too large! Max=16bytes");
    return;
  }
  int k;
