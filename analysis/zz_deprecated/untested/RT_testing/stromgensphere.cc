...
int main(int argc, char **argv)
{
  ...
  outf << "# Radiative Shock Test Problem outputs.  First file: " << infile
       << endl;
  outf
      << "# Columns are [time, I-front position, err, posn from Nions, Nions, Nphotons], and should be in cgs units (s,cm).\n\n";
  ...
  do {
    ...
    do {
      ...
      nions += c->P[RO] / GS.m_p() * c->P[var]
               * cvol;  // this is number of ions in cell.
      i++;
    ...
    // err += fit_data(SimPM.Ncell, dist, ifrac, grid->DX()/2., grid->Range(XX),
    // &a, &rchisq);
