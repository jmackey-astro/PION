...
#define MACHINEACCURACY                                                        \
  5.e-16  ///< This should be found out before running the code!
...
#elif defined(MHD)
#define NVAR                                                                   \
  (2 + 2 * EQNDIM)  // density, pressure, plus velocity and magnetic field.
#else
...
public:
  ...
  int timestep; /**< \brief Integer count of the number of timesteps since the
                   start. */
  // Grid Point data
  ...
  double dA;  ///< Area of one surface of the (uniform, cubic, cartesian) grid
              ///< cells.
  double
      dV;  ///< Volume of one cubic, cartesian grid cell (same for all cells).
  // Boundary cell data.
  ...
  double CFL; /**< \brief Courant factor, must be less than one, should be about
                 0.5.*/
...
public:
  ...
  double P[NVAR]; /**< \brief Primitive Variables. For 1D-HD they are \f$ [\rho,
                     u, p]\f$. */
...
public:
  ...
  int assignNGB()
  {
    ...
    do {
      ...
      if ((t = (cpt->id + 1) % nx) > cpt->id % nx)
        cpt->ngb[XP] = &data[cpt->id + 1];
      if ((t = (cpt->id - 1 + nx) % nx) < cpt->id % nx)
        cpt->ngb[XN] = &data[cpt->id - 1];
  ...
  cell *nextPt(const cell *cpt, const direction dir) { return (cpt->ngb[dir]); }
...
bool equalD(const double a, const double b)
{
  ...
  if ((fabs(a - b) / fabs(a + b + 1.e-100)) < /*10.* MACHINEACCURACY*/ 1.e-6)
...
int main(int argc, char **argv)
{
  if (argc != 4) {
    cerr
        << "Main: must call with 3 arguments... <main> <restartfile.h5> <outfile> <param>"
        << endl;
    exit(1);
  }
  ...
  if (err != 0) {
    cerr << "failed to get parameters.\n";
    return (1);
  }
  const int ncell = gp.Ncell;
  cout << "\tNcell: " << gp.Ncell << "\t" << gp.NG[XX] << "\t" << gp.NG[YY]
       << "\t" << endl;
  ...
  err += fits_write_key(
      ff, TINT, "gridType", &gp.gridType, "UniformGrid=1", &status);
  err +=
      fits_write_key(ff, TINT, "eqnType", &gp.eqnType, "HD=1,MHD=2", &status);
  err += fits_write_key(
      ff, TINT, "solverType", &gp.solverType, "LF=1,GD=2", &status);
  err += fits_write_key(
      ff, TINT, "gridNDim", &gp.gridNDim, "Num. spatial dimensions of grid",
      &status);
  err += fits_write_key(
      ff, TINT, "eqnNDim", &gp.eqnNDim, "Num. spatial dimensions in vectors",
      &status);
  err += fits_write_key(
      ff, TINT, "eqnNVar", &gp.eqnNVar, "Num. variables in equations", &status);
  err += fits_write_key(
      ff, TDOUBLE, "simtime", &gp.simtime, "current time in simulation",
      &status);
  err += fits_write_key(
      ff, TDOUBLE, "starttime", &gp.starttime,
      "initial time to start simulation", &status);
  err += fits_write_key(
      ff, TDOUBLE, "finishtime", &gp.finishtime,
      "Time to finish the simulation", &status);
  err += fits_write_key(
      ff, TINT, "timestep", &gp.timestep, "TIMESTEP SINCE START OF SIM",
      &status);
  err += fits_write_key(
      ff, TINT, "NG[XX]", &gp.NG[XX], "Number of grid-points in X-direction",
      &status);
  if (GNDIM > 1)
    err += fits_write_key(
        ff, TINT, "NG[YY]", &gp.NG[YY], "Number of grid-points in Y-direction",
        &status);
  if (GNDIM > 2)
    err += fits_write_key(
        ff, TINT, "NG[ZZ]", &gp.NG[ZZ], "Number of grid-points in Z-direction",
        &status);
  err += fits_write_key(
      ff, TINT, "Ncell", &gp.Ncell, "Total Number of Grid Points", &status);
  g.pt = g.firstPt();
  for (int i = 0; i < GNDIM; i++)
    pix[i] = 1;
  do {
    pix[XX] = static_cast<int>(
        (g.pt->x[XX] + gp.dx / 1.99) / gp.dx);  // truncates to integer part
    pix[YY] = static_cast<int>((g.pt->x[YY] + gp.dx / 1.99) / gp.dx);
    // fits_write_col(*fptr, datatype, colnum, firstpix, firstelem, nelements,
    // data, &status)
    if (param == -1) {
  ...
  do {
    ...
    pix[YY] = static_cast<int>( (g.pt->x[YY]+gp.dx/1.99)/gp.dx);
    //fits_write_col(*fptr, datatype, colnum, firstpix, firstelem, nelements,
  data, &status) err += fits_write_pix(ff, TDOUBLE, pix, 1, &(g.pt->P[PG]),
  &status); } while ( (g.pt=g.nextPt(g.pt))!=0 ); if(status)
  fits_report_error(stderr,status);
  */

  /*  int  fits_create_tbl(fitsfile ptr.to.file, ///< fits file pointer.
        int tbltype=bin/acs, ///<
        int num.reserved.rows,///< should be set to zero unless i have a good
     reason not to. int num.cols, ///< number of columns of table. char**
     col.name.arr, ///< array of c_strings with names of columns char**
     col.datattype.arr, ///< array of c_strings with datatypes
     J=int,E=float,D=double char** col.phys.units.arr, ///< array of phsyical
     units, can be null cahr* table.name, ///< name of table, can be null. int
     &status ///< error code for returning.
        )
  */
  /*
...
#endif
  string colnames[] =
{"X","Y","Density","Gas-Pressure","X-Velocity","Y-Velocity","Z-Velocity","InternalEnergy","Bx","By","Bz","divB"};
  string coltypes[] =
{"1D","1D","1D","1D","1D","1D","1D","1D","1D","1D","1D","1D"};
//  if(GNDIM==2) coltypes[0] = "2D"; if(GNDIM==3) coltypes[0] = "3D";
  string siunits[] =
{"m","m","kg/m^3","kg/m/s^2","m/s","m/s","m/s","J","T","T","T","T/m"}; cout
<<"setup string arrays\n"; cout <<"position vector: "<<coltypes[0]<<endl; char
*ttype[ncols]; char *tform[ncols]; char *tunit[ncols]; for (int i=0; i<ncols;
i++) { ttype[i] = new char [nels]; tform[i] = new char [nels]; tunit[i] = new
char [nels];
  }
...
ncols,ttype,tform,tunit,tblname,&status);
  //  err += fits_modify_vector_len(ff,1,GNDIM,&status); // change 1st column to
n-dim vector.


  err += fits_write_key(ff, TINT, "gridType"  ,&gp.gridType, "UniformGrid=1",
&status); err += fits_write_key(ff, TINT, "eqnType"   ,&gp.eqnType,
"HD=1,MHD=2", &status); err += fits_write_key(ff, TINT,
"solverType",&gp.solverType, "LF=1,GD=2", &status); err += fits_write_key(ff,
TINT, "gridNDim",  &gp.gridNDim, "Num. spatial dimensions of grid", &status);
  err += fits_write_key(ff, TINT, "eqnNDim",   &gp.eqnNDim,  "Num. spatial
dimensions in vectors", &status); err += fits_write_key(ff, TINT, "eqnNVar",
&gp.eqnNVar,  "Num. variables in equations", &status); err += fits_write_key(ff,
TDOUBLE, "simtime",&gp.simtime, "current time in simulation", &status); err +=
fits_write_key(ff, TDOUBLE, "starttime",&gp.starttime, "initial time to start
simulation", &status); err += fits_write_key(ff, TDOUBLE,
"finishtime",&gp.finishtime, "Time to finish the simulation", &status); err +=
fits_write_key(ff, TINT, "timestep",&gp.timestep, "TIMESTEP SINCE START OF SIM",
&status); err += fits_write_key(ff, TINT, "NG[XX]",&gp.NG[XX], "Number of
grid-points in X-direction", &status); if(GNDIM>1) err += fits_write_key(ff,
TINT, "NG[YY]",&gp.NG[YY], "Number of grid-points in Y-direction", &status);
  if(GNDIM>2)
    err += fits_write_key(ff, TINT, "NG[ZZ]",&gp.NG[ZZ], "Number of grid-points
in Z-direction", &status); err += fits_write_key(ff, TINT, "Ncell",&gp.Ncell,
"Total Number of Grid Points", &status);
...
data, &status) err += fits_write_col(ff, TDOUBLE, 1, row, 1, 1, &g.pt->x[XX],
&status); err += fits_write_col(ff, TDOUBLE, 2, row, 1, 1, &g.pt->x[YY],
&status); err += fits_write_col(ff, TDOUBLE, 3, row, 1, 1, &(g.pt->P[RO]),
&status); } while ( (g.pt=g.nextPt(g.pt))!=0 );
*/
  ...
  /*  ofstream opf(opfile.c_str());
    ...
    g.pt = g.firstPt();
    do {
      if ((g.nextPt(g.pt,XN)!=0) && (g.nextPt(g.pt,YP)!=0) &&
  !equalD(g.pt->P[PG],(g.nextPt(g.nextPt(g.pt,XN),YP))->P[PG])) {
        cout<<"Warning, ngb not the same! "<<g.pt->id;
        cout<<"\tcpt: "<<g.pt->P[PG]<<"\tnpt:
  "<<(g.nextPt(g.nextPt(g.pt,XN),YP))->P[PG]<<endl;
      }
  ...
  #ifdef MHD
      b2 = g.pt->P[BX]*g.pt->P[BX] +g.pt->P[BY]*g.pt->P[BY]
  +g.pt->P[BZ]*g.pt->P[BZ]; opf <<"  "<< g.pt->P[BX] <<"  "<< g.pt->P[BY] <<"
  "<< g.pt->P[BZ] <<"  "<< g.pt->P[PG]+b2/2.<<"  "<< g.divB(g.pt); #endif //MHD
      opf  <<endl;
      //if (g.nextPt(g.pt,YP) !=0) {cout<<"getting next point: id=";
  g.pt=g.nextPt(g.pt,YP);cout <<g.pt->id<<endl;} } while (
  (g.pt=g.nextPt(g.pt,XP))!=0 && (g.pt=g.nextPt(g.pt,YP))!=0);
...
int getParametersHDF5(string infile)
{
  ...
  if ((myh5t_dblNDVec = H5Tarray_create(H5T_NATIVE_DOUBLE, 1, NDArray, 0))
      < 0) {
    cerr << "myHDF5: Couldn't create state vector datatype.\n";
    return (1);
  }
  ...
  err                  = H5Tset_size(myh5t_String32, 32);
  myh5error(err, "Set string datatype");
  hid_t myh5t_String128 = H5Tcopy(H5T_C_S1);
  err                   = H5Tset_size(myh5t_String128, 128);
  myh5error(err, "Set string datatype");
  hid_t myh5t_GP;
  ...
  err = H5Tinsert(
      myh5t_GP, "Grid Type", HOFFSET(GridParams, gridType), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Eqn. Type", HOFFSET(GridParams, eqnType), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Solver Type", HOFFSET(GridParams, solverType), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Grid NDIM", HOFFSET(GridParams, gridNDim), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Eqn. NDIM", HOFFSET(GridParams, eqnNDim), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Eqn. NVAR", HOFFSET(GridParams, eqnNVar), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Sim Time", HOFFSET(GridParams, simtime), H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Start Time", HOFFSET(GridParams, starttime),
      H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Finish Time", HOFFSET(GridParams, finishtime),
      H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Time Step", HOFFSET(GridParams, timestep), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "NGrid[GNDIM]", HOFFSET(GridParams, NG), myh5t_intNDVec);
  err += H5Tinsert(
      myh5t_GP, "Total NCell", HOFFSET(GridParams, Ncell), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Range[GNDIM]", HOFFSET(GridParams, range), myh5t_dblNDVec);
  err += H5Tinsert(
      myh5t_GP, "Xmin[GNDIM]", HOFFSET(GridParams, xmin), myh5t_dblNDVec);
  err += H5Tinsert(
      myh5t_GP, "Xmax[GNDIM]", HOFFSET(GridParams, xmax), myh5t_dblNDVec);
  err += H5Tinsert(
      myh5t_GP, "Cell diameter", HOFFSET(GridParams, dx), H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Cell Face Area", HOFFSET(GridParams, dA), H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Cell Vol. Area", HOFFSET(GridParams, dV), H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Type Of B.C.", HOFFSET(GridParams, typeOfBC), myh5t_String32);
  err += H5Tinsert(
      myh5t_GP, "Total Point No.", HOFFSET(GridParams, Nbc), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Space OOA", HOFFSET(GridParams, spOOA), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Time  OOA", HOFFSET(GridParams, tmOOA), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "ArtVisc Flag", HOFFSET(GridParams, artviscosity),
      H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Gas Cnst Gamma", HOFFSET(GridParams, gamma),
      H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "CFL parameter", HOFFSET(GridParams, CFL), H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Visc parameter", HOFFSET(GridParams, etav), H5T_NATIVE_DOUBLE);
  err += H5Tinsert(
      myh5t_GP, "Type of O/P", HOFFSET(GridParams, typeOfOP), H5T_NATIVE_INT);
  err += H5Tinsert(
      myh5t_GP, "Output File", HOFFSET(GridParams, outFileBase),
      myh5t_String128);
  err += H5Tinsert(
      myh5t_GP, "O/P Frequency", HOFFSET(GridParams, opfreq), H5T_NATIVE_INT);
  ...
  /*  cout <<"\tReading in Units...";
    ...
    cout <<" Done.\n";
  */
  myh5_err = H5Tclose(myh5t_GP);
  myh5error(myh5_err, "Closing Param dtype");
  myh5_err = H5Tclose(myh5t_UC);
  myh5error(myh5_err, "Closing Unit  dtype");
  myh5_err = H5Tclose(myh5t_String32);
  myh5error(myh5_err, "Closing str32 dtype");
  myh5_err = H5Tclose(myh5t_String128);
  myh5error(myh5_err, "Closing st128 dtype");
  myh5_err = H5Tclose(myh5t_intNDVec);
  myh5error(myh5_err, "Closing NDvec dtype");
  myh5_err = H5Tclose(myh5t_dblNDVec);
  myh5error(myh5_err, "Closing NDvec dtype");
...
int tempgrid::assignDataFromHDF5(string infile)
{
  ...
  // The grid parameters must be already read, and the grid set up to hold the
  // data.

  int nel = gp.Ncell;
  double *dblarr;
  int *intarr;
  dblarr = new double[nel];
  if (!dblarr) {
    cerr << "memory assignment dblarr.\n";
    exit(1);
  }
  intarr = new int[nel];
  if (!intarr) {
    cerr << "memory assignment intarr.\n";
    exit(1);
  }
  //  hsize_t dim[1];
  //  dim[0] = nel;
  ...
  herr_t myh5_err = H5Dread(
      myh5_Dset, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, dblarr);
  ...
  if (nel != i) {
    cerr << "Not Correct Number of points read\n";
    exit(1);
  }
  myh5_err = H5Dclose(myh5_Dset);
  myh5error(myh5_err, "Closing myh5_Dset");
  if (GNDIM > 1) {
  ...
  // cout<<" Reading IDs...";
  myh5_err =
      H5Dread(myh5_Dset, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, intarr);
  ...
  for (int vct = 0; vct < NVAR; vct++) {  // Loop 5 or 8 times, for HD and MHD.
    ...
    myh5error(myh5_err, "Closing myh5_Dset");
    // cout <<"Read, assigning to grid... ";
    // Assign data to grid: Assumes grid is set up for same point ordering as
    // was written.
    cpt                = firstPt();
    i                  = 0;
    enum primitive var = static_cast<primitive>(vct);
    do {
      cpt->P[var] = dblarr[i];
      i++;
    } while ((cpt = nextPt(cpt)) != 0);
    ...
    if (i != gp.Ncell) {
      cerr << "Error: too many gridpoints in output!\n";
      exit(100);
    }
    // cout <<"\tDone.\n";
  }
  myh5_err = H5Sclose(myh5_Dspace);
  myh5error(myh5_err, "Closing myh5_Dspace dataspace");
  myh5_err = H5Gclose(myh5_Group);
  myh5error(myh5_err, "Closing myh5_Group");
  myh5_err = H5Fclose(myh5_InFile);
  myh5error(myh5_err, "Closing myh5_InFile file");
  delete[] dblarr;
  delete[] intarr;

  //  cpt = firstPt();
  //  do {for(int v=0;v<NVAR;v++) cpt->Ph[v]=cpt->P[v];} while
  //  ((cpt=nextPt(cpt))!=0);
  return (0);
}
