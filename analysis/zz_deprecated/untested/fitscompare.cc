...
int main(int argc, char **argv)
{
  ...
  cout
      << "Note the output file contains the absolute error, as opposed to relative error.\n";
  cout
      << "You should use \'fdiff\' from the command line to see if the headers differ.\n";
  ...
  int bitpix1, bitpix2;
  int naxis1;
  long int *naxes1 = new long int[3];
  int naxis2;
  long int *naxes2 = new long int[3];
  double nulval    = -1.e99;
  int anynul       = 0;
  ...
  do {
    ...
    err +=
        fits_read_img(ff1, TDOUBLE, 1, npix, &nulval, array1, &anynul, &status);
    if (status) {
      fits_report_error(stderr, status);
      return (err);
    }
    ...
    err +=
        fits_read_img(ff2, TDOUBLE, 1, npix, &nulval, array2, &anynul, &status);
    if (status) {
      fits_report_error(stderr, status);
      return (err);
    }
    // write (d1-d2)/(d1+d2) (or 0 if d1,d2=0) into array3.
    double maxval = -1.e99, minval = 1.e99;
    double meanval = 0;
    for (long int j = 0; j < npix; j++) {
      //      array3[j] =
      //      (fabs(array1[j])-fabs(array2[j]))/(fabs(array1[j])+fabs(array2[j]));
    ...
    cout.precision(6);
    cout << "max diff = " << maxval << "\t and min diff = " << minval
         << "\t and meanval=" << meanval / npix << endl;
