...
int main(int argc, char **argv)
{
  if (argc != 3) {
    cerr
        << "Main: must call with 2 arguments... <main> <restartfile.h5> <outfile>"
        << endl;
    exit(1);
  }
  ...
  double *origin = new double[GNDIM];
  for (int i = 0; i < GNDIM; i++)
    origin[i] = 0.;
  ...
  do {
    if ((g.nextPt(g.pt, XN) != 0) && (g.nextPt(g.pt, YP) != 0)
      ...
      cout << "\tcpt: " << g.pt->P[PG]
           << "\tnpt: " << (g.nextPt(g.nextPt(g.pt, XN), YP))->P[PG] << endl;
    }
    ...
    b2 = g.pt->P[BX] * g.pt->P[BX] + g.pt->P[BY] * g.pt->P[BY]
         + g.pt->P[BZ] * g.pt->P[BZ];
    opf << "  " << g.pt->P[BX] << "  " << g.pt->P[BY] << "  " << g.pt->P[BZ]
        << "  " << g.pt->P[PG] + b2 / 2. << "  " << g.divB(g.pt);
#endif  // MHD
    opf << endl;
    // if (g.nextPt(g.pt,YP) !=0) {cout<<"getting next point: id=";
    // g.pt=g.nextPt(g.pt,YP);cout <<g.pt->id<<endl;}
  } while ((g.pt = g.nextPt(g.pt, XP)) != 0
           && (g.pt = g.nextPt(g.pt, YP)) != 0);
