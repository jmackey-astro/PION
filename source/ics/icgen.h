...
class IC_basic_tests : public ICsetup_base {
 ...
 protected:
    ...
    * */
   int setup_uniformgrid(class ReadParams *, ///< pointer to parameter list.
			 class GridBaseClass * ///< pointer to grid
			 );
...
class IC_blastwave : public ICsetup_base {
  ...
  double interface; ///< optional position of interface between 2 ambient media.
  void get_amb2_params();
   /** \brief Set up a blast wave in cylindrical symmetry, centred on the origin. */
   int setup_cyl_bw();
   /** \brief Set up a blast wave in cartesian geometry, at the centre of the grid. */
   int setup_cart_bw();
...
class IC_shock_cloud : public ICsetup_base {
  ...
  protected:
   ...
   int eqns;   ///< set to 1 for euler eqns, 2 for mhd equations (including all divB cleaning methods)
   double
...
class IC_shocktube : public ICsetup_base {
  ...
  protected:
   ...
   int assign_data(double *, ///< left state.
		   double *, ///< right state.
		   double  ///< shock position (in units of X-range)
		   ); ///< given angles,shockpos,l,r, set data.
   int get_riemann_ics(int, ///< test number to run
		       double *, ///< left state.
		       double *, ///< right state.
...
class IC_jet : public ICsetup_base {
  ...
  protected:
   int 
     ...
     eqns,   ///< set to 1 for euler eqns, 2 for mhd equations (including all divB cleaning methods)
...
class IC_photoevaporatingclump : public ICsetup_base {
  ...
  protected:
   ...
   int setup_radialprofile(); ///< setup data for a density profile out from source.
   int setup_powerlaw_density(); ///< data for a slab/axi-symmetric power law density profile.
   int setup_paralleltest();  ///< setup data for parallel rays with differing density in each ray.
   ///
...
struct clump {
  ...
  double centre[MAX_DIM]; ///< Position of centre of clump (same units as grid).
  double size[MAX_DIM];   ///< Scale radius of clump in each direction in its principal axes frame.
  double ang[MAX_DIM];    ///< Rotation vector [alpha,beta,gamma] from grid axes to principal axes.
  double rm[MAX_DIM][MAX_DIM]; ///< Rotation matrix associated with orientation[].
...
class IC_photevap_random_clumps : public ICsetup_base {
 ...
 protected:
  ...
  int profile; ///< flag to say what kind of radial density profile to use (0=top-hat, 1=Gaussian)
  double
    min_overdensity, ///< minimum overdensity of clumps (multiple of background)
    ...
    *cltr,    ///< value of optional tracers for cloud.
    ...
    gam,      ///< eos gamma.
    *ambient, ///< ambient state vector.
    ambdens;  ///< ambient gas density (after taking clump mass out of total mass).
  struct clump *cl;
  ...
  int clumps_random_setup(); ///< setup clumps (original version, random total mass).
  int clumps_random_setup_fixedmass(); ///< setup clumps (fixed total mass).
#ifdef PARALLEL
  int clumps_random_setup_pllel(); ///< Setup clumps in parallel (orig. variable total mass).
  int clumps_random_setup_pllel_fixedmass(); ///< setup clumps in parallel (fixed total mass).
#endif // PARALLEL
  int clumps_random_set_dens(cell * ///< cell to operate on.
			     );
  void print_clump(struct clump *);
  double random_frac(); ///< returns random value on [0,1], assuming seed is set.
};
...
struct random_clump_data {
  ...
  double *border;  ///< 2*ndim doubles saying what fraction of the domain in each direction has no clumps.
  double density,  ///< mean number density of gas to go into clumps (within clumpy region).
    total_mass,    ///< total mass to draw clumps from.
    min_mass,      ///< min. fraction of total mass that a clump can have (for fixed mass clumps).
    max_mass,      ///< max. fraction of total mass that a clump can have (for fixed mass clumps).
...
/** \brief Setup a 2D or 3D cartesian domain with a number of random clumps
 * (random in position, size, orientation, overdensity), with a radiation source
 * either on or off grid, or at infinity, and a number of strategically positioned
 * clumps with specified properties.
 */
class IC_photevap_multi_clumps : public ICsetup_base {
 ...
 protected:
  ...
  int coords; ///< coord sys used
  int eqns;   ///< set to 1 for euler eqns, 2 for mhd equations (including all divB cleaning methods)
  double ambdens;  ///< ambient gas density (after taking clump mass out of total mass).
  double ambdivider; ///< dividing x-value between the two ambient medium states.
  ...
  //
  int get_ambient_params(class ReadParams *,
			 struct ambient_data *
			 );
  int add_ambient_data_to_grid(class GridBaseClass *,
			       struct ambient_data *
			       );
  ...
  //
  int get_alternate_ambient_params(class ReadParams *,
				   struct ambient_data *
				   );
  int add_alternate_ambient_data_to_grid(class GridBaseClass *,
					 struct ambient_data *
					 );
  //
  ...
  double random_frac();
  void print_clump(struct clump *);
  int clumps_set_dens(class cell *,   ///< cell to operate on.
  ...
  //
  int get_random_clump_params(class ReadParams *,
			      struct random_clump_data *
			      );
  int rc_fixed_mass_range_params(class ReadParams *,
				 struct random_clump_data *
				 );
  int rc_fixed_number_params(class ReadParams *,
			     struct random_clump_data *
			     );
  /** \brief This function is hardcoded for Gaussian density distributions (for total mass).
   * It uses an analytic result to go from mass and size to peak overdensity.
   */
  int rc_set_clump_properties(struct random_clump_data *
			      );
  int add_random_clumps_to_grid(class GridBaseClass *,
				struct random_clump_data *
				);
  //
  // Strategically Placed Clumps Routines
  //
  int get_strategic_clump_params(class ReadParams *,
				 struct strategic_clump_data *
				 );
  int add_strategic_clumps_to_grid(class GridBaseClass *,
				   struct strategic_clump_data *
				   );
};
...
class IC_spherical_clump : public ICsetup_base {
  ...
  int
    SC_pressure_profile, ///< cloud pressure scaling 1=isothermal, 2=constant pressure.
    SC_density_profile;  ///< cloud density profile: 0=top-hat, 1=isothermal, 2=Gaussian.
};
