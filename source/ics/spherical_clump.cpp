			     ...
			     )
{
  ...
  // for 1/r^2 this is the scale radius of the constant density core (rho/(1+(r/rc)^2))
  // for gaussian this is sigma in the exp(-r^2/(2sigma^2))
  //
  seek = "SC_radius";
  str = rp->find_parameter(seek);
  if (str=="") rep.error("didn't find parameter",seek);
  ...
  seek = "SC_magfieldX";
  str = rp->find_parameter(seek);
  if (str=="") IC_spherical_clump::SC_BX = 0.0; // could have no field.
  else         IC_spherical_clump::SC_BX = atof(str.c_str());
  seek = "SC_magfieldY";
  str = rp->find_parameter(seek);
  if (str=="") IC_spherical_clump::SC_BY = 0.0; // could have no field.
  else         IC_spherical_clump::SC_BY = atof(str.c_str());
  seek = "SC_magfieldZ";
  str = rp->find_parameter(seek);
...
int IC_spherical_clump::setup_clump()
{
  ...
  //
  double centre[ndim];
  for (int i=0;i<ndim;i++) centre[0] = 0.0;
  ...
  do {
    ...
    switch (SC_density_profile) {
      ...
      case 2: // Gaussian
      cpt->P[RO] *= SC_overdensity*
                exp(-0.5*pow(gg->distance_vertex2cell(centre,cpt)/SC_rad,2.0));
      break;
    ...
    switch (SC_pressure_profile) {
      ...
      case 2:
      // constant pressure equal to the ambient pressure.  So do nothing.
      break;
