    ...
    string& test)
{
    ...
    //
    int test_id = -1;
    if (test == "StarBench_ContactDiscontinuity1")
        test_id = 1;
    else if (test == "StarBench_ContactDiscontinuity2")
        test_id = 2;
    else if (test == "StarBench_ContactDiscontinuity3")
        test_id = 3;
    else if (test == "StarBench_ContactDiscontinuity4")
        test_id = 4;
    ...
    if (SimPM->ntracer != 1) {
        rep.error(
            "Need exactly one colour tracer for CD tests", SimPM->ntracer);
    }
    ...
    if (test_id == 1) {
        ...
        //
        double theta = 1.0;
        double tt = tan(theta), itt = 1.0 / tt, ifst = 1.0 / (4.0 * sin(theta));
        //
    ...
    string& test)
{
    ...
    if (seek != "") {
        ...
        cout << "Using given IF_pos=" << IF_pos
             << " cm.  Calculated value=" << tmp << " cm\n";
    }
    else {
            ...
            for (int iy = 0; iy < nsub; iy++) {
                for (int ix = 0; ix < nsub; ix++) {
                    ...
                    if (ptype == 2) {
                        ...
                        deflection = A
                                     * sin(2.0 * M_PI
                                           * (psub[YY] + 0.5 * SimPM->Range[YY])
                                           / lambda);
                    }

                    if (psub[XX] <= IF_pos + deflection)
                        f_dn += vfrac;
                    else if (psub[XX] <= shock_pos + deflection)
                        f_sh += vfrac;
                    else
                        f_up += vfrac;
                }
    ...
    // ----------------------------------------------------------------
        ...
        IF_pos =
            0.65 * SimPM->RS.sources[0].strength / IF_pos + ggg->SIM_Xmin(XX);
        //
        ...
        do {
            ...
            if (pos[XX] <= IF_pos) {
                ...
                deltarho =
                    amp
                    * sin(2.0 * M_PI * (pos[YY] + 0.5 * ggg->SIM_Range(YY))
                          / lambda);
    ...
    string& test)
{
    ...
    int id = 0;
    if (test == "StarBench_IFI_testA")
        id = 1;
    else if (test == "StarBench_IFI_testB")
        id = 2;
    ...
    string& test)
{
    ...
    int id = 0;
    if (test == "StarBench_IrrCloud_Uniform")
        id = 1;
    ...
    do {
        ...
        c->P[PG] =
            50.0 * pconst.kB() * 1000.0;  // Just pick some temperature...
        c->P[VX] = c->P[VY] = c->P[VZ] = 0.0;
        for (int v = 0; v < SimPM->ntracer; v++)
            c->P[SimPM->ftr + v] = 0.0;
    } while ((c = ggg->NextPt(c)) != 0);
    ...
    if (id == 1) {
        ...
        do {
            ...
            dist = gg->distance(cl_centre, pos);
            rho_cell =
                rho_cl * r_core * r_core / (r_core * r_core + dist * dist);
    ...
    string& test)
{
    ...
    do {
        ...
        c->P[PG] = 2.0 * c->P[RO] * pconst.kB() * 1.0e4
                   / pconst.m_H();  // 10000K ionised gas (pure H).
        c->P[VX] = c->P[VY] = c->P[VZ] = 0.0;
        for (int v = 0; v < SimPM->ntracer; v++)
