			     ...
			     )
{
  ...
  seek = "STshockpos";
  str = rp->find_parameter(seek);
  if (str=="") IC_shocktube::shockpos = 0.0;
  else IC_shocktube::shockpos = atof(str.c_str());
  cout <<"shock pos:"<<shockpos<<endl;

  //
  // angle in XY plane (degrees) (angle normal makes with x-axis, projected onto xy plane)
  // Read in as an integer angle in degrees and converted to radians.
  // A negative value is interpreted as a request for arctan(0.5)
  //
  seek = "STangleXY";
  str = rp->find_parameter(seek);
  if (str=="") IC_shocktube::angleXY = 0.0;
  else IC_shocktube::angleXY = static_cast<double>(atoi(str.c_str()));
  angleXY *= M_PI/180.0;
  if (angleXY<0.0) angleXY = atan(0.5);

  // angle in XZ plane (degrees) (angle normal makes with x-axis, projected onto xz plane)
  seek = "STangleXZ";
  str = rp->find_parameter(seek);
  if (str=="") IC_shocktube::angleXZ = 0.0;
  else IC_shocktube::angleXZ = static_cast<double>(atoi(str.c_str()));
  angleXZ *= M_PI/180.0;
  ...
  if (number<=0) {
    ...
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::preshock[RO] = atof(str.c_str());
    else         IC_shocktube::preshock[RO] = -1.0e99;
    temp.str("");
    temp << "STpostvec" << v;
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::postshock[RO] = atof(str.c_str());
    else         IC_shocktube::postshock[RO] = -1.0e99;
    ...
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::preshock[PG] = atof(str.c_str());
    else         IC_shocktube::preshock[PG] = -1.0e99;
    temp.str("");
    temp << "STpostvec" << v;
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::postshock[PG] = atof(str.c_str());
    else         IC_shocktube::postshock[PG] = -1.0e99;
    ...
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::preshock[VX] = atof(str.c_str());
    else         IC_shocktube::preshock[VX] = -1.0e99;
    temp.str("");
    temp << "STpostvec" << v;
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::postshock[VX] = atof(str.c_str());
    else         IC_shocktube::postshock[VX] = -1.0e99;
    ...
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::preshock[VY] = atof(str.c_str());
    else         IC_shocktube::preshock[VY] = -1.0e99;
    temp.str("");
    temp << "STpostvec" << v;
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::postshock[VY] = atof(str.c_str());
    else         IC_shocktube::postshock[VY] = -1.0e99;
    ...
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::preshock[VZ] = atof(str.c_str());
    else         IC_shocktube::preshock[VZ] = -1.0e99;
    temp.str("");
    temp << "STpostvec" << v;
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str!="") IC_shocktube::postshock[VZ] = atof(str.c_str());
    else         IC_shocktube::postshock[VZ] = -1.0e99;
    ...
    if (eqns ==2) { // mhd sim
      ...
      seek = temp.str();
      str = rp->find_parameter(seek);
      if (str!="") IC_shocktube::preshock[BX] = atof(str.c_str());
      else         IC_shocktube::preshock[BX] = -1.0e99;
      temp.str("");
      temp << "STpostvec" << v;
      seek = temp.str();
      str = rp->find_parameter(seek);
      if (str!="") IC_shocktube::postshock[BX] = atof(str.c_str());
      else         IC_shocktube::postshock[BX] = -1.0e99;
      ...
      seek = temp.str();
      str = rp->find_parameter(seek);
      if (str!="") IC_shocktube::preshock[BY] = atof(str.c_str());
      else         IC_shocktube::preshock[BY] = -1.0e99;
      temp.str("");
      temp << "STpostvec" << v;
      seek = temp.str();
      str = rp->find_parameter(seek);
      if (str!="") IC_shocktube::postshock[BY] = atof(str.c_str());
      else         IC_shocktube::postshock[BY] = -1.0e99;
      ...
      seek = temp.str();
      str = rp->find_parameter(seek);
      if (str!="") IC_shocktube::preshock[BZ] = atof(str.c_str());
      else         IC_shocktube::preshock[BZ] = -1.0e99;
      temp.str("");
      temp << "STpostvec" << v;
      seek = temp.str();
      str = rp->find_parameter(seek);
...
int IC_shocktube::assign_data(double *left, double *right, double interface)
{
  ...
  else if (eqns==2) eqn = new class eqns_mhd_ideal (nvar);
  else rep.error("Bad equations in assign_data()",eqns);
  if (!eqn) rep.error("Bad equations in assign_data()",eqns);

  cout <<"Setting up a "<<ndim<<"-D simulation with a ShockTube problem.\n";
  cout <<"Initial shock position is at x="<<interface<<" and angle is t="<<angleXY*180.0/M_PI<<" degrees.\n";
  ...
  if (ndim==1){  
	 ...
	 for (int v=0;v<nvar;v++) cpt->P[v] = left[v];
       else if (CI.get_dpos(cpt,XX) > (interface+dx))
	 for (int v=0;v<nvar;v++) cpt->P[v] = right[v];
       else {
  ...
  if (IC_shocktube::number==8) {
	...
	if (dpos[XX]>=interface && dpos[XX]<(interface+len)) {
	  ...
	  cpt->P[BY] = cpt->Ph[BY] = 
	    amp*cos(2.0*M_PI*(dpos[XX]-interface)/len);
	  cpt->P[VZ] = cpt->Ph[VZ] = 
	    amp*(1.0+sin(2.0*M_PI*(dpos[XX]-interface)/len));
	  cpt->P[BZ] = cpt->Ph[BZ] = 
	...
	cpt->P[PG] = 0.1;
	cpt->P[VX] = 0.0;
	cpt->P[VY] = amp*sin(2.0*M_PI*(dpos[XX]*cos(theta)+dpos[YY]*sin(theta)));
	cpt->P[VZ] = amp*cos(2.0*M_PI*(dpos[XX]*cos(theta)+dpos[YY]*sin(theta)));
	cpt->P[BX] = 1.0;
	cpt->P[BY] = cpt->P[VY];
	cpt->P[BZ] = cpt->P[VZ];
...
int IC_shocktube::get_riemann_ics(int sw, double *l, double *r, double *xm)
{
  ...
  case 9:
    ...
    if(eqns!=2)
      {cerr<<"(IC_shocktube::get_riemann_ics) Not using MHD but asking for MHD test problem. Exiting.\n"; return(1);}
    //l[RO]=3.; l[PG]=49./3.; l[VX]=1.0-sqrt(3.0); l[VY]=-4./3.; l[VZ]=0.;
    //l[BX]=3.; l[BY]=sqrt(16.0/3.0); l[BZ]=0.0; //1.;
    //r[RO]=1.; r[PG]=1.; r[VX]=-sqrt(12.0)-sqrt(3)+1.0; r[VY]=0.; r[VZ]=0.;
    //r[BX]=3.; r[BY]=0.; r[BZ]=0.;
    l[RO]=3.; l[PG]=16.33; l[VX]=-0.732; l[VY]=-1.333; l[VZ]=0.;
    l[BX]=3.; l[BY]=2.309; l[BZ]=0.0; //1.;
