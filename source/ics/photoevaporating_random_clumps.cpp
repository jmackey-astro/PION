...
)
{
    ...
    str  = rp->find_parameter(seek);
    if (str != "")
        IC_photevap_random_clumps::ambient[RO] = atof(str.c_str());
    else
        IC_photevap_random_clumps::ambient[RO] = -1.0e99;

    // set this for now to the ambient density, and will take mass out of it
    // later if needed.
    ...
    str  = rp->find_parameter(seek);
    if (str != "")
        IC_photevap_random_clumps::ambient[PG] = atof(str.c_str());
    else
        IC_photevap_random_clumps::ambient[PG] = -1.0e99;
    ...
    str  = rp->find_parameter(seek);
    if (str != "")
        IC_photevap_random_clumps::ambient[VX] = atof(str.c_str());
    else
        IC_photevap_random_clumps::ambient[VX] = -1.0e99;
    ...
    str  = rp->find_parameter(seek);
    if (str != "")
        IC_photevap_random_clumps::ambient[VY] = atof(str.c_str());
    else
        IC_photevap_random_clumps::ambient[VY] = -1.0e99;
    ...
    str  = rp->find_parameter(seek);
    if (str != "")
        IC_photevap_random_clumps::ambient[VZ] = atof(str.c_str());
    else
        IC_photevap_random_clumps::ambient[VZ] = -1.0e99;
    ...
    if (eqns == 2) {  // mhd sim
        ...
        str  = rp->find_parameter(seek);
        if (str != "")
            IC_photevap_random_clumps::ambient[BX] = atof(str.c_str());
        else
            IC_photevap_random_clumps::ambient[BX] = -1.0e99;
        ...
        str  = rp->find_parameter(seek);
        if (str != "")
            IC_photevap_random_clumps::ambient[BY] = atof(str.c_str());
        else
            IC_photevap_random_clumps::ambient[BY] = -1.0e99;
        ...
        str  = rp->find_parameter(seek);
        if (str != "")
            IC_photevap_random_clumps::ambient[BZ] = atof(str.c_str());
        else
            IC_photevap_random_clumps::ambient[BZ] = -1.0e99;
    ...
    for (int t = 0; t < SimPM->ntracer; t++) {
        ...
        str  = rp->find_parameter(seek);
        if (str != "")
            IC_photevap_random_clumps::ambient[t + SimPM->ftr] =
                atof(str.c_str());
        else
            IC_photevap_random_clumps::ambient[t + SimPM->ftr] = -1.0e99;
    }
    ...
    if (ics == "")
        ...
        ics = "PERC2";
        cout
            << "\t\tSetting up PhotoEvaporation problem with Random Clumps (FIXED "
               "TOTAL MASS!).\n";
    }
    else
...
int IC_photevap_random_clumps::clumps_random_setup_fixedmass()
{
    ...
    if (str == "")
        ...
        empty_xn = atof(str.c_str()) * xmax;
    seek = "PERCempty_xp";
    str  = rp->find_parameter(seek);
    if (str == "")
        empty_xp = x3;
    else
        empty_xp = atof(str.c_str()) * xmax;
    seek = "PERCempty_yn";
    str  = rp->find_parameter(seek);
    if (str == "")
        empty_yn = x3;
    else
        empty_yn = atof(str.c_str()) * ymax;
    seek = "PERCempty_yp";
    str  = rp->find_parameter(seek);
    if (str == "")
        empty_yp = x3;
    else
        empty_yp = atof(str.c_str()) * ymax;
    seek = "PERCempty_zn";
    str  = rp->find_parameter(seek);
    if (str == "")
        empty_zn = x3;
    else
        empty_zn = atof(str.c_str()) * zmax;
    seek = "PERCempty_zp";
    str  = rp->find_parameter(seek);
    if (str == "")
        empty_zp = x3;
    else
        empty_zp = atof(str.c_str()) * zmax;
...
#define FIXED_MASS_RANGE_CLUMPS
    ...
    if (str == "")
        ...
        min_mass = atof(str.c_str()) * clump_mass;
    seek = "PERCmax_mass";
    str  = rp->find_parameter(seek);
    if (str == "")
        max_mass = 0.0;
    ...
    for (int j = 0; j < Nclumps; j++) {
        ...
        if (SimPM->BC_YN == "periodic") {
            ...
            // have to modify it for x, to get no clumps within a physical
            // max_size of the XP boundary (so we don't lose lots of mass of the
            // XP boundary).
            ...
            cl[j].centre[YY] = SimPM->Xmin[YY] + empty_yn
                               + (ymax - empty_yp - empty_yn) * random_frac();
            cl[j].centre[ZZ] = SimPM->Xmin[ZZ] + empty_zn
                               + (zmax - empty_zp - empty_zn) * random_frac();
        }
        // radius of clumps
        cl[j].size[XX] =
            min_size * ymax + (max_size - min_size) * ymax * random_frac();
        cl[j].size[YY] =
            min_size * ymax + (max_size - min_size) * ymax * random_frac();
        cl[j].size[ZZ] =
            min_size * ymax + (max_size - min_size) * ymax * random_frac();
        ...
        if (SimPM->ndim == 2) {
            cl[j].ang[ZZ] = cl[j].ang[YY] = cl[j].size[ZZ] = cl[j].centre[ZZ] =
                0.0;
        }
        ...
        for (int v = 0; v < SimPM->ndim; v++)
            cl[j].overdensity /= sqrt(M_PI) * cl[j].size[v];
        ...
        cl[j].rm[XX][XX] =
            cos(cl[j].ang[XX]) * cos(cl[j].ang[YY]) * cos(cl[j].ang[ZZ])
            - sin(cl[j].ang[XX]) * sin(cl[j].ang[ZZ]);
        cl[j].rm[XX][YY] =
            sin(cl[j].ang[XX]) * cos(cl[j].ang[YY]) * cos(cl[j].ang[ZZ])
            + cos(cl[j].ang[XX]) * sin(cl[j].ang[ZZ]);
        cl[j].rm[XX][ZZ] = -sin(cl[j].ang[YY]) * cos(cl[j].ang[ZZ]);
        cl[j].rm[YY][XX] =
            -cos(cl[j].ang[XX]) * cos(cl[j].ang[YY]) * sin(cl[j].ang[ZZ])
            - sin(cl[j].ang[XX]) * cos(cl[j].ang[ZZ]);
        cl[j].rm[YY][YY] =
            -sin(cl[j].ang[XX]) * cos(cl[j].ang[YY]) * sin(cl[j].ang[ZZ])
            + cos(cl[j].ang[XX]) * cos(cl[j].ang[ZZ]);
...
int IC_photevap_random_clumps::clumps_random_setup()
{
    ...
    for (int j = 0; j < Nclumps; j++) {
        cl[j].overdensity =
            min_overdensity
            + random_frac() * (max_overdensity - min_overdensity);
        //
        // This restricts clump centres to be at least 0.1*Range from the edge.
        //
        // if (SimPM->typeofbc.find("YNper") != std::string::npos) {
        if (SimPM->BC_YN == "periodic") {
            ...
            cl[j].centre[XX] =
                0.2 * xmax
                + (0.8 - (2.0 * max_size * xmax / ymax)) * xmax * random_frac();
            ...
            cl[j].centre[ZZ] =
                max_size * zmax + (1.0 - 2.0 * max_size) * zmax * random_frac();
        }
        // radius of clumps
        ...
        cl[j].rm[XX][XX] =
        cos(cl[j].ang[XX])*cos(cl[j].ang[YY])*cos(cl[j].ang[ZZ])
        -sin(cl[j].ang[XX])*sin(cl[j].ang[ZZ]); cl[j].rm[XX][YY] =
        cos(cl[j].ang[XX])*cos(cl[j].ang[YY])*sin(cl[j].ang[ZZ])
        -sin(cl[j].ang[XX])*cos(cl[j].ang[ZZ]); cl[j].rm[XX][ZZ] =
        cos(cl[j].ang[XX])*sin(cl[j].ang[YY]); cl[j].rm[YY][XX] =
        sin(cl[j].ang[XX])*cos(cl[j].ang[YY])*cos(cl[j].ang[ZZ])
        +cos(cl[j].ang[XX])*sin(cl[j].ang[ZZ]); cl[j].rm[YY][YY] =
        -sin(cl[j].ang[XX])*cos(cl[j].ang[YY])*sin(cl[j].ang[ZZ])
        +cos(cl[j].ang[XX])*cos(cl[j].ang[ZZ]); cl[j].rm[YY][ZZ] =
        sin(cl[j].ang[XX])*sin(cl[j].ang[YY]); cl[j].rm[ZZ][XX] =
        -sin(cl[j].ang[YY])*cos(cl[j].ang[ZZ]); cl[j].rm[ZZ][YY] =
        sin(cl[j].ang[YY])*sin(cl[j].ang[ZZ]); cl[j].rm[ZZ][ZZ] =
        cos(cl[j].ang[YY]);
        */
        // all done, move on to next clump.
    }
...
int IC_photevap_random_clumps::clumps_random_set_dens(class cell* c)
{
    ...
    for (int j = 0; j < Nclumps; j++) {
        ...
        if (profile == 0) {
            // top-hat profile
            string e =
                "Need to make this trace ellipses, not postage stamps!!!";
            ...
            // centre we are. Then set density based on that. Set tracers
            // according to fraction of mass that is ambient and clump:
            //
            double ef = 0.0;
            for (int v = 0; v < ndim; v++)
