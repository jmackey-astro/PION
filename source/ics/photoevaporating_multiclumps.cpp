...
)
{
    ...
    if (coords != COORD_CRT) {
        cout
            << "WARNING! using cylindrical coords, so MAKE SURE SOURCE AND ALL "
               "CLUMPS ARE ON-AXIS!!\n";
    ...
    do {
        ...
        if (amb->radial_profile != 0) {
            ...
            if (dist > amb->cloudradius) {
                c->P[RO] *=
                    exp(amb->radial_profile * log(amb->cloudradius / dist));
                c->P[PG] *=
                    exp(amb->radial_profile * log(amb->cloudradius / dist));
            }
        }
        ...
        if (amb->xscale == true) {
            ...
            // scale factor:
            dist = exp(-pow(
                fabs(dpos[XX] - amb->xscale_x0) / amb->xscale_l0,
                amb->xscale_alpha));
    ...
    class ReadParams* rparams, struct random_clump_data* rcd)
{
    ...
    // cout <<"\tvolume="<<volume;
    if (ndim > 1)
        volume *= SimPM->Range[YY];  //-rcd->border[YN]-rcd->border[YP];
    // cout <<"\tvolume="<<volume;
    if (ndim > 2)
        volume *= SimPM->Range[ZZ];  //-rcd->border[ZN]-rcd->border[ZP];
    // cout <<"\tvolume="<<volume;
    ...
    if (str == "") {
        cout
            << "WARNING! Parameter PERC_selection_criterion not found, assuming "
    ...
    class ReadParams* rparams, struct random_clump_data* rcd)
{
    ...
    do {
        this_mass =
            rcd->min_mass + random_frac() * (rcd->max_mass - rcd->min_mass);
    ...
    struct random_clump_data* rcd)
{
    ...
    for (int j = 0; j < rcd->Nclumps; j++) {
        ...
        rcd->cl[j].rm[XX][XX] =
            cos(rcd->cl[j].ang[XX]) * cos(rcd->cl[j].ang[YY])
                * cos(rcd->cl[j].ang[ZZ])
            - sin(rcd->cl[j].ang[XX]) * sin(rcd->cl[j].ang[ZZ]);
        rcd->cl[j].rm[XX][YY] =
            sin(rcd->cl[j].ang[XX]) * cos(rcd->cl[j].ang[YY])
                * cos(rcd->cl[j].ang[ZZ])
            + cos(rcd->cl[j].ang[XX]) * sin(rcd->cl[j].ang[ZZ]);
        rcd->cl[j].rm[XX][ZZ] =
            -sin(rcd->cl[j].ang[YY]) * cos(rcd->cl[j].ang[ZZ]);
        rcd->cl[j].rm[YY][XX] =
            -cos(rcd->cl[j].ang[XX]) * cos(rcd->cl[j].ang[YY])
                * sin(rcd->cl[j].ang[ZZ])
        ...
        rcd->cl[j].rm[YY][ZZ] =
            sin(rcd->cl[j].ang[YY]) * sin(rcd->cl[j].ang[ZZ]);
        rcd->cl[j].rm[ZZ][XX] =
            cos(rcd->cl[j].ang[XX]) * sin(rcd->cl[j].ang[YY]);
        rcd->cl[j].rm[ZZ][YY] =
            sin(rcd->cl[j].ang[XX]) * sin(rcd->cl[j].ang[YY]);
    ...
    class GridBaseClass* ggg, struct random_clump_data* rcd)
{
    ...
    do {
        for (int v = SimPM->ftr; v < SimPM->nvar; v++) {
            ...
            c->P[v] = (ambdens / c->P[RO]) * amb_data.ambient[v]
                      + (1.0 - ambdens / c->P[RO])
                            * rcd->cl[0].tracer_vals[v - SimPM->ftr];
    ...
    class ReadParams* rparams, struct strategic_clump_data* scd)
{
    ...
    if (str == "" || str == "NO" || str == "no" || str == "No") {
        cout << seek << "=" << str
             << ", so not doing any strategic clumps...\n";
        return 0;
    }
    else
        cout << seek << "=" << str
             << ", so continuing to add strategic clumps.\n";
    scd->used = true;
    ...
    if (scd->Nclumps < 1) {
        ...
        rep.error(
            "Need at least one clump for strategic clumps.", scd->Nclumps);
    }
    else {
    ...
    for (int c = 0; c < scd->Nclumps; c++) {
        ...
        if (scd->profile == 0) {
            ...
            scd->cl[c].mass *= 4.0 * M_PI * scd->cl[c].size[XX]
                               * scd->cl[c].size[XX] * scd->cl[c].size[XX]
                               / 3.0;
        }
            ...
            rep.error(
                "Bad profile in get_strategic_clump_params()", scd->profile);
        ...
        //
        scd->cl[c].rm[XX][XX] =
            cos(scd->cl[c].ang[XX]) * cos(scd->cl[c].ang[YY])
                * cos(scd->cl[c].ang[ZZ])
            - sin(scd->cl[c].ang[XX]) * sin(scd->cl[c].ang[ZZ]);
        scd->cl[c].rm[XX][YY] =
            sin(scd->cl[c].ang[XX]) * cos(scd->cl[c].ang[YY])
                * cos(scd->cl[c].ang[ZZ])
            + cos(scd->cl[c].ang[XX]) * sin(scd->cl[c].ang[ZZ]);
        scd->cl[c].rm[XX][ZZ] =
            -sin(scd->cl[c].ang[YY]) * cos(scd->cl[c].ang[ZZ]);
        scd->cl[c].rm[YY][XX] =
            -cos(scd->cl[c].ang[XX]) * cos(scd->cl[c].ang[YY])
                * sin(scd->cl[c].ang[ZZ])
        ...
        scd->cl[c].rm[YY][ZZ] =
            sin(scd->cl[c].ang[YY]) * sin(scd->cl[c].ang[ZZ]);
        scd->cl[c].rm[ZZ][XX] =
            cos(scd->cl[c].ang[XX]) * sin(scd->cl[c].ang[YY]);
        scd->cl[c].rm[ZZ][YY] =
            sin(scd->cl[c].ang[XX]) * sin(scd->cl[c].ang[YY]);
