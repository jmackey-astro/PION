      ...
      )
{
  ...
  if (str!="") amb->ambient[RO] = atof(str.c_str());
  else         amb->ambient[RO] = -1.0e99;
  ...
  seek = temp.str();
  str = rparams->find_parameter(seek);
  if (str!="") amb->ambient[PG] = atof(str.c_str());
  else         amb->ambient[PG] = -1.0e99;
  ...
  seek = temp.str();
  str = rparams->find_parameter(seek);
  if (str!="") amb->ambient[VX] = atof(str.c_str());
  else         amb->ambient[VX] = -1.0e99;
  ...
  seek = temp.str();
  str = rparams->find_parameter(seek);
  if (str!="") amb->ambient[VY] = atof(str.c_str());
  else         amb->ambient[VY] = -1.0e99;
  ...
  seek = temp.str();
  str = rparams->find_parameter(seek);
  if (str!="") amb->ambient[VZ] = atof(str.c_str());
  else         amb->ambient[VZ] = -1.0e99;
  ...
  if (eqns ==2) { // mhd sim
    ...
    seek = temp.str();
    str = rparams->find_parameter(seek);
    if (str!="") amb->ambient[BX] = atof(str.c_str());
    else         amb->ambient[BX] = -1.0e99;
    ...
    seek = temp.str();
    str = rparams->find_parameter(seek);
    if (str!="") amb->ambient[BY] = atof(str.c_str());
    else         amb->ambient[BY] = -1.0e99;
  ...
  seek = temp.str();
  str = rparams->find_parameter(seek);
  if (str!="") amb->radial_profile = atoi(str.c_str());
  else         amb->radial_profile = 0;
      ...
      )
{
  ...
  else cout <<seek<<"="<<str<<", so continuing to add random clumps.\n";
  rcd->used = true;
  ...
  str = rparams->find_parameter(seek);
  if (str=="") rep.error("Failed to get mass to put into clumps", seek);
  else         rcd->density = atof(str.c_str());
  ...
  else         rcd->random_seed = atoi(str.c_str());
  ...
  str = rparams->find_parameter(seek);
  if (str=="") rep.error("didn't find parameter",seek);
  else         rcd->profile = atoi(str.c_str());
  ...
  // different functions.
  //
  // N.B. These functions will bug out if less than one random clump is generated,
  // so safe for following code to assume at least one random clump exists.
  //
  seek = "PERC_selection_criterion";
  str = rparams->find_parameter(seek);
  if      (str=="") {
    cout <<"WARNING! Parameter PERC_selection_criterion not found, assuming fixed mass range clumps.\n";
  ...
  else rep.error("Bad parameter for PERC_selection_criterion", str);
  ...
  for (int v=0;v<SimPM.ntracer;v++) {
    ...
    for (int c=0; c<rcd->Nclumps; c++) {
      if (str=="") rcd->cl[c].tracer_vals[v] = 0.0;
      else         rcd->cl[c].tracer_vals[v] = atof(str.c_str());
    }
    cout <<"***********tracer["<<v<<"] = "<<rcd->cl[0].tracer_vals[v]<<endl;
  }


  //
  // Now we have Nclumps, and the clump properties, so select their properties:
  //
      ...
      )
{
  ...
  if (rcd->Nclumps<1) {
    cout <<"\tFound "<<rcd->Nclumps<<" Random Clumps, so not adding any clumps.\n";
    rep.error("Need at least one clump for Fixed number of clumps.",rcd->Nclumps);
  }
  else {
  ...
  //
  // choose N+1 random values on the interval [0,M_cl], and use them to get clump masses.
  // Note for large N this will give roughly equal mass clumps because random values have
  // uniform probability (this is why I moved away from this method...).
  //
  ...
  for (int j=0;j<rcd->Nclumps+1;j++) cout <<"masses: "<<masses[j]<<endl;
      ...
      )
{
  ...
  seek = "PERCmin_mass";
  str = rparams->find_parameter(seek);
  if (str=="") rep.error("Failed to get param",seek);
  else         rcd->min_mass = atof(str.c_str())*rcd->total_mass;
  seek = "PERCmax_mass";
  str = rparams->find_parameter(seek);
  if (str=="") rep.error("Failed to get param",seek);
  else         rcd->max_mass = atof(str.c_str())*rcd->total_mass;
  cout <<"min/max masses: "<<rcd->min_mass<<", "<<rcd->max_mass<<",  and total mass: "<<rcd->total_mass<<endl;


  //
  // Take a random chunk (between min and max) out of total mass, as store as a clump mass
  //
  std::vector<double> masses;
  ...
  cout <<"Used up all the mass in "<<nnn<<" clumps (Nclumps was "<<rcd->Nclumps<<")  Mass left over="<<mass_remaining<<"\n";
  rcd->Nclumps = nnn;

  if (rcd->Nclumps<1)
    rep.error("Got less than one clump in fixed total mass routine!",rcd->Nclumps);
      ...
      )
{
  ...
  for (int j=0;j<rcd->Nclumps;j++) {
    ...
    // radius of clumps
    //
    rcd->cl[j].size[XX] = rcd->min_size*ymax +(rcd->max_size-rcd->min_size)*ymax*random_frac();
    rcd->cl[j].size[YY] = rcd->min_size*ymax +(rcd->max_size-rcd->min_size)*ymax*random_frac();
    rcd->cl[j].size[ZZ] = rcd->min_size*ymax +(rcd->max_size-rcd->min_size)*ymax*random_frac();
    ...
    rcd->cl[j].rm[XX][XX] =  cos(rcd->cl[j].ang[XX])*cos(rcd->cl[j].ang[YY])*cos(rcd->cl[j].ang[ZZ]) -sin(rcd->cl[j].ang[XX])*sin(rcd->cl[j].ang[ZZ]);
    rcd->cl[j].rm[XX][YY] =  sin(rcd->cl[j].ang[XX])*cos(rcd->cl[j].ang[YY])*cos(rcd->cl[j].ang[ZZ]) +cos(rcd->cl[j].ang[XX])*sin(rcd->cl[j].ang[ZZ]);
    rcd->cl[j].rm[XX][ZZ] = -sin(rcd->cl[j].ang[YY])*cos(rcd->cl[j].ang[ZZ]);
    rcd->cl[j].rm[YY][XX] = -cos(rcd->cl[j].ang[XX])*cos(rcd->cl[j].ang[YY])*sin(rcd->cl[j].ang[ZZ]) -sin(rcd->cl[j].ang[XX])*cos(rcd->cl[j].ang[ZZ]);
    rcd->cl[j].rm[YY][YY] = -sin(rcd->cl[j].ang[XX])*cos(rcd->cl[j].ang[YY])*sin(rcd->cl[j].ang[ZZ]) +cos(rcd->cl[j].ang[XX])*cos(rcd->cl[j].ang[ZZ]);
      ...
      )
{
  ...
  //
  // Set tracer values, assuming all random clumps have the same tracer value, and that it
      ...
      )
{
  ...
  else cout <<seek<<"="<<str<<", so continuing to add strategic clumps.\n";
  scd->used = true;
  ...
  if (scd->Nclumps<1) {
    cout <<"\tFound "<<scd->Nclumps<<" Strategic Clumps, so not adding any clumps.\n";
  ...
  str = rparams->find_parameter(seek);
  if (str=="") rep.error("didn't find parameter",seek);
  else         scd->profile = atoi(str.c_str());
  ...
  for (int c=0; c<scd->Nclumps; c++) {
    ...
    // Clump Overdensity.
    //
    temp.str(""); temp<<"PE_SC"<<c<<"_overdensity";
    seek = temp.str();
    str = rp->find_parameter(seek);
    if (str=="") rep.error("no parameter found for clump",seek);
    else         scd->cl[c].overdensity = atof(str.c_str());
    ...
    //
    // Clump size: (note this is hard-coded to be circular, this param is as fraction
    // of the Y-Range).
    //
    temp.str(""); temp<<"PE_SC"<<c<<"_radius"; seek=temp.str();
    str = rp->find_parameter(seek);
    if (str=="") rep.error("didn't find parameter",seek);
    ...
    // Clump Velocity (cm/s).
    //
    temp.str(""); temp<<"PE_SC"<<c<<"_vx"; seek=temp.str();
    str = rp->find_parameter(seek);
    if (str=="") scd->cl[c].Vel[0] = 0.0;
    else         {scd->cl[c].Vel[0] = atof(str.c_str()); cout <<"VX="<<scd->cl[c].Vel[0]<<"\n";}

    temp.str(""); temp<<"PE_SC"<<c<<"_vy"; seek=temp.str();
    str = rp->find_parameter(seek);
    if (str=="") scd->cl[c].Vel[1] = 0.0;
    else         scd->cl[c].Vel[1] = atof(str.c_str());
    
    temp.str(""); temp<<"PE_SC"<<c<<"_vz"; seek=temp.str();
    str = rp->find_parameter(seek);
    if (str=="") scd->cl[c].Vel[2] = 0.0;
    else         scd->cl[c].Vel[2] = atof(str.c_str());
    ...
    for (int v=0;v<SimPM.ntracer;v++) {
      ...
      seek = temp.str();
      str = rp->find_parameter(seek);
      if (str=="") scd->cl[c].tracer_vals[v] = 0.0;
      else         scd->cl[c].tracer_vals[v] = atof(str.c_str());
    }
    ...
    //
    // Set Mass, based on profile.  Note this is total mass, including ambient gas.
    //
    ...
    // now set up the rotation matrix.
    //
    scd->cl[c].rm[XX][XX] =  cos(scd->cl[c].ang[XX])*cos(scd->cl[c].ang[YY])*cos(scd->cl[c].ang[ZZ]) -sin(scd->cl[c].ang[XX])*sin(scd->cl[c].ang[ZZ]);
    scd->cl[c].rm[XX][YY] =  sin(scd->cl[c].ang[XX])*cos(scd->cl[c].ang[YY])*cos(scd->cl[c].ang[ZZ]) +cos(scd->cl[c].ang[XX])*sin(scd->cl[c].ang[ZZ]);
    scd->cl[c].rm[XX][ZZ] = -sin(scd->cl[c].ang[YY])*cos(scd->cl[c].ang[ZZ]);
    scd->cl[c].rm[YY][XX] = -cos(scd->cl[c].ang[XX])*cos(scd->cl[c].ang[YY])*sin(scd->cl[c].ang[ZZ]) -sin(scd->cl[c].ang[XX])*cos(scd->cl[c].ang[ZZ]);
    scd->cl[c].rm[YY][YY] = -sin(scd->cl[c].ang[XX])*cos(scd->cl[c].ang[YY])*sin(scd->cl[c].ang[ZZ]) +cos(scd->cl[c].ang[XX])*cos(scd->cl[c].ang[ZZ]);
...
int IC_photevap_multi_clumps::clumps_set_dens(class cell *c,
					      const int Nclumps,
					      struct clump *cl,
					      const int profile
					      )
{
  ...
  for (int j=0;j<Nclumps;j++) {
    ...
    //
    // rotate clump according to rotation matrix generated from random angles.
    //
    ...
    if (profile==0) {
      ...
      bool inside=true;
      double dist=0.0;
      for (int k=0;k<ndim;k++) dist += x1[k]*x1[k]/cl[j].size[k]/cl[j].size[k];
	...
	c->P[RO] += add_rho;
      ...
      // Note profile has factor of 2, so that size is easily related to normal distribution.
      // Set tracers according to fraction of mass that is ambient and clump:
      //
      double ef=0.0;
      for (int v=0;v<ndim;v++) ef += x1[v]*x1[v]/cl[j].size[v]/cl[j].size[v]/2.0;
    ...
    //
    // Set tracer values: 
    // (random clumps over-writes this later, because it is sketchy if clumps overlap)
    ...
    if (add_rho/c->P[RO] > 0.001 ) {
      ...
      // from the max value at the centre to the ambient value near the edge.
      //
      //cout <<"V=["<<cl[j].Vel[0]<<","<<cl[j].Vel[1]<<","<<cl[j].Vel[2]<<"\n";
      c->P[VX] = cl[j].Vel[0]+ambdens/c->P[RO]*(amb_data.ambient[VX]-cl[j].Vel[0]);
      c->P[VY] = cl[j].Vel[1]+ambdens/c->P[RO]*(amb_data.ambient[VY]-cl[j].Vel[1]);
      c->P[VZ] = cl[j].Vel[2]+ambdens/c->P[RO]*(amb_data.ambient[VZ]-cl[j].Vel[2]);
    }
