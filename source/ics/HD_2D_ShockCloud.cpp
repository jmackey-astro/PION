          ...
          )
{
  ...
  for (int v=0;v<nvar;v++) {
    out[v] = data[v][imin] + (data[v][imax]-data[v][imin])*(seek-radius[imin])/(radius[imax]-radius[imin]);
  }

  if (dpos[Zcyl]>Rshock) {
    // rotate velocities of clump to point in radial direction.
    out[VY] = -out[VX]*dpos[Rcyl]/sqrt(dpos[Rcyl]*dpos[Rcyl]+dpos[Zcyl]*dpos[Zcyl]);
    out[VX] = -out[VX]*dpos[Zcyl]/sqrt(dpos[Rcyl]*dpos[Rcyl]+dpos[Zcyl]*dpos[Zcyl]);
  }
