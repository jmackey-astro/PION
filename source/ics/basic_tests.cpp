...
)
{
    ...
    do {
        ...
        if (use_core) {
            cpt->P[RO] /=
                (1.0 + exp(radial_slope * log(core_radius / distance)));
            cpt->P[PG] /=
                (1.0 + exp(radial_slope * log(core_radius / distance)));
...
int IC_basic_tests::setup_advection()
{
    ...
    int ndim = gg->Ndim();  // int nvar=gg->Nvar();
    if (ndim != 2 && ndim != 3)
        rep.error("Bad ndim in setupNDadvectionHD", ndim);

    if (ndim == 2 && !pconst.equalD(thetaXZ, M_PI / 2.)) {
        rep.warning(
            "Given 3d angle, but 2d sim.  setting 3d angle to zero.", 0,
            thetaXZ);
        thetaXZ = M_PI / 2.;
    }
...
int IC_basic_tests::setup_divBpeak()
{
    ...
    if (fabs(SimPM->Xmin[XX] + 0.5) > 1.e-6
        ...
        cout << fabs(SimPM->Xmin[XX] + 0.5) << "\t"
             << fabs(SimPM->Xmax[XX] - 1.5) << endl;
...
int IC_basic_tests::setup_KelvinHelmholz()
{
    ...
    do {
        ...
        c->P[VX] =
            -0.5 * Ux * tanh((dpos[YY]) / a);  //-0.5*SimPM->Range[YY])/a);
