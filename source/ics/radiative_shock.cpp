...
int IC_radiative_shock::setup_OutflowRadiativeShock()
{
  ...
  // This divisor factor is basically arbitrary -- I have used it at 3.0, 10.0, 30.0
  // The idea is to get it so that it produces a reasonable shock i.e. that the 
  // shock doesn't move off the grid one way or the other too quickly, and that the
  // waves advected downstream aren't too seriously messing the solution.  3.0 seems
  // to be a good value for Raymond's 1979 model E shock (100km/s).
  string seek, str;
  seek = "RADSH_divisor";
  str = rp->find_parameter(seek);
  double divisor=0.0;
  ...
  do {
	...
	c->P[BY] = c->P[BZ] = B0*(rho1/rho0)/sqrt(2.); c->P[BX] = 0.0;
      ...
      // magnetic field???
      if (eqns==2) {
	c->P[BY]=c->P[BZ]= frac*B0/sqrt(2.)+(1.-frac)*B0*(rho1/rho0)/sqrt(2.);
	c->P[BX] = 0.0;
      }
      //
