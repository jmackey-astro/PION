...
)
{
    ...
    str  = rp->find_parameter(seek);
    if (str == "")
        IC_photoevaporatingclump::pratio =
            1.0;  // default to pressure equilibrium
    else
    ...
    for (int t = 0; t < SimPM->ntracer; t++) {
        ...
        str  = rp->find_parameter(seek);
        if (str != "")
            IC_photoevaporatingclump::ambient[t + SimPM->ftr] =
                atof(str.c_str());
        else
    ...
    if (ics == "")
        ...
        ics = "PE_SLOPE";
        cout
            << "\t\tSetting up Photoevaporation into radial density profile.\n";
    }
    else if (ics == "PhotoEvap_powerlaw") {
        ics = "PE_POWERLAW";
        cout
            << "\t\tSetting up Photoevaporation into power law density profile in "
               "Zcyl (XX).\n";
    }
...
int IC_photoevaporatingclump::setup_pec2()
{
    ...
    do {
        ...
        if (d < edge) {
            ...
            cpt->P[RO] =
                rho1 + (ambient[RO] - rho1) * (d - edge) / (clrad - edge);
            cpt->P[PG] =
                pg1 + (ambient[PG] - pg1) * (d - edge) / (clrad - edge);
            for (int v = 0; v < SimPM->ntracer; v++)
                cpt->P[SimPM->ftr + v] = cltr[v]
                                         + (ambient[SimPM->ftr + v] - cltr[v])
                                               * (d - edge) / (clrad - edge);
        }
...
int IC_photoevaporatingclump::setup_pec()
{
    ...
    do {
        ...
        if ((vfrac = stest.volumeFraction(cpt)) > 0) {
            // cout <<"Setting cell "<<cpt->id<<" to internal value.\n";
            cpt->P[RO] =
                vfrac * (dratio * cpt->P[RO]) + (1. - vfrac) * cpt->P[RO];
            cpt->P[PG] =
                vfrac * (pratio * cpt->P[PG]) + (1. - vfrac) * cpt->P[PG];
            if (eqns == 2)
                cpt->P[BX] =
                    vfrac * (Bratio * cpt->P[BX]) + (1. - vfrac) * cpt->P[BX];
            for (int v = 0; v < SimPM->ntracer; v++)
                cpt->P[SimPM->ftr + v] =
