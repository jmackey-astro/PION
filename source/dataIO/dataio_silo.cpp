...
)
{
    ...
    if (silo_dtype == DB_FLOAT) {
...
#ifdef WRITE_GHOST_ZONES
            posx[i] =
                static_cast<float>(grid->Xmin(XX) - SimPM.Nbc * dx + i * dx);
#else
        ...
        if (ndim > 1) {
            for (int i = 0; i < ny; i++) {
#ifdef WRITE_GHOST_ZONES
                posy[i] = static_cast<float>(
                    grid->Xmin(YY) - SimPM.Nbc * dx + i * dx);
#else
        ...
        if (ndim > 2) {
            for (int i = 0; i < nz; i++) {
#ifdef WRITE_GHOST_ZONES
                posz[i] = static_cast<float>(
                    grid->Xmin(ZZ) - SimPM.Nbc * dx + i * dx);
#else
...
#ifdef WRITE_GHOST_ZONES
            posx[i] =
                static_cast<double>(grid->Xmin(XX) - SimPM.Nbc * dx + i * dx);
#else
    ...
    if (v >= 0) {
        ...
        if (MP) {
            if (silo_dtype == DB_FLOAT) {
                do {
                    farr[ct] =
                        static_cast<float>(MP->Temperature(c->P, SimPM.gamma));
                    ct++;
                }
#ifdef WRITE_GHOST_ZONES
                while ((c = gp->NextPt_All(c)) != 0);
                ...
                do {
                    darr[ct] =
                        static_cast<double>(MP->Temperature(c->P, SimPM.gamma));
                    ct++;
                }
#ifdef WRITE_GHOST_ZONES
                while ((c = gp->NextPt_All(c)) != 0);
            ...
            if (silo_dtype == DB_FLOAT) {
                ...
                do {
                    darr[ct] =
                        static_cast<double>(eqn->eint(c->P, SimPM.gamma));
                    ct++;
                }
#ifdef WRITE_GHOST_ZONES
                while ((c = gp->NextPt_All(c)) != 0);
...
)
{
    ...
    if (variable == "NG_Mask") {
        // int variable
        err = DBPutQuadvar1(
            dbfile, variable.c_str(), meshname.c_str(), data, zonedims, ndim, 0,
            0, DB_INT, DB_ZONECENT, 0);
    }
    else {
        // double/float
        err = DBPutQuadvar1(
            dbfile, variable.c_str(), meshname.c_str(), data, zonedims, ndim, 0,
            0, silo_dtype, DB_ZONECENT, 0);
    }
    return err;
}
...
)
{
    ...
    if (variable == "Velocity" || variable == "MagneticField") {
        ...
        if (ct != npt)
            rep.error(
                "wrong number of points read for vector variable", ct - npt);
    }  // vector variable
    ...
    //
        ...
        if (ct != npt)
            rep.error(
                "wrong number of points read for scalar variable", ct - npt);
    }  // scalar variable
