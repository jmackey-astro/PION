      ...
      )
{
  ...
  if (SWP.Nsources > 0) {
    ...
    for (int isw=0; isw<SWP.Nsources; isw++) {
      ...
      nm.str(""); nm << "WIND_"<<isw<<"_ecentricity_fac";
      if ( (*iter)->name.compare( nm.str() ) !=0) {
        rep.error("Stellar wind parameters not ordered as expected!",(*iter)->name);
      }
      ...
      nm.str(""); nm << "WIND_"<<isw<<"_orbital_period";
      if ( (*iter)->name.compare( nm.str() ) !=0) {
        rep.error("Stellar wind parameters not ordered as expected!",(*iter)->name);
      }
      ...
      nm.str(""); nm << "WIND_"<<isw<<"_periastron_vec_x";
      if ( (*iter)->name.compare( nm.str() ) !=0) {
        rep.error("Stellar wind parameters not ordered as expected!",(*iter)->name);
      }
      ...
      nm.str(""); nm << "WIND_"<<isw<<"_periastron_vec_y";
      if ( (*iter)->name.compare( nm.str() ) !=0)
        rep.error("Stellar wind parameters not ordered as expected!",
      (*iter)->name);
      //(*iter)->set_ptr(static_cast<void *>(posn));
...
void DataIOBase::set_windsrc_params()
{
  ...
  for (int n=0; n<SWP.Nsources; n++) {
    ...
    ostringstream temp17; temp17.str(""); temp17<< "WIND_"<<n<<"_ecentricity_fac";
    ostringstream temp18; temp18.str(""); temp18<< "WIND_"<<n<<"_orbital_period";
    ostringstream temp19; temp19.str(""); temp19<< "WIND_"<<n<<"_periastron_vec_x";
    ostringstream temp20; temp20.str(""); temp20<< "WIND_"<<n<<"_periastron_vec_y";    
