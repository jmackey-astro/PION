...
)
{
    ...
    if (!params.empty()) {
        cerr
            << "WARNING! params list not empty. CLEARING IT, BUT THIS SHOULDN'T "
               "HAPPEN!\n";
        params.clear();
    }
    ...
    for (int v = 0; v < MAX_DIM; v++)
        defngcentre[v] = 0.0;
    pm_ddimarr* n003 =
        new pm_ddimarr("NG_centre", SimPM.NG_centre, defngcentre);
    p           = n003;
    p->critical = false;
    params.push_back(p);
    delete[] defngcentre;
    ...
    for (int v = 0; v < MAX_DIM; v++)
        defngrefine[v] = 0;
    pm_idimarr* n004 =
        new pm_idimarr("NG_refine", SimPM.NG_refine, defngrefine);
    p           = n004;
    p->critical = false;
    params.push_back(p);
    delete[] defngrefine;
    ...
    params.push_back(p);
    pm_string* p012a =
        new pm_string("chem_code", &SimPM.chem_code, "CHEM_CODE");
    p           = p012a;
    p->critical = false;
    params.push_back(p);
...
)
{
    if (params.empty())
        rep.error(
            "Parameter list is empty -- make sure it populates itself!!", 0);
    ...
    if (JP.jetic) {
        ...
        if (jet_pm.empty())
            rep.error(
                "Jet parameter list is empty -- make sure it populates itself!!",
                0);
        //
        // now read them:
        //
        for (list<pm_base*>::iterator iter = jet_pm.begin();
             iter != jet_pm.end(); ++iter) {
            p   = (*iter);
            err = read_header_param(p);
            if (err) rep.error("Error reading parameter", p->name);
        }
    ...
    if (SimPM.RS.Nsources > 0) {
        ...
        //
        for (list<pm_base*>::iterator iter = rt_src.begin();
             iter != rt_src.end(); ++iter) {
            p   = (*iter);
            err = read_header_param(p);
            if (err && p->critical) {
                rep.error("Error reading parameter", p->name);
        ...
        for (int i = 0; i < SimPM.RS.Nsources; i++) {
            ...
            if (SimPM.RS.sources.at(i).opacity_var + SimPM.ftr >= SimPM.nvar) {
                rep.error(
                    "Opacity var for source is off end of array (ftr offset!)",
                    i);
            }
    ...
    if (SWP.Nsources > 0) {
        ...
        for (int isw = 0; isw < SWP.Nsources; isw++) {
            ...
            // the global list.
            //
            // NOTE THIS WIND STUCT DOESN'T GET FREED AT THE END OF THE
            // SIMULATION, SO IT IS TECHNICALLY A MEMORY LEAK.  I CAN'T THINK OF
            // A GOOD WAY TO GET RID OF IT THOUGH... IF I DELETE A DATAIO CLASS
            // I DON'T WANT TO DELETE THE SWP DATA B/C SOMETIMES I'M NOT
...
)
{
    if (have_setup_tracers) {
        rep.error(
            "Trying to setup tracer parameters twice!", have_setup_tracers);
    }
    if (SimPM.ntracer == 0) {
...
void DataIOBase::set_windsrc_params()
{
    ...
    for (int n = 0; n < SWP.Nsources; n++) {
        ...
        pm_ddimarr* w001 =
            new pm_ddimarr(temp01.str());  // position of source (cm)
        windsrc.push_back(w001);

        pm_double* w002 =
            new pm_double(temp02.str());  // radius of wind BC (cm)
...
)
{
    ...
    if (SimPM.RS.sources.size()
        != static_cast<unsigned int>(SimPM.RS.Nsources)) {
...
// ADD SOURCE_EFFECT VARIABLE
...
#endif
        pm_ddimarr* rtpos =
            new pm_ddimarr(temp2.str(), (SimPM.RS.sources[n].pos));
        ...
        pm_int* rtinf =
            new pm_int(temp5.str(), &(SimPM.RS.sources[n].at_infinity));
        rt_src.push_back(rtinf);
        pm_int* rtupd = new pm_int(temp6.str(), &(SimPM.RS.sources[n].update));
        rt_src.push_back(rtupd);
        pm_int* rttsc =
            new pm_int(temp7.str(), &(SimPM.RS.sources[n].opacity_src));
        rttsc->critical = false;
        rt_src.push_back(rttsc);
        // -------------------------------------------------------------
        // include this because of typo in old version: Remove later.
        pm_int* rtts2 =
            new pm_int(tmp17.str(), &(SimPM.RS.sources[n].opacity_src));
        rtts2->critical = false;
        rt_src.push_back(rtts2);
        // -------------------------------------------------------------
        pm_int* rttvr =
            new pm_int(temp8.str(), &(SimPM.RS.sources[n].opacity_var));
        ...
        pm_string* rtEvo = new pm_string(
            tmp12.str(), &(SimPM.RS.sources[n].EvoFile), "NOFILE");
...
)
{
    ...
    if (SimPM.RS.Nsources > 0) {
#ifdef TESTING
        cout << "WRITING Radiation Source PARAMETERS, Nsrc="
             << SimPM.RS.Nsources << "\n";
#endif
        //
        ...
        for (list<pm_base*>::iterator iter = rt_src.begin();
            ...
            err = write_header_param(p);
            if (err)
                rep.error("Error writing Radiation source parameter", p->name);
        }
    }  // write RT data.
    ...
    if (SWP.Nsources > 0) {
        ...
        for (int isw = 0; isw < SWP.Nsources; isw++) {
            ...
            (*iter)->set_ptr(
                static_cast<void*>(&SWP.params[isw]->enhance_mdot));
            err = write_header_param(*iter);
            if (err) rep.error("Error writing parameter", (*iter)->name);
            ++iter;
            ...
            (*iter)->set_ptr(
                static_cast<void*>(&SWP.params[isw]->t_scalefactor));
            err = write_header_param(*iter);
            if (err) rep.error("Error writing parameter", (*iter)->name);
            ++iter;
...
)
{
    ...
    if (SimPM.starttime < 0) {
        rep.warning(
            "(Dataio::check_header_parameters) startime <0, PROBABLY AN ERROR!",
            0., SimPM.starttime);
        SimPM.starttime = 0.;
    }
    ...
    if (SimPM.timestep < 0) {
        rep.warning(
            "(Dataio::check_header_parameters) timestep <0, PROBABLY AN ERROR!",
            1, SimPM.timestep);
        SimPM.timestep = 0;
    }
    if (SimPM.NG[0] < 0)
