...
)
{
    ...
    if (!cg[0])
        rep.error(
            "dataio_text::output_ascii_data() null pointer to grid!", cg[0]);
    dataio_text::gp = cg[0];
...
std::string dataio_text::set_filename(
    const std::string outfile, const long int counter)
{
    ostringstream temp;
    temp.str("");
    temp << outfile << ".";
...
{
    cout << "(dataio_text::get_parameters) from file " << pfile
         << " starting.\n";
    ...
    if (ts == "") {
        ...
        SimPM.solverType = FLUX_RSlinear;
    }
    else if (ts == "RSexact") {
        SimPM.solverType = FLUX_RSexact;
    }
    else if (ts == "RShybrid") {
        SimPM.solverType = FLUX_RShybrid;
    }
    else if (ts == "RSroe") {
        SimPM.solverType = FLUX_RSroe;
    }
    else if (ts == "RSroe_pv") {
        SimPM.solverType = FLUX_RSroe_pv;
    }
    else if (ts == "RS_FVS") {
        SimPM.solverType = FLUX_FVS;
    }
    else if (ts == "RS_HLLD") {
        SimPM.solverType = FLUX_RS_HLLD;
    }
    else if (ts == "RS_HLL") {
    ...
    if (ts == "") {
        ...
        cout
            << " Please use \'euler\',\'i-mhd\',\'glm-mhd\',\'euler-Eint\' for "
               "eqn-type\n";
        rep.error(
    ...
    SimPM.ntracer = tr;
    SimPM.ftr = SimPM.nvar;  // Set first tracer element in state vector to be
                             // after all primitive vars.
    ...
    if (SimPM.nvar <= nv) {
        ...
        rep.warning(
            "nvar in paramfile less than that specified by eqn. type",
            SimPM.nvar, nv);
    }
    ...
    else if (ts == "cartesian")
        SimPM.coord_sys = COORD_CRT;
    else if (ts == "cylindrical")
    ...
    if (oft == "TXT" || oft == "txt" || oft == "TEXT" || oft == "text")
        SimPM.typeofop = 1;
#ifdef FITS
    else if (oft == "FITS" || oft == "fits")
        SimPM.typeofop = 2;
    else if (oft == "BOTH" || oft == "both")
        SimPM.typeofop = 4;
#endif  // if FITS
#ifdef SILO
    else if (oft == "SILO" || oft == "silo")
        SimPM.typeofop = 5;
#endif  // if SILO
    else {
    ...
    if ((SimPM.artviscosity =
             atoi((rp->find_parameter("ArtificialViscosity")).c_str()))
        == 0) {
    ...
    if (SimPM.spOOA == OA1) {
    }  // cout<<"\tFirst Order Spatial Accuracy.\n";
    ...
    if (SimPM.tmOOA == OA1) {
    }  // cout<<"\tFirst Order Time Accuracy.\n";
    ...
    if ((rp->find_parameter("FinishTime")) == "")
        SimPM.finishtime = -1.;
    else
    ...
    if ((unit = rp->find_parameter("units")) == "") {
        cout
            << "No units specified in parameterfile.  Using code units for input "
               "data.\n";
        uc.unitsys  = "code";
        uc.density  = "no units";
        uc.length   = "no units";
...
)
{
    ...
    //  x[Ndim], rho, p_g, v_x, v_y, v_z, e_int(erg/mass), [B_x, B_y, B_z,
    //  p_g+p_m].\n";
    ...
    outf
        << "# format: "
           "x,[y,z,],rho,pg,vx,vy,vz,[Bx,By,Bz],[Tr0,Tr1,Tr2,..],T,[Tau0,Tau1,.."
           ".]\n";
    ...
    do {
        ...
        if (SimPM.eqntype == EQMHD || SimPM.eqntype == EQGLM
            || SimPM.eqntype == EQFCD) {
            b2 = cpt->P[BX] * cpt->P[BX] + cpt->P[BY] * cpt->P[BY]
                 + cpt->P[BZ] * cpt->P[BZ];
            //       outf <<"  "<< cpt->P[BX] <<"  "<< cpt->P[BY] <<"  "<<
            //       cpt->P[BZ]
            //       <<"  ";
            outf << "  " << cpt->P[PG] + b2 / 2.;
...
)
{
    ...
    if (typeofic == "SHOCKTUBE") {
        ...
        if (SimPM.ndim == 1) {
            ...
            if ((rp->find_parameter("ShockAngle")) == "") {
                ...
                theta = atof((rp->find_parameter("ShockAngle")).c_str()) * M_PI
                        / 180.;
            }
            cout << "theta=" << theta << "\n";
            ...
            if (fabs(theta) < 1.e-6) {  // If zero slope, then just run a
                ...
                do {
                    if (CI.get_dpos(cpt, YY) - dx
                        > ypivot
                              + slope
                                    * (CI.get_dpos(cpt, XX) + dx / 2. - xpivot))
                        for (int v = 0; v < SimPM.nvar; v++)
                            cpt->P[v] = cpt->Ph[v] = left[v];
                    else if (
                        ...
                        <= ypivot
                               + slope
                                     * (CI.get_dpos(cpt, XX) - dx / 2.
                                        - xpivot))
                        for (int v = 0; v < SimPM.nvar; v++)
                            cpt->P[v] = cpt->Ph[v] = right[v];
                    else {
                        //      cout <<"intermediate point.
                        //      x="<<CI.get_dpos(cpt,XX)<<",
                        //      y="<<CI.get_dpos(cpt,YY)<<",\t";
                        ...
                        for (int i = 0; i < ntot; i++) {
                            ...
                            if (SimPM.ndim > 1)
                                pos[YY] =
                                    startpt[YY] + ((i / nint) % nint) * dxc;
                            if (SimPM.ndim > 2)
                                pos[ZZ] = startpt[ZZ]
                                          + ((i / nint / nint) % nint) * dxc;
                            if (pos[YY]
                                > ypivot
                                      + slope * (pos[XX] + dxc / 2. - xpivot)) {
                                frac += dv;
                            }
                        }
    ...
    if (noise > 0) {
        cout << "\t Adding noise to Data at 0.1% level to pressure, type="
             << noise;
