				      ...
				      )
{
  ...
  int status=0;
  long int pix[ndim];
  for (int j=0;j<ndim;j++) pix[j]=npt[j];

  //  for (int j=0;j<ndim;j++) cout <<"  pix[j]="<<pix[j];
  //cout <<"\tff: "<<ff<<" imtype: "<<DOUBLE_IMG<<" ndim:"<<ndim<<" status:"<<status<<" name:"<<name<<"\n";
  fits_create_img(ff,DOUBLE_IMG,ndim,pix,&status);
  //cout <<"\tff: "<<ff<<" imtype: "<<DOUBLE_IMG<<" ndim:"<<ndim<<" status:"<<&status<<"\n";

  char temp[256]; strcpy(temp,name.c_str());
				     ...
				     )
{
  ...
  //
  // First convert xmin,xmax into pixels, and create bottom-left and top-right pixels.
  //
  long int npix=1;
  for (int i=0;i<ndim;i++) {
    // FITS uses unit offset values to address pixels. First pixel is [1,1,1]
    fpix[i] = static_cast<long int>((localxmin[i]-globalxmin[i])*ONE_PLUS_EPS/pix_size) +1;
    lpix[i] = fpix[i] + npt[i] -1; // -1 because it's inclusive: fpix,pfix+1,...,lpix
    npix *= (lpix[i]-fpix[i]+1);  // +1 because of previous line.
    //    cout <<"fpix[i],lpix[i] = "<<fpix[i]<<", "<<lpix[i]<<"\n";
  }
  if (npix != ntot) rep.error("Pixel counting failed in write_fits_image()",npix-ntot);

  // finally write the data.
  //  int fits_write_subset/ffpss(ff,TDOUBLE,long *fpix, long*lpix,*data,&status)
  // fpix is a vector of the bottom left pixel (unit offset).  lpix is the top right
  // pixel.  The function then writes data into that subset of the image.
