...
#ifdef TEST_SILO_IO
        cout << "\n\tSRAD_read_var2grid() quadvar has type="
             << silodata->datatype;
...
#ifdef TEST_SILO_IO
                cout << "!READING DATA!!... myrank=" << mpiPM->get_myrank()
                     << "  i=" << count;
#endif
...
#ifdef TEST_SILO_IO
                cout << "waiting my turn... myrank=" << mpiPM->get_myrank()
                     << "  i=" << count;
#endif
            }
...
)
{
    ...
    for (std::vector<string>::iterator i = readvars.begin();
        ...
        err = PP_read_var2grid(
            dbfile, ggg, SimPM, (*i), SimPM.Ncell, mesh_iXmin, mesh_iXmax);
        if (err)
        ...
        for (int igroup = 0; igroup < ng; igroup++) {
            ...
            if (R != 0) {
                if (ifile < R)
                    ...
                    filePM.set_myrank(
                        (R * groupsize) + (ifile - R) * (groupsize - 1)
                        + igroup);
            }
            else {
                filePM.set_myrank(ifile * groupsize + igroup);
            ...
            if (!get_data) {
#ifdef TEST_SILO_IO
                cout << "*** skipping mesh " << qm_name
                     << " because not on local domain.\n";
#endif
            }
            else {
#ifdef TEST_SILO_IO
                cout << "**** reading mesh " << qm_name
                     << " because it is on local domain.\n";
                ...
                for (std::vector<string>::iterator i = readvars.begin();
                     i != readvars.end(); ++i) {
                    err = PP_read_var2grid(
                        dbfile, ggg, SimPM, (*i), filePM.LocalNcell, mesh_iXmin,
                        mesh_iXmax);
                    if (err) rep.error("error reading variable", (*i));
                }  // loop over variables.
            }
...
)
{
    ...
    get_quadmesh_extents(
        dbfile, mesh_dir, qm_name, mesh_xmin, mesh_xmax, SimPM);
    ...
    if (silo_dtype == DB_FLOAT) {
        ...
        for (int v = 0; v < ndim; v++) {
            buffer =
                std::min(1.0e-5, 0.1 / ggg->SIM_iRange(static_cast<axes>(v)));
