 ...
 * basic parallel I/O, where each processor writes its own file.  Its
 * other advantage is that I know it well, having used it before.
...
/** \brief Utility class for various fits operations. */
class utility_fitsio {
 public:
  utility_fitsio();
  virtual ~utility_fitsio() {}
   /** \brief Called by OutputData() to create an empty fits image in the 
    * current output file, to hold the simulation data for some variable.
    * */
   int create_fits_image(fitsfile *,   ///< fitsfile pointer.
			 const string, ///< extname of hdu to create.
			 const int,    ///< Dimensionality of image
			 const int *   ///< vector npix in each dimension
			 );
   /** \brief Called by OutputData() once an image has been created; this
    ...
    * domain.
    * */
   int write_fits_image(fitsfile *,     ///< fitsfile pointer.
   ...
   /** \brief This checks that the fits image to be read or written has the
    * dimensions and number of pixels we expect.  If not, it says why and the
    * code exits, as this would be a serious problem.
    * */
   int check_fits_image_dimensions(fitsfile *,   ///< fitsfile pointer.
   ...
   /** \brief Given that we have opened a fits file, read in the header, 
...
/** \brief Class for reading and writing uniform grid data to fits images. */
class DataIOFits : public utility_fitsio, public DataIOBase {
  ...
  std::string choose_filename(
   ...
   /** \brief Reads the fits header from the file specified, and puts the
   ...
   /** \brief This reads the fits images in turn, and puts the data into
  ...
  protected:
   ...
   /** \brief Puts a grid variable into a 1D data array. */
   int put_variable_into_data_array(const string,   ///< variable name to put in array.
    ...
    * */
   int read_fits_image(fitsfile *, ///< fitsfile pointer.
