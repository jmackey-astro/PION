...
)
{
    ...
    if (!mpiPM->WriteSingleFile) {
        ...
        temp.str("");
        cout << "Proc " << mpiPM->get_myrank() << ":\t writing to file "
             << outfile << "\n";
        ...
        for (int i = 0; i < nvar; i++) {
            if (mpiPM->WriteFullImage) {  // write full image, with only local
                ...
                err += write_fits_image(
                    ...
                    SimPM.levels[l].dx, SimPM.ndim, mpiPM->LocalNG,
                    mpiPM->LocalNcell, data);
            }
            else {  // Write only part of image that is on local grid.
                err += create_fits_image(
                    ff, extname[i], SimPM.ndim, mpiPM->LocalNG);
                err += put_variable_into_data_array(
                    SimPM, extname[i], mpiPM->LocalNcell, &data);
                err += write_fits_image(
        ...
        cout
            << "WARNING! THIS IS NOT SYNCHRONOUS!  WILL FAIL RANDOMLY AND CAUSE "
               "CRASH.\n";
        if (file_exists(outfile)) {
        ...
        for (int i = 0; i < nvar; i++) {
            ...
            err = check_fits_image_dimensions(
                ff, extname[i], SimPM.ndim, SimPM.NG);
            if (err != 0)
                rep.error(
        ...
        if (status) {
            fits_report_error(stderr, status);
            cerr
                << "DataIOFits_pllel::OutputData() SingleFile: Error writing to "
                   "existing file\n";
            return (err);
        }
        release_lock(outfile);
...
)
{
    ...
    if (SimPM.eqntype == EQEUL || SimPM.eqntype == EQEUL_ISO
        ...
        if (SimPM.nvar > 10) {
            rep.error(
                "DataIOFits_pllel::ReadData() too many tracers.", SimPM.nvar);
        }
            ...
            if (err != 0) {
                ...
                if (!mpiPM->ReadSingleFile) {
                    ...
                    cout << "Proc " << mpiPM->get_myrank()
                         << ":\t reading from file " << infile << "\n";
                    err += check_fits_image_dimensions(
                        ff, var[i], SimPM.ndim, SimPM.NG);
                    if (err) rep.error("image wrong size.", err);
                    err += read_fits_image(
                        SimPM, ff, var[i], mpiPM->LocalXmin, SimPM.Xmin,
                        mpiPM->LocalNG, mpiPM->LocalNcell);
                    if (err) rep.error("error reading image.", err);
                }
                else
