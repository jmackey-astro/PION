  ...
  if (d[ZN]) {
    if (d[XN])
      full_ngb_list.push_back(
          nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY]) + (ix[XX] - 1));
    if (d[XP])
      full_ngb_list.push_back(
          nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY]) + (ix[XX] + 1));
    if (d[YN])
      full_ngb_list.push_back(
          nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY] - 1) + (ix[XX]));
    if (d[YP])
      full_ngb_list.push_back(
          nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY] + 1) + (ix[XX]));
    if (d[YN] && d[XN])
    ...
    if (par.ndim == 1) {
      ...
      for (int i = 0; i < par.ndim; i++) {
        ...
        for (size_t v = 0; v < children.size(); v++) {
          child.rank = children[v];
#ifdef TEST_BC89FLUX
          cout << " found neighbour on l+1 (neg-dir) with rank " << child.rank
               << endl;
#endif
        ...
        for (size_t v = 0; v < children.size(); v++) {
          child.rank = children[v];
#ifdef TEST_BC89FLUX
          cout << " found neighbour on l+1 (pos-dir) with rank " << child.rank
               << endl;
#endif
