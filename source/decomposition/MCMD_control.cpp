...
MCMDcontrol::MCMDcontrol()
{
    ...
    WriteSingleFile =
        false;  ///< If all processes to write to one file, set this
    WriteFullImage =
...
)
{
    ...
    // is always the one in which the domain is longest.  In this way we
    // minimize domain interfaces, which should minimize communication.
    //
    if (SimPM.ndim == 1) {
        ...
        while (npcounter < nproc) {
            ...
            if (SimPM.EP.raytracing && SimPM.RS.Nsources > 0) {
                ...
                //
                // Now if at_infinity is still set, we want to get the direction
                // along which we don't want to decompose the domain. Now also
                // checks if there are multiple sources at infinity in different
                // directions, in which case we can't make all of them fully
                // parallel.
                //
                if (at_infinity) {
                    for (int isrc = 0; isrc < SimPM.RS.Nsources; isrc++) {
                        if ((SimPM.RS.sources[isrc].type == RT_SRC_SINGLE)
                            ...
                            for (int ii = 0; ii < SimPM.ndim; ii++) {
                                if (fabs(SimPM.RS.sources[isrc].pos[ii])
                                    > 1.e99) {
                                    if (dsrc == -1) dsrc = ii;
                ...
                if (at_infinity) {
                    // cout <<"\t\tFound a source at infinity in direction
                    // "<<dsrc; cout <<", so not decomposing domain in this
                    // direction.\n";
                    if (dsrc < 0)
                        rep.error("no direction to source at infinity", dsrc);
                }
                else {
                    // cout <<"\t\tEither multiple or no sources at
                    // infinity.\n";
                    dsrc = -1;
                }
            }
        ...
        for (int i = 0; i < SimPM.ndim; i++)
            nx[i] =
                static_cast<int>(ONE_PLUS_EPS * level.Range[i] / LocalRange[i]);
        int temp = myrank;
        if (SimPM.ndim == 3) {
        ...
        rep.error("Bad NDIM in DecomposeDomain", SimPM.ndim);
...
)
{
    ...
    if (d[ZN]) {
        if (d[XN])
            full_ngb_list.push_back(
                nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY])
                + (ix[XX] - 1));
        if (d[XP])
            full_ngb_list.push_back(
                nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY])
        ...
        if (d[YN])
            full_ngb_list.push_back(
                nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY] - 1)
                + (ix[XX]));
        if (d[YP])
            full_ngb_list.push_back(
                nx[XX] * nx[YY] * (ix[ZZ] - 1) + nx[XX] * (ix[YY] + 1)
...
)
{
    ...
    if (l > 0) {
        ...
        for (int i = 0; i < par.ndim; i++) {
            ...
            pgrid.Xmin[i] =
                par.levels[l - 1].Xmin[i] + ix[i] * 2.0 * LocalRange[i];
        ...
        if (pconst.equalD(pgrid.Xmin[XX], par.levels[l - 1].Xmin[XX]))
            pgrid_ngb[XN].rank = -999;
        if (pconst.equalD(pgrid.Xmax[XX], par.levels[l - 1].Xmax[XX]))
            pgrid_ngb[XP].rank = -999;
        if (par.ndim > 1) {
            pgrid_ngb[YN].rank = parent_proc - nx[XX];
            pgrid_ngb[YP].rank = parent_proc + nx[XX];
            if (pconst.equalD(pgrid.Xmin[YY], par.levels[l - 1].Xmin[YY]))
                pgrid_ngb[YN].rank = -999;
            if (pconst.equalD(pgrid.Xmax[YY], par.levels[l - 1].Xmax[YY]))
                pgrid_ngb[YP].rank = -999;
        }
        if (par.ndim > 2) {
            pgrid_ngb[ZN].rank = parent_proc - nx[XX] * nx[YY];
            pgrid_ngb[ZP].rank = parent_proc + nx[XX] * nx[YY];
            if (pconst.equalD(pgrid.Xmin[ZZ], par.levels[l - 1].Xmin[ZZ]))
                pgrid_ngb[ZN].rank = -999;
            if (pconst.equalD(pgrid.Xmax[ZZ], par.levels[l - 1].Xmax[ZZ]))
                pgrid_ngb[ZP].rank = -999;
        }
        // Now set Xmin, Xmax:
        for (int d = 0; d < 2 * par.ndim; d++) {
            if (pgrid_ngb[d].rank >= 0) {
                ...
                for (int i = 0; i < par.ndim; i++)
                    pgrid_ngb[d].Xmax[i] =
                        pgrid_ngb[d].Xmin[i] + 2.0 * LocalRange[i];
            }
        }

#ifdef TEST_BC89FLUX
        // *** debugging info ***
        cout << "level " << l << ", parent proc = " << parent_proc
             << ", parent grid xmin/xmax:\n";
        rep.printVec("Xmin", pgrid.Xmin, par.ndim);
        rep.printVec("Xmax", pgrid.Xmax, par.ndim);
        for (int d = 0; d < 2 * par.ndim; d++) {
            if (pgrid_ngb[d].rank < 0)
                cout << "pproc ngb in direction " << d
                     << " has no neighbour.\n";
            else {
                cout << "pproc ngb in direction " << d
                     << " has neighbour proc ";
    ...
    if (l < par.grid_nlevels - 1 && nproc == 1) {
        ...
        if (children.size() > 0) {
            ...
            children.erase(
                unique(children.begin(), children.end()), children.end());
        }
#ifdef TEST_BC89FLUX
        // *** debugging info ***
        ...
        for (size_t v = 0; v < children.size(); v++) {
            ...
            for (int i = 0; i < par.ndim; i++) {
                ...
                child.Xmin[i] =
                    par.levels[l + 1].Xmin[i] + ix[i] * 0.5 * LocalRange[i];
        ...
        for (size_t v = 0; v < child_procs.size(); v++) {
            cout << "child procs on level " << l + 1
                 << ": rank=" << child_procs[v].rank << "\n";
    ...
    if (l == par.grid_nlevels - 1 || nproc == 1) {
        ...
        if (par.ndim == 1) {
...
#ifdef TEST_BC89FLUX
            cout << "2D: getting ranks for l+1 grids that share a face with my "
                    "grid.\n";
            cout.flush();
#endif
            for (int i = 0; i < par.ndim; i++) {
                ...
                for (int p = 0; p < 4; p++) {
                    centre[pdir[0]] =
                        LocalXmin[pdir[0]] + 0.25 * p * LocalRange[pdir[0]];
                    child_rank = get_grid_rank(par, centre, l + 1);
                    if (child_rank >= 0) children.push_back(child_rank);
                }
                sort(children.begin(), children.end());
                ...
                for (size_t v = 0; v < children.size(); v++) {
                    child.rank = children[v];
#ifdef TEST_BC89FLUX
                    cout << " found neighbour on l+1 (neg-dir) with rank "
                         << child.rank << endl;
#endif
                ...
                for (size_t v = 0; v < children.size(); v++) {
                    child.rank = children[v];
#ifdef TEST_BC89FLUX
                    cout << " found neighbour on l+1 (pos-dir) with rank "
                         << child.rank << endl;
#endif
...
#ifdef TEST_BC89FLUX
            // *** debugging info ***
            cout << "3D: getting ranks for l+1 grids that share a face with my "
                    "grid.\n";
            cout.flush();
            // *** debugging info ***
#endif
            for (int i = 0; i < par.ndim; i++) {
...
#ifdef TEST_BC89FLUX
                // *** debugging info ***
                cout << "3D: axis = " << i << ", perp dirs = " << pdir[0]
                     << ", " << pdir[1] << "\n";
                cout.flush();
                // *** debugging info ***
#endif
                ...
                for (int p = 0; p < 4; p++) {
                    for (int q = 0; q < 4; q++) {
                        centre[pdir[0]] =
                            LocalXmin[pdir[0]] + 0.25 * p * LocalRange[pdir[0]];
                        centre[pdir[1]] =
                            LocalXmin[pdir[1]] + 0.25 * q * LocalRange[pdir[1]];
                        child_rank = get_grid_rank(par, centre, l + 1);
                        if (child_rank >= 0) children.push_back(child_rank);
                    }
                }
                if (children.size() > 0) {
                    sort(children.begin(), children.end());
                    children.erase(
                        unique(children.begin(), children.end()),
                        children.end());
                }
                for (size_t v = 0; v < children.size(); v++) {
                    child.rank = children[v];
...
#ifdef TEST_BC89FLUX
                        // *** debugging info ***
                        // cout <<"+ve direction, facing child rank =
                        // "<<child_rank<<" : "; rep.printVec("+ve dir,
                        // centre",centre,par.ndim);
                        // *** debugging info ***
#endif
                        if (child_rank >= 0) children.push_back(child_rank);
                    }
        ...
        for (int d = 0; d < 2 * par.ndim; d++) {
            for (size_t cg = 0; cg < cgrid_ngb[d].size(); cg++) {
                ...
                for (int i = 0; i < par.ndim; i++) {
                    // child level has range 0.5x my range in each direction
                    cgrid_ngb[d][cg].Xmin[i] =
                        par.levels[l + 1].Xmin[i] + ix[i] * 0.5 * LocalRange[i];
                    cgrid_ngb[d][cg].Xmax[i] =
                        cgrid_ngb[d][cg].Xmin[i] + 0.5 * LocalRange[i];
                }
            }  // loop over grids
        }      // loop over dims
        ...
        for (int d = 0; d < 2 * par.ndim; d++) {
            for (size_t cg = 0; cg < cgrid_ngb[d].size(); cg++) {
                cout << "dir=" << d << ", cg=" << cg
                     << ", l+1 ngb grid rank=" << cgrid_ngb[d][cg].rank << "\n";
