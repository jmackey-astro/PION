    ...
    if (Ns > 0) {
        ...
        if (wtype == 0) {
            grid->Wind = new stellar_wind(
                par.ndim, par.nvar, par.ntracer, par.ftr, par.tracers,
    ...
    for (int isw = 0; isw < Ns; isw++) {
        ...
        if (SWP.params[isw]->type == WINDTYPE_CONSTANT) {
            ...
            err = grid->Wind->add_source(
                SWP.params[isw]->dpos, SWP.params[isw]->radius,
                SWP.params[isw]->type, SWP.params[isw]->Mdot,
                SWP.params[isw]->Vinf, SWP.params[isw]->Vrot,
            ...
            err = grid->Wind->add_evolving_source(
                SWP.params[isw]->dpos, SWP.params[isw]->radius,
                SWP.params[isw]->type, SWP.params[isw]->tr,
