...
NG_BC89flux::~NG_BC89flux()
{
    ...
    for (unsigned int l = 0; l < flux_update_recv.size(); l++) {
        for (unsigned int d = 0; d < flux_update_recv[l].size(); d++) {
            for (unsigned int f = 0; f < flux_update_recv[l][d].fi.size();
                 f++) {
    ...
    for (unsigned int l = 0; l < flux_update_send.size(); l++) {
        for (unsigned int d = 0; d < flux_update_send[l].size(); d++) {
            for (unsigned int f = 0; f < flux_update_send[l][d].fi.size();
                 f++) {
...
#endif
            add_cells_to_face(
                par, grid, static_cast<enum direction>(d), ixmin, ixmax, ncell,
                1, flux_update_recv[l][d]);
#ifdef TEST_BC89FLUX
...
)
{
    ...
    size_t nc = 1;  // number of cells in each interface
    int l     = lm1 + 1;
    int ixmin[MAX_DIM], ixmax[MAX_DIM], ncell[MAX_DIM],
        nface[MAX_DIM];                        // interface
    ...
    for (int d = 0; d < 2 * par.ndim; d++) {
        if (send[d]) {
#ifdef TEST_BC89FLUX
            cout << "FLUX_SEND: d=" << d << ", nel=" << nel[d]
                 << "... adding cells to face\n";
#endif
            add_cells_to_face(
                par, grid, static_cast<enum direction>(d), ixmin, ixmax, nface,
                2, flux_update_send[l][d]);
        }
    }
...
)
{
    ...
    if (par.ndim == 1) {
        ...
        if (nface[perpaxis] != static_cast<int>(flux.fi.size())) {
            cout << "add_cells_to_face(" << d << ", " << ixmin[a] << ", "
                 << ixmax[a];
            cout << ", " << nface[a] << ", " << ncell
                 << "), list-size=" << flux.fi.size() << "\n";
            rep.printVec("ixmin", ixmin, par.ndim);
        ...
        if (nface[perpaxis1] * nface[perpaxis2]
            ...
            rep.printVec("ixmax", ixmax, par.ndim);
            rep.printVec("nface", nface, par.ndim);
            cout << "ERROR: nfaces=" << nface[perpaxis1] * nface[perpaxis2]
                 << ", and ";
            cout << "flux struct size is " << flux.fi.size() << "\n";
            cout.flush();
    ...
    for (unsigned int d = 0; d < flux_update_send[l].size(); d++) {
        ...
        for (unsigned int f = 0; f < flux_update_send[l][d].fi.size(); f++) {
            ...
            if (fi == 0 && f == 0)
                ...
                rep.error("save_fine_fluxes fi=0", d);
            ...
            for (int i = 0; i < flux_update_send[l][d].Ncells; i++) {
                if (!fi->c[i]->F[a])
                    rep.error("fine flux is not allocated!", f);
