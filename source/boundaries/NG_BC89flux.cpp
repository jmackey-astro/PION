      ...
      )
{
  ...
  if (grid != par.levels[lp1].parent)
    rep.error("level lp1 is not my child!",lp1);
  ...
  for (int v=0;v<par.ndim;v++) {
    ...
    if (cg_ixmin[v] < f_lxmin[v]) recv[2*v] = true;
    else                          recv[2*v] = false;
    
    ixmax[v] = std::min(cg_ixmax[v], f_lxmax[v]);
    if (cg_ixmax[v] > f_lxmax[v]) recv[2*v+1] = true;
  ...
  for (int v=0;v<2*par.ndim;v++) nel[v]=0;
  ...
  for (int d=0; d<2*par.ndim; d++) {
    if (recv[d]) {
      ...
      add_cells_to_face(par,grid,static_cast<enum direction>(d),
              ixmin,ixmax, ncell,1,flux_update_recv[l][d]);
#ifdef TEST_BC89FLUX
      cout <<"UG:FRECV: d="<<d<<", nel="<<nel[d]<<"\n";
#endif
    }
  }
  return 0;
      ...
      )
{
  ...
  for (int ax=0;ax<par.ndim;ax++) {
    ...
    if (fg_ixmin[ax] > c_lxmin[ax]) send[2*ax] = true;
    else                            send[2*ax] = false;
    
    ixmax[ax] = std::min(fg_ixmax[ax], c_lxmax[ax]);
    if (fg_ixmax[ax] < c_lxmax[ax]) send[2*ax+1] = true;
  ...
  for (int d=0;d<2*par.ndim;d++) nel[d]=0;
  ...
  case 3:
    ...
    break;
  default:
    rep.error("bad ndim in setup_flux_send",par.ndim);
    break;
  }
...
#ifdef TEST_BC89FLUX
      cout <<"d="<<d<<", nel="<<nel[d]<<"... allocating memory for ";
      cout <<nel[d]<<" elements\n";
#endif
...
#endif
      add_cells_to_face(par,grid,static_cast<enum direction>(d),
            ixmin,ixmax, nface,2,flux_update_send[l][d]);
    }
  }
      ...
      )
{
  ...
  if (par.ndim==1) {
    if (d == XN) {
      while (c->pos[XX] < ixmin[XX]-idx) c = grid->NextPt(c,XP);
    }
    else if (d == XP) {
      while (c->pos[XX] < ixmax[XX]) c = grid->NextPt(c,XP);
    }
    flux.fi[0]->c[0] = c;
    ...
    case XN:
      while (c->pos[XX] < ixmin[XX]-idx) c = grid->NextPt(c,XP);
      while (c->pos[YY] < ixmin[YY]) c = grid->NextPt(c,YP);
      perpdir = YP;
      perpaxis = YY;
      break;
    case XP:
      ...
      break;
    case YN:
      while (c->pos[XX] < ixmin[XX]) c = grid->NextPt(c,XP);
      while (c->pos[YY] < ixmin[YY]-idx) c = grid->NextPt(c,YP);
      perpdir = XP;
      perpaxis = XX;
      break;
    case YP:
      while (c->pos[XX] < ixmin[XX]) c = grid->NextPt(c,XP);
      while (c->pos[YY] < ixmax[YY]) c = grid->NextPt(c,YP);
      perpdir = XP;
      perpaxis = XX;
      break;
    default:
    ...
    case XN:
      while (c->pos[XX] < ixmin[XX]-idx) c = grid->NextPt(c,XP);
      while (c->pos[YY] < ixmin[YY]) c = grid->NextPt(c,YP);
      while (c->pos[ZZ] < ixmin[ZZ]) c = grid->NextPt(c,ZP);
      perpdir1 = YP;
      perpdir2 = ZP;
      perpaxis1 = YY;
      perpaxis2 = ZZ;
    ...
    case ZN:
      while (c->pos[XX] < ixmin[XX]) c = grid->NextPt(c,XP);
      while (c->pos[YY] < ixmin[YY]) c = grid->NextPt(c,YP);
      while (c->pos[ZZ] < ixmin[ZZ]-idx) c = grid->NextPt(c,ZP);
      perpdir1 = XP;
      perpdir2 = YP;
      perpaxis1 = XX;
      perpaxis2 = YY;
    ...
    case ZP:
      while (c->pos[XX] < ixmin[XX]) c = grid->NextPt(c,XP);
      while (c->pos[YY] < ixmin[YY]) c = grid->NextPt(c,YP);
      while (c->pos[ZZ] < ixmax[ZZ]) c = grid->NextPt(c,ZP);
      perpdir1 = XP;
      perpdir2 = YP;
      perpaxis1 = XX;
      perpaxis2 = YY;
    ...
    for (int i=0;i<nface[perpaxis2]; i++) {
      for (int j=0;j<nface[perpaxis1]; j++) {
        if (ncell==1) {
          ...
          c->F[a] = mem.myalloc(c->F[a],par.nvar);
          for (int v=0;v<par.nvar;v++) c->F[a][v]=0.0;
          c->isbd_ref[d] = true;
          ...
          for (int v=0;v<par.nvar;v++) m2->F[a][v]=0.0;
          m2->isbd_ref[d] = true;
        ...
        for (int ic=0;ic<ncell;ic++) c = grid->NextPt(c,perpdir1);
      }
      for (int ic=0;ic<ncell;ic++) marker = grid->NextPt(marker,perpdir2);
      c = marker;
    }
  }  // 3D
    ...
    )
{
  ...
  for (unsigned int d=0; d<flux_update_send[l].size(); d++) {
    ...
    cout <<flux_update_send[l][d].fi.size()<<"\n";
#endif
...
#ifdef TEST_BC89FLUX
  cout <<"level "<<l<<": correcting fluxes from finer grid\n";
#endif
