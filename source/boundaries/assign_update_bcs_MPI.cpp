...
)
{
    ...
    for (size_t i = 0; i < grid->BC_bd.size(); i++) {
        switch (grid->BC_bd[i]->itype) {
            case BCMPI:
#ifdef TEST_MPI_BC
                cout << ppar->get_myrank() << " assigning MPI boundary in dir "
                     << i << "\n";
#endif
                err += BC_assign_BCMPI(
                    par, level, grid, grid->BC_bd[i], BC_MPItag);
                break;
            case PERIODIC:
                break;
            case OUTFLOW:
    ...
    const int maxstep)
{
    ...
    for (i = 0; i < grid->BC_bd.size(); i++) {
        ...
        switch (b->itype) {
            ...
            case BCMPI:
                err += BC_update_BCMPI(
                    par, level, grid, b, cstep, maxstep, BC_MPItag);
                break;
            case OUTFLOW:
            ...
            default:
                rep.error(
                    "assign_update_bcs_MPI::Unhandled BC: external", b->itype);
                break;
        }
