        ...
        for (int ic = 0; ic < nchild; ic++) {
            ...
            for (int ax = 0; ax < par.ndim; ax++) {
                ...
                if ((ixmax[ax] - ixmin[ax]) % idx != 0) {
                    rep.error(
                        "interface region not divisible!",
                        ixmax[ax] - ixmin[ax]);
                }
            }  // all dimensions
...
#endif
                    add_cells_to_face(
                        par, grid, static_cast<enum direction>(d), ixmin, ixmax,
                        ncell, 1, flux_update_recv[l][off + d]);
                }
            }  // loop over dims.
            ...
            if (nsub > 0) {
                ...
                for (size_t ic = 0; ic < nsub; ic++) {
                    ...
                    switch (par.ndim) {
                        ...
                        case 2:
                            ixmin[perp[0]] =
                                std::max(ixmin[perp[0]], cg_ixmin[perp[0]]);
                            ixmax[perp[0]] =
                                std::min(ixmax[perp[0]], cg_ixmax[perp[0]]);
                            ncell[axis] = 1;
                            ncell[perp[0]] =
                                (ixmax[perp[0]] - ixmin[perp[0]]) / idx;
                            nel = ncell[perp[0]];
#ifdef TEST_BC89FLUX
                            cout << "BC89 recv 2D from l+1 ngb: xmax="
                                 << ixmax[perp[0]];
                            cout << ", xmin=" << ixmin[perp[0]]
                                 << ", nel=" << nel << "\n";
#endif
                            break;
                        ...
                        case 3:
                            ...
                            ixmax[perp[0]] =
                                std::min(ixmax[perp[0]], cg_ixmax[perp[0]]);
                            ixmin[perp[1]] =
                                std::max(ixmin[perp[1]], cg_ixmin[perp[1]]);
                            ixmax[perp[1]] =
                                std::min(ixmax[perp[1]], cg_ixmax[perp[1]]);
                            ncell[axis] = 1;
                            ncell[perp[0]] =
                                (ixmax[perp[0]] - ixmin[perp[0]]) / idx;
                            ncell[perp[1]] =
                                (ixmax[perp[1]] - ixmin[perp[1]]) / idx;
                            nel = 1;
                            for (int p = 0; p < 2; p++)
                                nel *= ncell[perp[p]];
#ifdef TEST_BC89FLUX
                    ...
                    add_cells_to_face(
                        par, grid,
                        grid->OppDir(static_cast<enum direction>(edge)), ixmin,
...
#ifdef TEST_BC89FLUX
                    cout
                        << "setup_flux_send: negative direction, ngb of parent "
                        << pp << endl;
#endif
                }
                // else it is parent.
...
#ifdef TEST_BC89FLUX
                    cout
                        << "setup_flux_send: positive direction, ngb of parent "
                        << pp << endl;
#endif
                }
                // else it is parent.
...
#ifdef TEST_BC89FLUX
                    cout
                        << "setup_flux_send: positive direction, send to parent "
                        << pp << endl;
#endif
                }
                flux_update_send[l][d].dir = d;
...
)
{
    ...
    for (int isend = 0; isend < n_send; isend++) {
        ...
        if (fup->fi[0] == 0) {
#ifdef TEST_BC89FLUX
            cout << "l=" << l << ": BC89_FLUX send " << isend << " of "
                 << n_send;
    ...
    for (int irecv = 0; irecv < n_bd; irecv++) {
        ...
        for (size_t ii = 0; ii < n_el; ii++) {
            ...
            if (fup->dir % 2 == 0) {
                ...
                spatial_solver->DivStateVectorComponent(
                    fi->c[0], grid, static_cast<axes>(fup->ax), par.nvar,
                    fi->flux, ftmp, utmp);
            }
#ifdef TEST_BC89FLUX
