...
)
{
...
#ifdef TEST_C2F
                    cout << "added " << bdata->c.size()
                         << " cells to C2F send el ";
    ...
    if (fgngb.size() != static_cast<size_t>(2 * par.ndim)) {
        rep.error(
            "l+1 neigbouring grids vector not set up right", fgngb.size());
    }
    for (int d = 0; d < par.ndim; d++) {
        // negative direction
        int dir = 2 * d;
            ...
            if (fgngb[dir].size() != 0) {
                ...
                for (size_t f = 0; f < fgngb[dir].size(); f++) {
                    if (fgngb[dir][f].rank == MCMD->get_myrank()) {
                        ...
                        bdata->dir =
                            2 * d + 1;  // outward normal of child is +ve
                        bdata->c.clear();
                        add_cells_to_C2F_send_list(
                            par, grid, bdata, fg_ixmin, fg_ixmax, l + 1,
                            fl_ixmin, fl_ixmax);
                        b->NGsendC2F.push_back(bdata);
                    }
                }
            ...
            if (fgngb[dir].size() != 0) {
                ...
                for (size_t f = 0; f < fgngb[dir].size(); f++) {
                    if (fgngb[dir][f].rank == MCMD->get_myrank()) {
                        ...
                        bdata->rank       = fgngb[dir][f].rank;
                        bdata->dir =
                            2 * d;  // outward normal of child is -ve dir
                        bdata->c.clear();
                        add_cells_to_C2F_send_list(
                            par, grid, bdata, fg_ixmin, fg_ixmax, l + 1,
                            fl_ixmin, fl_ixmax);
...
#ifdef TEST_C2F
        cout << "BC_update_COARSE_TO_FINE_SEND: l=" << l << ", Sending "
             << n_el;
        ...
        if (par.ndim == 1) {
            ...
            for (f_iter = b->data.begin(); f_iter != b->data.end(); ++f_iter) {
                ...
                if (c->pos[YY] == row_y && c->pos[ZZ] == row_z) {
                    ...
                    c->pos[YY] - row_y == idx || c->pos[ZZ] - row_z == idx) {
...
)
{
    ...
    if (MCMD->get_myrank() == b->NGrecvC2F_parent) {
        ...
        //
        err =
            COMM->receive_double_data(from_rank, recv_tag, recv_id, n_el, buf);
            ...
            if (par.ndim == 1) {
                ...
                for (unsigned int ic = 0; ic < b->NGrecvC2F.size(); ic++) {
                    ...
                    interpolate_coarse2fine2D(
                        par, grid, solver, Ph, ipos, c_vol, sx, sy, f[0], f[1],
                        f[2], f[3]);
