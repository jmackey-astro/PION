...
#ifdef RT_TESTING
        cout << "\tReceive_RT_Boundaries() src=" << src_id
             << ": to recv from: ";
#endif
...
#ifdef RT_TESTING
            cout << "\tReceive_RT_Boundaries() rad_src=" << src_id
                 << ": getting " << ct << " cells from proc. " << from_rank
                 << "\n";
#endif
...
)
{
    ...
    if (!b || !b2)
        rep.error(
            "RT_MPI_bc::RT_populate_recv_boundary() Null b/b2 pointer", b);
