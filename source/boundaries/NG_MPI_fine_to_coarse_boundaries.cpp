...
)
{
  ...
  spdlog::info("***F2C F2C F2C*** Adding F2C SEND for l={}", l);
#endif
...
#ifdef TEST_MPI_NG_F2C
    spdlog::info("***F2C added {} avg cells to this grid\n", nel);
#endif
...
#ifdef TEST_MPI_NG_F2C
  spdlog::info("BC_update_FINE_TO_COARSE_SEND: returned with id={}", id);
#endif
...
)
{
  ...
  int nchild = cg.size();
#ifdef TEST_MPI_NG_F2C
  spdlog::info("level = {}, nchild={}", l, nchild);
#endif
  ...
  for (int i = 0; i < nchild; i++) {
#ifdef TEST_MPI_NG_F2C
    spdlog::info("child = {}, processing.", i);
#endif
...
#ifdef TEST_MPI_NG_F2C
    spdlog::info("F2C MPI: child xmin : {}", ixmin);
    spdlog::info("F2C MPI: child xmax : {}", ixmax);
#endif
    ...
    const int maxstep)
{
  ...
  for (int i = 0; i < nchild; i++) {
#ifdef TEST_MPI_NG_F2C
    spdlog::info("F2C_RECV: child {}, receiving...", i);
#endif
    ...
    if (sub_domain->get_myrank() == cg[i].rank) {
      ...
      for (c_iter = b->NGrecvF2C[i].begin(); c_iter != b->NGrecvF2C[i].end();
        ...
        if (errors) {
          ...
          spdlog::info("cell pos : {}", pos);
          spdlog::info("recv pos : {}", &(buf[i_el]));
        }
        i_el += par.ndim;
#endif
        solver->UtoP(
