    ...
    m_p(pconst.m_p())
{
    ...
    MPv0::pv_elec = -1;
    int firstion =
        2;  // location in p-vec of first ion/electron tracer variable.
    ...
    // this has tracers, but none relating to chemistry
        ...
        if (!cool)
            rep.error(
                "set up MP but no microphysics and not doing cooling!", nions);
        if (nions != 0) {
    ...
    for (int i = 0; i < nions; i++) {
        ...
        ii[i]->index =
            lvar[ions[i]];  // set ion's index in the local state vector.
            ...
            for (unsigned int v = 0; v < ee[i]->ions.size(); v++) {
                for (int vv = 0; vv < nions; vv++)
                    if (ions[vv] == ee[i]->ions[v]) {
                        cout << "\t\tFor element " << ee[i]->el
                             << " adding ion " << ions[vv] << " with index "
                             << ii[vv]->index << "\n";
    ...
    if (nions > 0) {
        ...
        for (int i = 0; i < nions; i++) {
            ...
            if (p_local[xi] > 1.01) {
                ...
                cerr << "ion fraction >1! for ion " << ii[i]->ion
                     << " at value " << p_local[xi] - 1. << "\n";
    ...
    double* ttt)
{
    ...
    gamma   = g;
    double
        P[nvl];  // local state vector for microphysics = [n_h,Eint,x_e,(x_i)]
...
)
{
    ...
    for (int i = 0; i < nions; i++) {
        ...
        if (ep.coll_ionisation) {
            ...
            if (iip1) {
                ...
                temp =
                    Coll_Ion_rate(temperature, ii[i]) * P[lv_elec] * P[lv_nh];
                temp *= P[xi];
        ...
        */
        ...
        if (ep.rad_recombination) {
            if (iim1) {
                ...
                temp = Rad_Recomb_rate(temperature, ii[i]) * P[lv_elec]
                       * P[lv_nh];  // rate [1/s]
                R[xi] -=
                    temp * P[xi];  // recomb from i to i-1 reduces fraction.
                // if (!ep.cooling || temperature<1.0e4) // avoid double
                // counting!
                //
                // This is a bad approximation!!! UPDATE ME!!!
                //
                R[lv_eint] -= temp * P[xi] * ii[i]->e->numfrac * kB
                              * temperature / (gamma - 1.0);
        ...
        if (ep.phot_ionisation) {
            if (ii[i]->i
                == H_1p) {  // only know Hydrogen photo-ionisation for now.
                // if (P[xi]>1.) cout <<"P[xi]="<<P[xi]<<"\n"; //1.0-1.e-12;
                R[lv_dtau] =
                    P[lv_nh] * (1.0 - P[xi]) * phot_xsection(ii[i])
                ...
                R[xi] += temp;
                R[lv_eint] +=
                    temp * 1.602e-12 * 2.4;  // this adds in 2.4eV of energy per
                                             // photo-ionisation.
    ...
    if (ep.cooling) {
        temp = cool->CoolingRate(
            temperature, P[lv_hp], P[lv_nh], FUV_unattenuated_flux,
            FUV_extinction);
        R[lv_eint] -=
...
)
{
    ...
    if (nions == 0 && ep.cooling) {
        ...
        double T =
            (gam - 1.) * P[lv_eint] / kB / 2.0 / P[lv_nh];  // Temperature.
        ...
        rep.error(
            "Can't limit timestep in Microphysics unless only cooling", 99);
    }
