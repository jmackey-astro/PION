...
)
{
    ...
    double Edot = 0.0;  // Edot is calculated in units of erg/s per H nucleon,
                        // multiplied by mpv_nH at the end.
...
#define EXCESS_ENERGY 8.01e-12
                ...
                temp1 = Hi_discrete_mono_photoion_rate(
                    mpv_Tau0, temp1, mpv_nH, mpv_NIdot, PHOTON_ENERGY,
    ...
    //
    // Add Helium free-free (Z^2*n(He)/n(H) = X(He)/X(H) of the H+ free-free
    ...
    //
    Edot -= 1.68e-27 * EP->Helium_MassFrac / (1.0 - EP->Helium_MassFrac)
            * sqrt(T) * x_in * ne;
#endif  // HE_INERT
    ...
    if (N_diff_srcs) {
        ...
        Edot += 1.9e-26 * METALLICITY * mpv_G0_UV
        ...
        //
        Edot +=
            7.7e-32 * METALLICITY * mpv_G0_IR / pow(1.0 + 3.0e4 / mpv_nH, 2.0);
        // cout <<"
        // DfIR="<<7.7e-32*METALLICITY*mpv_G0_IR/pow(1.0+3.0e4/mpv_nH,2.0)<<"\n";
    }
    ...
    // important! Oxygen abundance set to 4.90e-4 from Asplund+(2009,ARAA).
    //
    temp1 = 1.20e-22 * METALLICITY
            * exp(-33610.0 / T - (2180.0 * 2180.0 / T / T)) * x_in * ne
            * exp(-T * T / 5.0e10);
    ...
    //
    temp1 += 1.85e-23 * METALLICITY
             * exp(-28390.0 / T - (1780.0 * 1780.0 / T / T)) * ne * OneMinusX;
    ...
    // rho^{1.6}. I have modified the Henney equation by multiplying by the
    // neutral fraction squared and also exponentially cutting off the function
    // for T>10^5K.
    //
    temp1 = 70.0 + 220.0 * pow(mpv_nH / 1.0e6, 0.2);
    temp2 =
        3.981e-27 * METALLICITY * pow(mpv_nH, 0.6) * sqrt(T) * exp(-temp1 / T);
    //
    ...
    // scaling changes discontinuously (but the function is obviously
    // continuous!). 0.1585 is 100^(-0.4).
    //
