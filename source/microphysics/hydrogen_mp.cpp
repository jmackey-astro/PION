...
/// x-section.
/// - 2011.03.29 JM: Added coll_ion_rates() function to return both ionisation-
/// and
///    heating rates.
...
{
  // -------------------------------------------------------------------
  // We need to set up a spline fit for the Aggarwal (1993) collisional
  // excitation of HI data.
  //
  //
  // Tabulated values for the cooling rate from Raga, Mellema, \& Lundqvist,
  // 1997, ApJS, 109, 517.
  // The rates are for collisionally excited cooling from neutral hydrogen (H
  // I), and the rates are per electron, per H I atom, in units erg.cm3/s
  //
  double T[26] = {
      3162.2776602,   3981.0717055,   5011.8723363,   6309.5734448,
  ...
  double R[26] = {1.150800e-34, 2.312065e-31, 9.571941e-29, 1.132400e-26,
                  4.954502e-25, 9.794900e-24, 1.035142e-22, 6.652732e-22,
                  2.870781e-21, 9.036495e-21, 2.218196e-20, 4.456562e-20,
                  7.655966e-20, 1.158777e-19, 1.588547e-19, 2.013724e-19,
                  2.393316e-19, 2.710192e-19, 2.944422e-19, 3.104560e-19,
                  3.191538e-19, 3.213661e-19, 3.191538e-19, 3.126079e-19,
                  3.033891e-19, 2.917427e-19};

  for (int i = 0; i < cx_Nspl; i++) {
    cx_T[i]    = log10(T[i]);
    cx_rate[i] = log10(R[i]);
  }
  spline(cx_T, cx_rate, cx_Nspl, 0.0, 0.0, cx_spline_id);
  //
  ...
  // cout << "\t\tAlpha min-slope="<<cx_MinSlope<<"
  // max-slope="<<cx_MaxSlope<<"\n";
  //
...
double Hydrogen_chem::Hi_coll_excitation_cooling_rate(double T)
{
  ...
  if (T < cx_minT) {
    ...
    splint(cx_T, cx_rate, cx_spline_id, cx_Nspl, T, &rate);
  }
...
double Hydrogen_chem::Hi_coll_ion_rate(double T)
{
  ...
  if (T < 5.0e3) {
    ...
    // double A=2.91e-8, X=0.232, K=0.39, IP=13.6*1.602e-12; // ionisation
    // potential. T = ion_pot/kB/T; return A*exp(K*log(T) -T)/(X+T);
    //
    // Here we avoid allocating new variables, and just hard-code the
    // equation with its parameters.
    //
    T = 1.578e5 / T;  // Inverse of temperature in units of the ionisation
                      // potential.
...
{
  //
  // This is just the collisional ionisation rate times the ionisation
  // potential, which for hydrogen is 13.6*1.602e-12 ergs.  Answer is returned
  // in erg.cm3/s
  //
  T = 1.578e5 / T;
...
void Hydrogen_chem::Hi_coll_ion_rates(double T, double *cir, double *cicr)
{
  ...
  //
  T = 1.578e5
      / T;  // Inverse of temperature in units of the ionisation potential.
