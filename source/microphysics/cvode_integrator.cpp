...
cvode_solver::cvode_solver()
{
  ...
  n_xd = -1; // number of extra paramaters needed in ydot/jacobian functions (maybe none).
              ...
              )
{
  ...
  //
  // integrate one timestep, returning answer to temp array Y_Output, and new time to 
  ...
  //
  // If we get to here then the loop failed, so we reset the time and shorten dt.
  ...
  do {
    ...
    if (err!=CV_SUCCESS) {
      ...
      //
#ifdef CVODE_DEBUG
      cout <<"STEPPER: old-t="<<t_now<<", returned t="<<t_temp<<", dt was = "<<dt;
      cout <<".\t This_loop_fail_ct="<<fail_ct<<", step_ct="<<step_ct<<": yout = [";
      for (int v=0;v<n_eq-1;v++) cout << NV_Ith_S(Y_Output,v)<<", ";
      cout << NV_Ith_S(Y_Output,n_eq-1) <<"]\n";
      ...
      for (int v=0;v<n_eq;v++) NV_Ith_S(Y_Input,v) = NV_Ith_S(Y_Output,v);
      t_now = t_temp;
      dt *= 1.5;
      dt = min(dt,tf-t_now);
      //
  ...
  if (fail_ct>MAX_TRYS) {
    cout <<"integrate_cvode_step(): Integration failed after bisecting the step "<<MAX_TRYS<<"times.\n";
    return fail_ct;
  }
  if (step_ct>=MAX_STEPS) {
    cout <<"integrate_cvode_step(): Integration took "<<MAX_STEPS<<" and not at end of step.  giving up.\n";
    return step_ct;
  }
