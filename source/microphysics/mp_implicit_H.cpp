...
#ifdef MPV3_DEBUG
    cout <<" INT(exp(-dtau),dt) = "<<edtau<<" <deltau>="<<deltau<<", resetting\n";
#endif // MPV3_DEBUG
...
#endif
    tmin = min(tmin, 
      DTFRAC*NV_Ith_S(y_in,lv_eint)/(fabs(NV_Ith_S(y_out,lv_eint))+TINYVALUE) );
  }
...
#else 
    double rate = Hii_rad_recomb_rate(T)*(1.0-NV_Ith_S(y_in,lv_H0))*mpv_nH*JM_NELEC;
#endif
