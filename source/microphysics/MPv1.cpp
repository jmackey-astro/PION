...
#endif
{
...
#ifdef HUMMER_RECOMB
    //
    // Use Hummer (1994) recombination rates and recombination energy loss
    // rates. These are given as tables, so we need to fit cubic splines to the
    // data.
    //
    ...
    double t = 10.0;
    do {
        outf << t << "\t" << rad_recomb_rate(t) * sqrt(t) << "\t"
             << rad_recomb_energy(t) * sqrt(t);
        outf << "\t" << rad_recomb_rate(t) << "\t" << rad_recomb_energy(t)
             << "\t";
        outf << 3.41202e-10 * exp(-0.782991 * log(t)) << "\t"
             << 3.41202e-10 * exp(-0.782991 * log(t)) * kB * t / (2. / 3.)
             << "\n";
        t *= 1.03;
    } while (t < 1.e7);
    outf.close();
...
)
{
    ...
    double t = (gamma - 1.) * P[lv_eint] / kB / (1.0 + P[lv_Hp])
               / P[lv_nh];  // Temperature.
#ifdef ISOTHERMAL_MP
    // **************************************************
    // ************** BIG ISOTHERMAL HACK!!!! ***********
...
)
{
    ...
    if (T > 0)
        p_local[lv_eint] =
            (1.0 + p_local[lv_Hp]) * p_local[lv_nh] * kB * T / (gamma - 1.);
    else
        T = (gamma - 1.) * p_local[lv_eint] / kB / (1.0 + p_local[lv_Hp])
            / p_local[lv_nh];  // Temperature.
    //  cout <<"Temp = "<<T<<"\n";
    ...
    const pion_flt* p_in, double* p_local, const double gam)
{
    ...
    // ************** BIG ISOTHERMAL HACK!!!! ***********
    // **************************************************
    p_local[lv_eint] = (1.0 + p_local[lv_Hp]) * p_local[lv_nh] * kB
    ...
    const double gam)
{
    ...
    //
    // Check for nans/infs (easier to do this from calling function who has cell
    // pointer).
    //
    // for (int v=0;v<nv_prim;v++) {
    ...
    double* ttt)
{
    ...
    double
        P[nvl];  // local state vector for microphysics = [n_h,Eint,x_Hp,d_tau]
    ...
    if (sw_int == 0) {
        ...
        err = Integrator_Base::Int_Adaptive_RKCK(
            nvl, P, 0.0, dt, errtol, P, &tout);
    }
    else if (sw_int == 1) {
        // errtol = 1.0e-3;
        err = Integrator_Base::Int_DumbAdaptive_Euler(
            nvl, P, 0.0, dt, errtol, P, &tout);
    }
    else if (sw_int == 2) {
        // DANGEROUS!!!
...
#define CHECK_IFRAC
    ...
    if (P[lv_Hp] > 2.0) {
        ...
        int nstep = 10;
        err += convert_prim2local(
            p_in, P, gamma);  // reset P to the initial values.
    ...
    *ttt = (gamma - 1.) * P[lv_eint] / kB / (1.0 + P[lv_Hp])
           / P[lv_nh];  // Temperature.
    // if (*ttt >1.e3)
...
)
{
    ...
    FUV_extinction =
        1.086 * 5.0e-22 * tau2cell
        / phot_xsection(1.0);  // Extinction from Henney et al. 2009.
    ...
    double T = (gamma - 1.) * P[lv_eint] / kB / (1.0 + P[lv_Hp])
               / P[lv_nh];  // Temperature.
    double irate = P[lv_nh] * (1.0 - P[lv_Hp]) * phot_xsection(T)
                   * path_length;  // Tau through cell
    ...
    // will have to try the explicit/implicit method.  I set the dividing line
    // at irate=20.
    //
    ...
    if (irate < 20.0) {
        // ionisation rate is not super strong, so do regular explicit
        // integration.
        double t_out = 0.0;
        if (sw_int == 0) {
            err += Integrator_Base::Int_Adaptive_RKCK(
        ...
        if (P2[lv_Hp] > 1.0) {
            ...
            for (int i = 0; i < nvl; i++)
                P[i] = P2[i];
            // cout <<"Integration didn't go for full timestep.  Too much
            // work!\n"; cout <<"Will try more careful method to get the rest of
            // the way.\n"; cout <<"integrator gave code: err="<<err<<";
            // t="<<t_now<<" and dt="<<t_final<<"\n"; cout <<"irate =
            // "<<irate<<"\n";
        }
        else {  // step worked.
            if (err) {
                cout << "integrator gave code: err=" << err << "; t=" << t_out
                     << " and dt=" << t_final << "\n";
                rep.error(
                    "Integration failed, but I didn't catch why! Fix me.", err);
            }
            for (int i = 0; i < nvl; i++)
                P[i] = P2[i];
            t_now += hh;
    ...
    while (P[lv_Hp] < i_crit && !pconst.equalD(t_now, t_final)) {
        ...
        // cout <<"strongly driven: irate="<<irate<<" and x/xdot="<<hh<<" with
        // dt="<<dt<<"\n"; rep.printVec("P",P,nvl); rep.printVec("R",R,nvl);
        ...
        // cout <<"\tDoing Explicit Step: x="<<P[lv_Hp]<<", t_now="<<t_now<<"
        // and hh="<<hh<<" and t_final="<<t_final<<"\n";
        double t_out = 0.0;
        if (sw_int == 0) {
            err += MP_Hydrogen::Int_Adaptive_RKCK(
                nvl, P, 0.0, hh, errtol, P, &t_out);
        }
        else
            rep.error(
                "Only allowed to Adaptive_RKCK Integration here!", sw_int);
    ...
    // put updated state vector into p_out.
    T = (gamma - 1.) * P[lv_eint] / kB / (1.0 + P[lv_Hp])
        / P[lv_nh];  // Temperature.
    // if (T >1.e3)
    ...
    //
    // Convert integrated exp(-tau) into a time-averaged optical depth to return
    // to the code. Have to check that it is finite and positive, since log is
    ...
    *deltau = -log(tau_cell / dt);
    if (isinf(*deltau) || isnan(*deltau)) {
        ...
        *deltau =
            1000.0;  // This should be big enough -- exp(-1000)=10^(-440)!!!
    }
    if (*deltau < 0.0) {
        if (*deltau < -1.e-14)
...
{
    //
    // Do some paranoid checking first -- right number of variables and no
    // NAN/INF in input vector!!!
    //
    // cout <<"\t\t\timplicit step!\n";
    ...
    double x_inf, t_ion, A = 0.0, B = 0.0, C = 0.0, LL = 0, T = 0.0, tau,
                         tau_int, e_int;
    double e_phot, e_ci, e_rr;
    //  static long int count=0;

    //
    // Run a do-loop, where we double the number of sub-steps in each iteration,
    // starting with a single step over the whole timestep. The loop runs at
    // least twice so we can estimate the relative error.
    //
    do {
        //
        ...
        //
        //    T = (gamma-1.)*p_now[lv_eint]/kB/(1.0+p_now[lv_Hp])/p_now[lv_nh];
        //    // Temperature. if (T<1.0) {
        //      cout <<"Implicit_step: fixing very low Temperature="<<T<<" and
        //      setting to 1K.\n";
        ...
        for (int i = 0; i < nsub; i++) {
            // edot=0.0;
            //
            // Set the temperature based on p_now[].  If the temperature is not
            // valid, there is no point continuing the integration, so set
            ...
            if (T < 0.0 || isnan(T) || isinf(T)) {
                ...
                //
                // I have functions for all these energy gains and losses so
                // that the explicit and implicit integration methods boths use
                // the same bits of code!
                //
                e_phot = phot_ion_energy(T);  // positive value.
                e_ci   = coll_ion_energy(T);  // positive value.
#ifdef HUMMER_RECOMB
                e_rr = rad_recomb_energy(T) * p_now[lv_Hp]
                       * p_now[lv_nh];  // erg/s = lambda*n_e (positive value)
#endif                                  // HUMMER_RECOMB
                ...
                if (EP->phot_ionisation) {
                    tau = p_now[lv_nh] * (1.0 - p_now[lv_Hp]) * phot_xsection(T)
                          * path_length;
                    if (tau < 0.01)
                        A = photons_in * phot_xsection(T) * path_length;
                    else
                        A = photons_in * (1.0 - exp(-tau)) / p_now[lv_nh]
                            / (1.0 - p_now[lv_Hp]);
                }
                else
                ...
                if (EP->coll_ionisation) {
                    B = coll_ion_rate(T) * p_now[lv_Hp]
                        * p_now[lv_nh];  // This is A_ci*n_e
                }
                else
                    B = 0.0;

                if (EP->rad_recombination) {
                    C = rad_recomb_rate(T) * p_now[lv_Hp]
                        * p_now[lv_nh];  // This is alpha_rr*n_e
                }
                else
                    C = 0.0;
                ...
                if (EP->cooling) {
                    LL = cool->CoolingRate(
                             T, p_now[lv_Hp], p_now[lv_nh],
                             FUV_unattenuated_flux, FUV_extinction)
                         / p_now[lv_Hp] / p_now[lv_nh];  // This is Lambda*n_e
                ...
                e_int =
                    hh * x_inf
                    - t_ion * (x_inf - p_now[lv_Hp]) * (1.0 - exp(-hh / t_ion));

                // now int(edot,dt) = (A_phot*e_phot -B_ci*E_ci)hh
                // -(A_phot*E_phot -B_ci*E_ci +C_rr*E_rr +Lambda*n_e)int(x,dt)
#ifdef HUMMER_RECOMB
                // here e_rr is a cooling rate, just like LL
                e_int = hh * (A * e_phot - B * e_ci)
                        - e_int * (A * e_phot - B * e_ci + e_rr + LL);
#endif  // HUMMER_RECOMB
#ifndef HUMMER_RECOMB
                // here e_rr is just an energy per recombination, which is not
                // quite right since it is supposed to be inside the integration
                // over the boltzmann distribution in the rate calculation.
                // hummer_recomb is better!
                e_int = hh * (A * e_phot - B * e_ci)
                ...
                // x_inf="<<x_inf<<" photons_in="<<photons_in<<" A="<<A<<"
                // B="<<B<<" C="<<C<<"\n"; cout
                // <<"*** 1st="<<hh*(A*e_phot -B*e_ci)<<" 2nd=("<<hh*x_inf -
                // t_ion*(x_inf-p_now[lv_Hp])*(1.0-exp(-hh/t_ion))<<")*("; cout
                // <<(A*e_phot -B*e_ci +e_rr   +LL)<<")\n";
#endif

                if (EP->update_erg) p_now[lv_eint] += e_int * p_now[lv_nh];

                if (p_now[lv_eint] < 0.) {
                    // cout <<"e_old=
                    // "<<p_now[lv_eint]-e_int*p_now[lv_nh]<<"\tde =
                    // "<<e_int*p_now[lv_nh]<<"\n";
                    p_now[lv_eint] -=
                        HUGEVALUE * nsub;  // make sure integration fails if we
                                           // get negative energy!!!
                }
                ...
                // ************** BIG ISOTHERMAL HACK!!!! ***********
                // **************************************************
                p_now[lv_eint] = p_now[lv_nh] * (1.0 + p_now[lv_Hp])
                                 * pconst.kB()
                                 * (100.0 + 9900.0 * p_now[lv_Hp]);
                // **************************************************
                // ************** BIG ISOTHERMAL HACK!!!! ***********
                // **************************************************
#endif  // ISOTHERMAL_MP
...
#define SIMPLE_TAU
                ...
                // tau =
                // path_length*phot_xsection(T)*p_now[lv_nh]*(1.-p_now[lv_Hp]);
                ...
                tau = path_length * phot_xsection(T) * p_now[lv_nh]
                      * (1. - p_now[lv_Hp]);
                if (tau < 0.01)
                    tau_int = (1.0 - tau);
                else
                ...
                // update ion fraction
                p_now[lv_Hp] =
                    x_inf + (p_now[lv_Hp] - x_inf) * exp(-hh / t_ion);
                // get upper limit and make integral from 0.5*hh*(lower+upper)
                tau = path_length * phot_xsection(T) * p_now[lv_nh]
                      * (1. - p_now[lv_Hp]);
                if (tau < 0.01)
                    tau_int = 0.5 * hh * ((1.0 - tau) + tau_int);
                else
                    tau_int = 0.5 * hh * (exp(-tau) + tau_int);
                // add integral to sum.
                p_now[lv_dtau] += tau_int;
#endif
#ifndef SIMPLE_TAU
                // new tau int... more accurate, but takes much longer to run
                // with all the exp() calls, so it is better to use the simple
                // guesstimate above.
                double xx[5], tau0, tau1;
                xx[0] = 0.0;
                if (t_ion < hh / 10.0) {
                    xx[1] = t_ion / 100.;
                    xx[2] = t_ion / 10.;
                ...
                tau_int = 0.0;
                tau0 =
                    exp(-path_length * phot_xsection(T) * p_now[lv_nh]
                        * (1. - p_now[lv_Hp]));
                for (int ii = 1; ii < 5; ii++) {
                    p_now[lv_Hp] =
                        x_inf + (p_now[lv_Hp] - x_inf) * exp(-xx[ii] / t_ion);
                    tau1 =
                        exp(-path_length * phot_xsection(T) * p_now[lv_nh]
                            * (1. - p_now[lv_Hp]));
        ...
        //
        // Now we have done all the substeps in this iteration, so check if the
        // old and new values are similar, and if they satisfy the relative
        // error tolerance.
        //
        maxerr = TINYVALUE;
        for (int v = 0; v < nvl; v++) {
            maxerr = max(
                maxerr, fabs(p_now[v] - p_old[v])
                            / (fabs(p_now[v]) + fabs(p_old[v]) + TINYVALUE));
        ...
        //
        // Increment counter, double no. of substeps, halve the timestep in
        // preparation for the next iteration (if needed).
        //
        ct++;
        nsub *= 2;
        hh /= 2.0;
...
)
{
    ...
    *tf      = t0 + dt;
    ...
    // Note that the stepper now checks for NAN/INF in the temp and output
    // vectors, so should always return a valid vector if it returns without
    // errors.
    //
    do {
        // rep.printVec("adaptive p1",p1,nv);
        err += Stepper_RKCK(nv, p1, t, h, etol, p2, &hdid, &hnext);
        // cout <<"XX: h="<<h<<" hdid="<<hdid<<" hnext="<<hnext<<" told="<<t<<"
        ...
        if (p2[lv_Hp] >= 0.999) {
            ...
            // iter="<<ct<<"\n"; cout <<"ADAPTIVE INT:\t ";
            // rep.printVec("pnew",p1,nvl); cout <<"ADAPTIVE
            // INT:\t\thdid="<<hdid<<" tf-t="<<*tf-t<<"\n"; cout <<"s:
            // "<<sizeof(h)<<"\ttf-t-hdid="<<*tf-t-hdid<<"\th="<<h<<"\n"; cout
            // <<"\t\terr="<<err<<"\tct="<<ct<<"\tp1[lv_Hp]-i_crit="<<p1[lv_Hp]-i_crit<<"\n";
        }
    } while (!pconst.equalD(*tf, t) && (err == 0) && (ct < ctmax)
             && p1[lv_Hp] < i_crit);

    if (err || ct >= ctmax) {
        cerr << "MP_HYDROGEN::Int_Adaptive_RKCK() errors encountered. nstep="
             << ct << "\n";
...
)
{
    ...
    if (EP->coll_ionisation) {
        ...
        R[lv_eint] -=
            temp * coll_ion_energy(T);  // reduces energy by the amount it took
                                        // to ionise ion (i-1)
#ifdef COUNT_ENERGETICS
        if (!have_counted_ergs) {
            GLOBAL_CE->ci_rate = coll_ion_rate(T) * (1.0 - P[lv_Hp]) * P[lv_Hp]
                                 * P[lv_nh] * P[lv_nh];
    ...
    if (EP->rad_recombination) {
        temp =
            rad_recomb_rate(T) * P[lv_Hp] * P[lv_Hp] * P[lv_nh];  // rate [1/s]
        ...
        if (!have_counted_ergs) {
            GLOBAL_CE->rr_rate =
                rad_recomb_rate(T) * P[lv_Hp] * P[lv_Hp] * P[lv_nh] * P[lv_nh];
            GLOBAL_CE->rr_cooling = rad_recomb_energy(T) * P[lv_Hp] * P[lv_Hp]
                                    * P[lv_nh] * P[lv_nh];
        }
#endif  // COUNT_ENERGETICS
    }
    ...
    if (EP->phot_ionisation) {
        R[lv_dtau] = P[lv_nh] * (1.0 - P[lv_Hp]) * phot_xsection(T)
                     * path_length;  // this is optical depth through cell.
        if (R[lv_dtau] > 0.01)
            temp = photons_in * (1.0 - exp(-R[lv_dtau])) / P[lv_nh]
                   / (1.0 - P[lv_Hp]);
        else
            temp = photons_in * phot_xsection(T)
                   * path_length;  // approx for 1-x<<1
        ...
        if (!have_counted_ergs) {
            ...
            GLOBAL_CE->pi_heating =
                temp * (1.0 - P[lv_Hp]) * P[lv_nh] * phot_ion_energy(T);
        }
#endif  // COUNT_ENERGETICS
    }
    ...
    if (!have_counted_ergs) {
        ...
        GLOBAL_CE->tot_cooling = GLOBAL_CE->rr_cooling + GLOBAL_CE->ci_cooling
                                 + GLOBAL_CE->fn_cooling;
        GLOBAL_CE->net_heating =
            GLOBAL_CE->tot_heating - GLOBAL_CE->tot_cooling;
        // cout <<"calculated heating="<<GLOBAL_CE->net_heating<<" and from
        // dpdt()="<<R[lv_eint]<<"\n";
        GLOBAL_CE->cooling_time =
            P[lv_eint] / GLOBAL_CE->tot_cooling / pconst.s_per_yr();
        GLOBAL_CE->recomb_time =
            P[lv_Hp] / GLOBAL_CE->rr_rate / pconst.s_per_yr();
    }
#endif  // COUNT_ENERGETICS
    ...
    // This is commonly used in Astrophysics, e.g. Williams, Ward-Thompson,
    // Whitworth (2001)
    //
    // Since gamma=1 I'm leaving out the (g-1) factor everywhere in the internal
    // energy calcs.
    //
    // cout <<"isothermal rate!!!\n";
    R[lv_eint] =
        (P[lv_nh] * pconst.kB()) * (1.0e4 + 1.98e4 * P[lv_Hp]) * R[lv_Hp];
    // **************************************************
    // ************** BIG ISOTHERMAL HACK!!!! ***********
    // **************************************************
#endif  // ISOTHERMAL_MP
...
)
{
    ...
    //  cout <<"T="<<T<<" K,";
    T = ion_pot / kB / T;
    //  cout <<"\tPsi/kT="<<T<<" and CIR="<<A*(1.+PP*sqrt(T))*exp(K*log(T)
    //  -T)/(X+T)<<" cm^3/s\n";
...
)
{
    ...
    double T = (gam - 1.) * P[lv_eint] / kB / (1.0 + P[lv_Hp])
               / P[lv_nh];  // Temperature.
    ...
    if ((EP->cooling) && (f_cool) && (T >= 10.0)) {
        ...
        if (EP->rad_recombination)
            rate += rad_recomb_energy(T) * P[lv_Hp] * P[lv_Hp] * P[lv_nh]
                    * P[lv_nh];
        //
