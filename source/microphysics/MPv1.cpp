...
)
{
  ...
  double x_inf, t_ion, A = 0.0, B = 0.0, C = 0.0, LL = 0, T = 0.0, tau, tau_int,
                       e_int;
  ...
  do {
    ...
    for (int i = 0; i < nsub; i++) {
      ...
      if (T < 0.0 || isnan(T) || isinf(T)) {
        ...
        if (EP->cooling) {
          LL = cool->CoolingRate(
                   T, p_now[lv_Hp], p_now[lv_nh], FUV_unattenuated_flux,
                   FUV_extinction)
        ...
        if (p_now[lv_eint] < 0.) {
          ...
          // "<<e_int*p_now[lv_nh]<<"\n";
          p_now[lv_eint] -= HUGEVALUE * nsub;  // make sure integration fails if
                                               // we get negative energy!!!
        }
        ...
        // ************** BIG ISOTHERMAL HACK!!!! ***********
        // **************************************************
        p_now[lv_eint] = p_now[lv_nh] * (1.0 + p_now[lv_Hp]) * pconst.kB()
...
#define SIMPLE_TAU
    ...
    for (int v = 0; v < nvl; v++) {
      maxerr =
          max(maxerr, fabs(p_now[v] - p_old[v])
...
)
{
  ...
  if (EP->coll_ionisation) {
    ...
    R[lv_eint] -= temp * coll_ion_energy(T);  // reduces energy by the amount it
                                              // took to ionise ion (i-1)
#ifdef COUNT_ENERGETICS
    if (!have_counted_ergs) {
      GLOBAL_CE->ci_rate =
          coll_ion_rate(T) * (1.0 - P[lv_Hp]) * P[lv_Hp] * P[lv_nh] * P[lv_nh];
  ...
  if (EP->rad_recombination) {
    ...
    if (!have_counted_ergs) {
      ...
      GLOBAL_CE->rr_cooling =
          rad_recomb_energy(T) * P[lv_Hp] * P[lv_Hp] * P[lv_nh] * P[lv_nh];
    }
#endif  // COUNT_ENERGETICS
  }
  ...
  if (EP->phot_ionisation) {
    ...
    if (R[lv_dtau] > 0.01)
      temp =
          photons_in * (1.0 - exp(-R[lv_dtau])) / P[lv_nh] / (1.0 - P[lv_Hp]);
    else
  ...
  if (!have_counted_ergs) {
    ...
    GLOBAL_CE->tot_cooling =
        GLOBAL_CE->rr_cooling + GLOBAL_CE->ci_cooling + GLOBAL_CE->fn_cooling;
