...
CoolingFn::CoolingFn(int flag)
{
    ...
    if (flag == 1) {
        ...
        double temp1[69] = {
            7.500, 7.445, 7.396, 7.347, 7.297, 7.246, 7.195, 7.143, 7.091,
            7.039, 6.987, 6.934, 6.882, 6.829, 6.776, 6.723, 6.670, 6.617,
            6.563, 6.510, 6.457, 6.403, 6.350, 6.296, 6.243, 6.189, 6.136,
            6.082, 6.028, 5.975, 5.921, 5.868, 5.814, 5.760, 5.707, 5.654,
            5.600, 5.547, 5.493, 5.440, 5.389, 5.336, 5.283, 5.230, 5.176,
            5.122, 5.069, 5.015, 4.961, 4.907, 4.854, 4.800, 4.746, 4.694,
            4.647, 4.604, 4.546, 4.495, 4.450, 4.409, 4.361, 4.318, 4.269,
            4.225, 4.185, 4.141, 4.099, 4.053, 4.007};
        double temp2[69] = {
            -22.73, -22.74, -22.74, -22.74, -22.73, -22.72, -22.70, -22.67,
            -22.63, -22.60, -22.58, -22.59, -22.60, -22.62, -22.62, -22.61,
            -22.58, -22.56, -22.53, -22.51, -22.46, -22.39, -22.27, -22.12,
            -22.00, -21.96, -21.96, -21.98, -21.99, -21.99, -21.99, -21.96,
            -21.91, -21.86, -21.82, -21.79, -21.76, -21.70, -21.61, -21.53,
            -21.47, -21.43, -21.41, -21.40, -21.41, -21.43, -21.46, -21.50,
            -21.54, -21.59, -21.65, -21.70, -21.77, -21.83, -21.89, -21.94,
            -22.00, -22.04, -22.09, -22.14, -22.21, -22.29, -22.40, -22.51,
            -22.62, -22.74, -22.86, -22.98, -23.08};
        ...
        cout << "\t\tFlag = " << flag
             << " corresponding to Koyami & Inutsuka (2002, ApJL, 564, L97) "
                "cooling function\n";
        cout
            << "\t\tThis is a double exponential fitting function, using their "
               "equations 4 and 5.\n";
        cout << "\t\tN.B. The KI02 equation had two typos: (1.148e5 instead of "
                "1.184e5, and 14 instead of 1.4e-2)\n";
        cout
            << "\t\tThese were corrected by Vasquez-Semadeni et al. 2007, ApJ, "
               "657, 870.\n";
        MinTemp = 5.0;
        MaxTemp = 5.0e8;
    }  // KI02 function
        ...
        double temp1[31] = {1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4,
        ...
        double temp1[91] = {
            ...
            4.55, 4.60, 4.65, 4.70, 4.75, 4.80, 4.85, 4.90, 4.95, 5.00, 5.05,
            5.10, 5.15, 5.20, 5.25, 5.30, 5.35, 5.40, 5.45, 5.50, 5.55, 5.60,
            5.65, 5.70, 5.75, 5.80, 5.85, 5.90, 5.95, 6.00, 6.05, 6.10, 6.15,
            6.20, 6.25, 6.30, 6.35, 6.40, 6.45, 6.50, 6.55, 6.60, 6.65, 6.70,
            6.75, 6.80, 6.85, 6.90, 6.95, 7.00, 7.05, 7.10, 7.15, 7.20, 7.25,
            7.30, 7.35, 7.40, 7.45, 7.50, 7.55, 7.60, 7.65, 7.70, 7.75, 7.80,
            7.85, 7.90, 7.95, 8.00, 8.05, 8.10, 8.15, 8.20, 8.25, 8.30, 8.35,
            8.40, 8.45, 8.50};
        double temp2[91] = {
            -23.06, -22.46, -22.17, -21.92, -21.79, -21.80, -21.86, -21.90,
            -21.88, -21.82, -21.73, -21.63, -21.53, -21.42, -21.32, -21.22,
            -21.14, -21.07, -21.01, -20.98, -20.99, -21.02, -21.03, -21.01,
            -20.98, -20.97, -20.96, -20.96, -20.99, -21.13, -21.35, -21.55,
            -21.66, -21.71, -21.71, -21.71, -21.76, -21.86, -21.93, -21.95,
            -21.96, -21.96, -21.96, -21.95, -21.94, -21.97, -22.07, -22.20,
            -22.31, -22.39, -22.44, -22.48, -22.50, -22.53, -22.56, -22.59,
            -22.60, -22.60, -22.59, -22.57, -22.57, -22.59, -22.62, -22.65,
            -22.68, -22.70, -22.72, -22.73, -22.73, -22.73, -22.73, -22.72,
            -22.71, -22.70, -22.68, -22.67, -22.65, -22.64, -22.62, -22.60,
            -22.58, -22.56, -22.54, -22.53, -22.51, -22.49, -22.47, -22.45,
            -22.43, -22.40, -22.38};
        for (int i = 0; i < Nspl; i++) {
        ...
        // HI
        cout
            << "\t\tC2RAY-like cooling due to free-free and HI coll.excitation.\n";
        cout << "\t\t the recombination and ionisation cooling are in the "
                "microphysics routine.\n";
        cout
            << "\t\t I approx the ff cooling with 1.34e-11*sqrt(T_e)*ne*ni, good "
               "for 10^3<T<10^4 or so.\n";
        cout
            << "\t\t I ignore inverse compton cooling off the cmb, since i'm at "
               "z=0.\n";
        cout
            << "\t\t I use 7.5e-19*exp(-118348/T)/(1+sqrt(T/1.e5))*n_e*n_H0 as "
    ...
    const double FUV_extinction)
{
    ...
    if (WhichFunction == 1 || WhichFunction == 4) {
        if (T > MaxTemp) {
            ...
            rate =
                max(1.e-50, Lamb[0] * exp(MinSlope * (log(T) - log(MinTemp))));
#endif
        }
        else {
        ...
        if (T > MaxTemp) {
            ...
            cout << "Returning Lambda(MaxTemp) = Lambda(" << MaxTemp << ")\n";
            interpolate.splint(
                Temp, Lamb, spline_id, Nspl, log10(MaxTemp), &rate);
        ...
        //
        rate += (1.0 - xHp) * (1.0 - xHp) * 3.981e-27 * exp(1.6 * log(nH))
                * sqrt(T)
