...
///
/// - For collisional ionisation the function of Voronov (1997,ADNDT,65,1) is
/// used.
///
/// - Collisional excitation of neutral H, table from Raga, Mellema, & Lundqvist
///   (1997,ApJS,109,517), using data from Aggarwal (1993).
///
/// - For cooling due to heavy elements, which are not explicitly included, we
/// use the CIE cooling curve of Sutherland & Dopita (1993,ApJS,88,253), but
/// this time I subtract the zero-metals curve from the solar-metallicity curve
/// so that I only have cooling due to metals.  This eliminates the potential
/// for double-counting which was in my previous cooling function.
///
/// - Then I use various formulae from Henney et al. (2009,MN,398,157) for
/// cooling due to collisional excitation of photoionised O,C,N (eq.A9),
/// collisional excitation of neutral metals (eq.A10), and the Wiersma+2009 CIE
/// metals-only curve.  I take the max of the WSS09 curve and the Henney et al.
/// functions, to avoid double counting.  For neutral gas I can use the cooling
/// curve of Henney et al 2009 eq.A14.
///
/// - Photoheating from ionisation is discussed above.  Cosmic ray heating will
/// use a standard value, X-ray heating is ignored.  UV heating due to the
/// interstellar radiation field (ISRF) is according to the optical depth from
/// the edge of the domain to the cell in question, using e.g. HEA09 eq.A3, if
/// requested.  UV heating from the star uses the same equation, but with the
/// optical depth from the source (using a total H-nucleon column density).
///
///
/// The integration method uses the CVODES solver from the SUNDIALS package by
/// (Cohen, S. D., & Hindmarsh, A. C. 1996, Computers in Physics, 10, 138)
/// available from https://computation.llnl.gov/casc/sundials/main.html The
/// method is backwards differencing (i.e. implicit) with Newton iteration.
///
/// Electrons, ions, ion fraction:  This module makes the (crude) approximation
/// that He is an identical atom to H -- that it is only ever singly ionised and
/// its ionisation and recombination rates are identical to H.  But because most
/// photons are below the ionisation potential of He0 the opacity of He0 atoms
/// is not counted in the optical depth of the medium to ionising photons.  This
/// is not quite self-consistent, but it is a better approximation than assuming
/// He absorbs 14eV photons.
///
/// Modifications:
...
/// - 2011.04.14 JM: Fixed bugs; I am testing the non-RT part now.  Looks good
/// so far.
/// - 2011.04.15 JM: Bugfixes
/// - 2011.04.17 JM: Added ifdefs for RT_TESTING to omit various processes in
/// dYdt().
///     Debugging.  Added ifdef to integrate neutral fraction instead of ion
///     fraction.
/// - 2011.04.18 JM: Bugfixes.
/// - 2011.04.22 JM: Debugged UV/IR heating.  Disabled X-ray heating.
/// - 2011.05.02 JM: Updated convert_prim2local to correct negative pressure/ion
/// fraction
...
/// - 2011.05.06 JM: Set cooling limiting as T approaches EP->MinTemperature so
/// we
...
/// - 2011.05.25 JM: Fixed a bad bug -- UV heating was not kicking in unless
/// there was a
///    diffuse sources, so the pt-src UV flux was useless up to now.
///    Got rid of "int-ion-frac" dydt function.  Too confusing to have two
///    functions. If for some reason I want to integrate the ion fraction in
///    future I can go back to an older revision.
/// - 2011.06.21 JM: Fixed bug in UV heating where Tau(FUV) was incorrectly
/// calculated
///    Simplified timestep-limit calculation for xdot, since 2nd order
///    integration is more accurate and so requires less tuning and allows
///    larger timesteps.
/// - 2011.07.03 JM: Changed how Helium is treated appproximately (it is not
/// ignored in
///    the photoionisation optical depths, which is a better approx. than
///    including it).
/// - 2011.07.12 JM: Fixed the timescales function to have a less restrictive
/// timestep
///    limitation based only on dt=0.15/|xdot|.
/// - 2011.09.21 JM: Set the cooling to be C2 (cooling=15 in cooling.cc) for
/// RT_TEST_PROBS
/// - 2011.10.13 JM: Added switch in TimeUpdate_RTnew() so that if nothing is
/// changing
...
///       which is the 'proto-solar nebula' value. The photospheric value is
///       lower 4.9e-4, and that is used by Wiersma et al. (2009,MN,393,99).
///
...
#define JM_NELEC                                                               \
    1.1  ///< ionised gas has 1.1 electrons per hydrogen nucleon (H+,He+, no
         ///< He2+!)
#define JM_NION                                                                \
    1.1  ///< There are 1.1 nucleons per H nucleon (10% He by number).
#define JM_RELTOL                                                              \
    1.0e-2  ///< relative-error tolerance is set to 0.01 of this value.
    ...
    for (int isrc = 0; isrc < RS->Nsources; isrc++) {
        if (RS->sources[isrc].effect == RT_EFFECT_PION_MONO) {
            ...
            cout << "mp_rates_ExpH_ImpMetals::set_gamma_and_srcs: Nisrc=" << ion
                 << ", got=" << got << ": ";
            cout << " found mono-p-ion src id=" << isrc
                 << " with NIdot=" << NIdot << "\n";
        }
            ...
            cout << "mp_rates_ExpH_ImpMetals::set_gamma_and_srcs: Nisrc=" << ion
                 << ", got=" << got << ": ";
            cout << " found multifreq-p-ion src id=" << isrc
                 << " with L=" << NIdot << "\n";
        }
    }
    if (ion > 0) {
...
)
{
    ...
    // contribute to the opacity. BUT KEEP IN MIND THIS IS AN APPROXIMATION!
    //
    NH0  = P[2];
    dNH0 = P[3];
    //
    ...
    // proportional to total column density to cell from all sided of the
    // domain.
    //
    // Av_diffuse = P[6];
    //
...
//#ifdef RT_TESTING
#ifdef MPV2_DEBUG
    cout << "MPR set params.  nH=" << nH << ", Vsh=" << Vshell
         << ", NH0=" << NH0 << ", dNH0=";
...
)
{
    ...
    // cout <<"mp_rates_ExpH_ImpMetals::get_temperature(): n.K="<<T<<",
    // T="<<gamma_minus_one*E/T<<"\n";
    ...
    double* Edot)
{
    ...
    //  1-x
    //
    // double OneMinusX = max(1.0-x_in, 1.0e-14); // fixes min-neutral-fraction
    // to 1e-14
    ...
    *Edot = 0.0;  // Edot is calculated in units of erg/s per H nucleon,
                  // multiplied by nH at the end.
    ...
    if (ion) {
        switch (ion_src_type) {
            case RT_EFFECT_MFION:
                ...
                // multiply by (1-x) to get xdot, we simply divide by n(H) since
                // this is more numerically stable.  To do this, n(H) is passed
                // to the rate function instead of n(H0).
                //
                // Also, instead of using dNH0 for the cell optical depth, we
                // use the current optical depth (nH*delta_S*OneMinusX) so that
                // we allow the photoionisation rate to decrease as the number
                // of neutral atoms decreases during the timestep. This is more
                // stable.
                //
                *xdot -= Hi_discrete_multifreq_photoion_rate(
...
#define EXCESS_ENERGY 8.01e-12
                temp1 = Hi_discrete_mono_photoion_rate(
                            NH0, nH * delta_S * OneMinusX, nH * OneMinusX,
                            NIdot, PHOTON_ENERGY, delta_S, Vshell)
                        * OneMinusX;
                *xdot -= temp1;
                *Edot += temp1 * EXCESS_ENERGY;
#ifdef MPV2_DEBUG
                cout << " PIR: NIdot=" << NIdot << " nH=" << nH
                     << " NH0=" << NH0;
                cout << " dNH0=" << dNH0 << " ds=" << delta_S
                     << " Vsh=" << Vshell;
    ...
    *Edot -= Hi_coll_excitation_cooling_rate(T) * OneMinusX * ne;
    ...
    if (diff) {
        ...
        // The quantity G0_UV is as defined in Henney et al. (2009) Appendix A1,
        // Eq.A3, and is set in set_parameters_for_current step()
        //
    ...
    *Edot += 5.0e-28;
    ...
    // This is collisionally excited lines of photoionised metals. (HAdCM09
    // eq.A9) I have exponentially damped this at high temperatures! This was
    // important! Oxygen abundance set to 5.81e-4 from Lodders et al.
    // (2003,ApJ,591,1220,Tab.2).
    //
    temp1 = 1.69e-22 * exp(-33610.0 / T - (2180.0 * 2180.0 / T / T)) * x_in * ne
            * exp(-T * T / 5.0e10);
#ifdef MPV2_DEBUG
    cout << "  CExMi="
         << -1.69e-22 * exp(-33610.0 / T - (2180.0 * 2180.0 / T / T)) * x_in
                * ne * exp(-T * T / 5.0e10);
#endif  // MPV2_DEBUG
    //
    ...
    // Oxygen abundance set to 5.81e-4 from Lodders et al.
    // (2003,ApJ,591,1220,Tab.2).
    //
    temp1 += 2.60e-23 * exp(-28390.0 / T - (1780.0 * 1780.0 / T / T)) * ne
             * OneMinusX;
#ifdef MPV2_DEBUG
    cout << "  CExMn="
    ...
    *Edot -= max(temp1, temp2);
    ...
    //
    // Finally "molecular" or PDR cooling from (HAdCM09 eq.A14), scaling with
    // rho^{1.6}.
    // TODO: Query whether this should be applied for all ion fractions, or only
    // for
    ...
    *Edot -= cooling_rate_SD93CIE(T) * x_in * x_in * nH;
    ...
    //
    temp1 =
        max(0.0, (1.0 + x_in) * (1.0 - x_in) * (1.0 - x_in) * k_B * (T - 100.0)
                     / (SimPM.gamma - 1.0) / 3.16e11 * exp(-T / 1.e4));
    *Edot -= temp1;
    ...
    *Edot *= nH;
    ...
    if (*Edot < 0.0 && T < 2.0 * EP->MinTemperature) {
        ...
        *Edot =
            min(0.0, (*Edot) * (T - EP->MinTemperature) / EP->MinTemperature);
...
//
// This has to be defined somewhere, so may as well make it as local as
// possible.
//
    ...
    coord_sys(csys)
{
    ...
    //
    // Also set gamma for MPR, and flags for whether we have diffuse and
    ...
    // ----------------------- DIFFUSE RADIATION -------------------------------
    // Set up solid angles for diffuse radiation, whether or not they are
    // needed.
    //
    diff_angle.clear();
        ...
        for (int isrc = 0; isrc < RS->Nsources; isrc++) {
            if (RS->sources[isrc].type == RT_SRC_SINGLE) {
                if (RS->sources[isrc].effect == RT_EFFECT_MFION) {
                    int err =
                        set_multifreq_source_properties(&RS->sources[isrc]);
                    if (err)
                        rep.error(
                            "multifreq photoionisation setup failed in MPv2 const.",
                            err);
                }
            }
        }
...
)
{
    ...
    int err = TimeUpdateMP_RTnew(
        p_in, 0, temp, 0, temp, p_out, dt, 0, 0, random_stuff);
    return err;
}
...
)
{
    ...
    setup_radiation_source_parameters(
        p_in, P, N_heat, heat_src, N_ion, ion_src);

    //
    // update radiation source properties, if needed (re-calculate
    // multi-frequency photoionisation rates if the source properties have
    // changed).
    ...
    if (err)
        rep.error(
            "dYdt() returned an error in MPv2::TimeUpdateMP_RTnew()", err);
    for (int v = 0; v < nvl; v++) {
...
)
{
    ...
    if (pv[RO] <= 0.0 || pv[PG] <= 0.0) {
        cout << "MPv2::Temperature() negative rho=" << pv[RO]
             << " or p=" << pv[PG] << "\n";
        return -1.0e99;
    }
    double P[nvl];
    convert_prim2local(pv, P);
    return MPR.get_temperature(
        p_in[RO] / mean_mass_per_H, P[lv_eint], P[lv_Hp]);
}
...
)
{
    ...
    // there is no way to set a temperature, but if p<0 we can just overwrite
    // it.
    //
    if (p[PG] <= 0.0) {
    ...
    P[lv_eint] =
        (JM_NION + JM_NELEC * P[lv_Hp]) * lv_nH * k_B * T / (gamma_minus_one);
...
)
{
    ...
    if (T > 1.01 * EP->MaxTemperature) {
        ...
        // cout <<"T="<<T<<", obtained from nH="<<lv_nH<<",
        // eint="<<p_local[lv_eint]<<", x="<<p_out[pv_Hp]; cout <<"...  limiting
        // to T="<<EP->MaxTemperature<<"\n";
...
)
{
    ...
    double t = HUGEVALUE;
    //
    // First get the ionisation timescale.  Since the timestep obtained will
    // eventually be multiplied by 0.3, this is effectively dt = 0.25/|xdot|.
    ...
    // ionisation fronts (although it may be needed for cooling!).
    //
    t =
        min(t,
...
)
{
    ...
    //
    // first if we have an ionising source, we get the path length ds, from
    // (x,nH), and source.DelCol = rho*ds*(1-x)
    //
    double delta_s = 0.0;
    //
...
#ifndef MP_DEBUG
    // if debugging, we want to run this loop regardless of whether ds is
    // already set.
    ...
    // sigma=5e-22 cm2, and that the input is \int \rho ds, integrated to the
    // front edge of the cell. Diffuse radiation strength is an intensity (i.e.
    // per solid angle), so the total flux going into the solver will be
    // roughly: sum(I(Omega)*delta-Omega)exp(-1.9Av). This comes from Henney et
    // al. (2009) eq. A3
    ...
    for (int v = 0; v < N_heat; v++) {
        if (heat_src[v].type == RT_SRC_DIFFUSE) {
            ...
            cout << ": attenuated flux="
                 << temp * exp(-Av_UV * heat_src[v].Column) << "\n";
#endif  // MP_DEBUG
            ...
            // cout <<" Col="<<heat_src[v].Column<<"
            // Av="<<Av_UV*heat_src[v].Column/1.9<<"\n";
        }
        else {
            //
            // This source must be a point source of UV heating. In this case
            // the strength is the photon luminosity, so flux =
            ...
            IR_diffuse_flux += temp * exp(-Av_IR * heat_src[v].Column);
            // cout
            // <<"UV_ptsc_flux="<<temp*exp(-Av_UV*heat_src[v].Column)<<"\n";
    ...
    //
    // now divide by 1.2e7 to get it normalised correctly for Will's equation
    // A3.
    //
    ...
    if (P[lv_Hp] < 0.99) {
        ...
        cout << ": NH0=" << params[2] << ", dNH0=" << params[3]
             << ", nH=" << lv_nH << "\n";
    }
#endif
...
int MPv2::setup_cvodes_solver_without_Jacobian()
{
    ...
    int err = 0;
    //
    // Get the number of equations to solve, and number of extra-data elements
    // needed. Should be [2,0].
    //
    n_eq = 0;
    n_xd = 0;
    ...
    // cout <<"reltol="<<reltol<<", atol=["<<atol[0]<<", "<<atol[1]<<",
    // "<<atol[2]<<"]\n";
...
)
{
    ...
    do {
        ...
        //
        // integrate one timestep, returning answer to temp array y_out, and new
        // time to temporary variable t_temp.
        //
        err = CVode(cvode_mem, t_now + dt, y_out, &t_temp, CV_NORMAL);
        if (err != CV_SUCCESS) {
            cout << "error with integrate: err=" << err
                 << ", trying smaller step.\n";
            ...
            //
            // cout <<"EEEEEE old-t="<<t_now<<", returned t="<<t_temp<<", dt was
            // =
            // "<<dt<<"\n"; cout <<"EEEEEE loop ct="<<fail_ct<<": yout = ["; for
            // (int v=0;v<n_eq-1;v++) cout << NV_Ith_S(y_out,v)<<", "; cout <<
            // NV_Ith_S(y_out,n_eq-1) <<"]\n"; cout <<"EEEEEE loop
            // ct="<<fail_ct<<": y = ["; for (int v=0;v<n_eq-1;v++) cout <<
            // NV_Ith_S(y_in,v)<<", "; cout
            // << NV_Ith_S(y_in,n_eq-1) <<"]\n";
            //
            for (int v = 0; v < n_eq; v++)
                NV_Ith_S(y_in, v) = NV_Ith_S(y_out, v);
            t_now = t_temp;
            dt *= 1.5;
            dt = min(dt, tf - t_now);
            //
