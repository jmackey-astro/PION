...
///  - 2009-04-28 Modified interface so function returns volume density cooling rate.
///  - 2010-01-05 JM: Added in low-ionisation-only cooling hack IFDEF. (deffed in global.h)
...
CoolingFn::CoolingFn(int flag)
{
  ...
  if (flag==1) {
	...
	6.457,6.403,6.350,6.296,6.243,6.189,6.136,6.082,6.028,5.975,5.921,
	5.868,5.814,5.760,5.707,5.654,5.600,5.547,5.493,5.440,5.389,5.336,
	5.283,5.230,5.176,5.122,5.069,5.015,4.961,4.907,4.854,4.800,4.746,
	4.694,4.647,4.604,4.546,4.495,4.450,4.409,4.361,4.318,4.269,4.225,
	4.185,4.141,4.099,4.053,4.007};
    double temp2[69] = {-22.73,-22.74,-22.74,-22.74,-22.73,-22.72,-22.70,
	-22.67,-22.63,-22.60,-22.58,-22.59,-22.60,-22.62,-22.62,-22.61,
	-22.58,-22.56,-22.53,-22.51,-22.46,-22.39,-22.27,-22.12,-22.00,
	-21.96,-21.96,-21.98,-21.99,-21.99,-21.99,-21.96,-21.91,-21.86,
	-21.82,-21.79,-21.76,-21.70,-21.61,-21.53,-21.47,-21.43,-21.41,
	-21.40,-21.41,-21.43,-21.46,-21.50,-21.54,-21.59,-21.65,-21.70,
	-21.77,-21.83,-21.89,-21.94,-22.00,-22.04,-22.09,-22.14,-22.21,
	-22.29,-22.40,-22.51,-22.62,-22.74,-22.86,-22.98,-23.08};
    ...
    double temp1[31] = {1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 
			...
			5.2, 5.4, 5.6, 5.8, 6.0,
			6.2, 6.4, 6.6, 6.8, 7.0};
    double temp2[31] = {-27.5, -27.3, -27.1, -26.9, -26.7, -26.5,
			-26.3, -26.1, -25.95, -25.8, -25.7,
			-25.6, -25.55, -25.5, -25.45, -25.4,
			-21.7, -21.5, -21.4, -21.3, -21.2,
			-21.3, -21.4, -21.5, -21.7, -22.3,
			-22.6, -22.7, -22.7, -22.7, -22.7};
    for (int i=0;i<Nspl;i++) {
  ...
  rep.error("Dalgarno and McCray cooling function is not usably coded -- get a better function",999);
    ...
    double temp2[91] = {-23.06,-22.46,-22.17,-21.92,-21.79,-21.80,-21.86,-21.90,-21.88,-21.82,-21.73,
			-21.63,-21.53,-21.42,-21.32,-21.22,-21.14,-21.07,-21.01,-20.98,-20.99,
			-21.02,-21.03,-21.01,-20.98,-20.97,-20.96,-20.96,-20.99,-21.13,-21.35,
			-21.55,-21.66,-21.71,-21.71,-21.71,-21.76,-21.86,-21.93,-21.95,-21.96,
			-21.96,-21.96,-21.95,-21.94,-21.97,-22.07,-22.20,-22.31,-22.39,-22.44,
			-22.48,-22.50,-22.53,-22.56,-22.59,-22.60,-22.60,-22.59,-22.57,-22.57,
			-22.59,-22.62,-22.65,-22.68,-22.70,-22.72,-22.73,-22.73,-22.73,-22.73,
			-22.72,-22.71,-22.70,-22.68,-22.67,-22.65,-22.64,-22.62,-22.60,-22.58,
			-22.56,-22.54,-22.53,-22.51,-22.49,-22.47,-22.45,-22.43,-22.40,-22.38};
    for (int i=0;i<Nspl;i++) {
    ...
    // C2Ray -- free-free cooling and collisional excitation cooling due to HI
    cout <<"\t\tC2RAY-like cooling due to free-free and HI coll.excitation.\n";
    cout <<"\t\t the recombination and ionisation cooling are in the microphysics routine.\n";
    cout <<"\t\t I approx the ff cooling with 1.34e-11*sqrt(T_e)*ne*ni, good for 10^3<T<10^4 or so.\n";
    cout <<"\t\t I ignore inverse compton cooling off the cmb, since i'm at z=0.\n";
    ...
    case 6: MinSlope=0.5; break;
    case 7: MinSlope=1.0; break;
    case 8: MinSlope=2.5; break;
    case 9: MinSlope=5.0; break;
    ...
    cout <<"\t\tSD93-CIE function with power law contribution to account for\n";
    cout <<"\t\tforbidden line cooling in ionised gas.\n";
    if (flag==12) cout <<"\t\tflag=="<<flag<<": Plus Exponential Cooling in neutral gas (TOY MODEL n_H*t_c=1e6yrs)\n";
    if (flag==13) cout <<"\t\tflag=="<<flag<<": Plus Exponential Cooling in neutral gas (TOY MODEL n_H*t_c=1e7yrs)\n";
    if (flag==14) cout <<"\t\tflag=="<<flag<<": Plus Exponential Cooling in neutral gas (TOY MODEL t_c=1e3yrs)\n";
  ...
  outf <<"Cooling Curve Data: Temperature(K) Rate(erg/cm^3/s) (n=1 per cc) ifracs: 1e-6, 1e-3, 0.1, 0.5, 0.99, 0.999999, and then the same for n=1e6/cc\n";
  outf.setf( ios_base::scientific );
  outf.precision(6);
  double t=1.e0;
  do {
    outf << t <<"\t"<< fabs(CoolingRate(t,1.e-6,1.0,0,0)) <<"\t" << fabs(CoolingRate(t,0.001,1.0,0,0));
    outf <<"\t"<< fabs(CoolingRate(t,0.1,1.0,0,0)) <<"\t" << fabs(CoolingRate(t,0.5,1.0,0,0));
    outf <<"\t"<< fabs(CoolingRate(t,0.99,1.0,0,0))<<"\t"<<fabs(CoolingRate(t,0.999999,1.0,0,0)); //<<"\n";

    outf <<"\t"<< fabs(CoolingRate(t,1.e-6,1.e6,0,0)) <<"\t"<< fabs(CoolingRate(t,0.001,1.e6,0,0));
    outf <<"\t"<< fabs(CoolingRate(t,0.1,1.e6,0,0))   <<"\t"<< fabs(CoolingRate(t,0.5,1.e6,0,0))  ;
    outf <<"\t"<< fabs(CoolingRate(t,0.99,1.e6,0,0))   <<"\t"<< fabs(CoolingRate(t,0.999999,1.e6,0,0)) <<"\n";
    t *=1.05;
  } while (t<1.e7);
  outf.close();  
#endif // COOL_TESTING
			      ...
			      )
{
  ...
  double rate=0.0;
  //
  // All these rates were originally coded for returning erg.cm^3/s, so I need to
  // Multiply them by n_e.n_i = (xHp.nH)^2 to get the volume density rate.
  //
...
#ifdef LINSLP
      //      cout <<"T="<<T<<", MinTemp="<<MinTemp<<", rate = "<<max(0.0, Lamb[0]+MinSlope*(T-MinTemp))<<"\n";
      rate = max(0.0, Lamb[0]+MinSlope*(T-MinTemp));
#endif
#ifdef LOGSLP
//      cout <<"T="<<T<<", MinTemp="<<MinTemp<<", fraction = "<<exp(MinSlope*(log(T)-log(MinTemp)))<<"\n";
    ...
    rate *= xHp*xHp*nH*nH;
     ...
     //
     if (T>MinTemp)
       rate += nH*nH* (2.0e-19*exp(-1.184e5/(T+1.0e3)) +2.8e-28*sqrt(T)*exp(-92.0/T));
     //
    ...
    cout <<"DALGARNO AND MCCRAY FUNCTION IS VERY DODGY -- BAD INTERPOLATION.\n";
    if (T>MaxTemp) {
      cout <<"Temp out of range!! Too large: T="<<T<<" and MAX.T="<<MaxTemp<<"\n";
#ifdef TESTING
      dp.grid->PrintCell(dp.c);
    ...
    //
    // collisionally excited neutral H cooling from Raga, Mellema, Lundqvist, 1997, ApJS, 109, 517, appendix.
    // I fit it with gnuplot, BY EYE!!!, SO THIS IS NOT FOR PRODUCTION!!!
    // log10(cool) = -18.5*(1+exp(-(log10(T)-3.44)/0.4)) in erg.cm3/s
    //
    //    rate += exp(-18.5*(1.0+exp(-2.5*(log10(T)-3.44)))*log(10.0))*nH*nH*xHp*(1.0-xHp);
    //
    // This is from somewhere else, and is almost the same...
    //
    rate += 7.5e-19*exp(-118348.0/T)/(1.0+sqrt(T/1.0e5))*nH*nH*xHp*(1.0-xHp);
    //
    // And the recombination cooling is already taken care of in microphysics
    //
    return rate;
    ...
    // OLD C2RAY GUESSES.
    //
    // I think this is C2Ray-like cooling, appropriate for pure hydrogen/helium gas.
    //
    // first free-free for hydrogen only.
//    rate += kB*sqrt(T)*1.25e-11; // rate erg.cm^3/s is kT*1.25e-11/sqrt(T)
...
//      rep.error("ion fraction too small in cool->CoolingRate()",xHp);
      // THIS MUST BE A CRAZY INTEGRATION ATTEMPT, SO RETURN A MASSIVE COOLING RATE...
//      rate += 1.0/TINYVALUE;
//    }
    //double temp = 7.5e-19*exp(-118348.0/T)/(1.0+sqrt(T/1.0e5))*(1.0-xHp)/xHp;
    //cout <<", c.ex. rate="<<temp;
    //rate += temp;
//    rate += 7.5e-19*exp(-118348.0/T)/(1.0+sqrt(T/1.0e5))*(1.0-xHp)/xHp;
    //cout <<", ::: total="<<rate<<"\n";
...
//    rate *= xHp*xHp*nH*nH;
  ...
  else if (WhichFunction==6 || WhichFunction==7 || WhichFunction==8 ||
    ...
    //
    // This corresponds to SD93 CIE cooling function, with an extra component for
    // cooling due to collisionally excited ions of C,O in photo-ionised plasma,
    // from Osterbrock, or Spitzer.
    // 
    if (T>MaxTemp) {
      //cout <<"Temp out of range!! Too large: T="<<T<<" and MAX.T="<<MaxTemp<<"\n";
#ifdef TESTING
    ...
    //
    // If the dynamics gives a negative pressure, we may need to ensure some heating happens...
    // (BUT THIS IS DANGEROUS -- CAN MAKE CLUMPS EXPLODE, SO USE WITH EXTREME CAUTION!!!!!)
    //double t_c=3.16e11, T_inf=50.0;
    //if (T<T_inf) {
    ...
    // cooling due to collisionally excited ions of C,O in photo-ionised plasma,
    // from Osterbrock, or Spitzer.
    // Plus a toy model for cooling neutral dense gas to 100K.
    // 
    ...
    if (T>MaxTemp) {
      ...
      // cout <<"T="<<T<<", MinTemp="<<MinTemp<<", fraction = "<<exp(25.0*(log(T)-log(MinTemp)))<<"\n";
      rate = max(1.e-50, Lamb[0]*exp(25.0*(log(T)-log(MinTemp))));
    }
    else {
      interpolate.splint(Temp, Lamb, Lam2, Nspl, T, &rate);
    ...
    // Add in the minimum Temperature so that dodgy calls won't get strange results.
    //
    if (T<2.0e4 && T>100.0) rate += 2.0e-24*T/8000.0; //exp(2.0*log(T/8000.0));
    ...
    // (nH in the denominator of t_c gives time constant shorter with density)
    // (Change t_c to get a different normalisation.) 
    // Have an exponential cutoff at 1e4K so that hot gas doesn't have this cooling.
    //
    double t_c=0.0, t_h=0.0, T_inf=100.0, T_min=5.0;
    switch (WhichFunction) {
    case 12:
      t_c = 3.16e13/nH; t_h = 3.16e11/nH; break; // 1e6 yrs/nH
    case 13:
      t_c = 3.16e14/nH; t_h = 3.16e12/nH; break; // 1e7 yrs/nH
    case 14:
      t_c = 1.0e11; t_h = 3.16e9; break; // ~3e3 yrs: Takes away dependency on density!
    case 15:
      t_c = 3.16e11; t_h = 3.16e9; break; // 1e4 yrs: Takes away dependency on density!
    default:
      rep.error("Bad flag in toy model cooling!!!",WhichFunction);
    }

    //
    // THIS IS A BIG HACK -- IF I GET A NEGATIVE PRESSURE, I START WITH A VERY LOW TEMPERATURE,
    // AND THE FOLLOWING LINE MEANS LOW TEMPERATURE CELLS HAVE VERY SHORT ENERGY GAIN TIMESCALES,
    // SO THERE IS **VERY** STRONG HEATING IN THE MICROPHYSICS
    //
    if (T<T_min) {
      //
    ...
    //
    rate += 2.905e-19*z0* xHp*xHp*nH*nH *exp(-33610.0/T -(2180.0*2180.0/T/T));

    //
    // Now the neutral metal line cooling (Henney et al. 2009, eq. A10)
    //
    rate += 4.477e-20*z0* xHp*(1.0-xHp)*nH*nH *exp(-28390.0/T -(1780.0*1780.0/T/T));
    ...
    //
    if (T>5.0e4) {
      double temp = 3.485e-15*z0*exp(-0.63*log(T))*(1.0-exp(-pow(1.0e-5*T,1.63)));
    ...
    //
    // HACK ALERT!!! I AM REDUCING THE DENSITY SCALING COMPARED TO WHAT HENNEY USES!
    //
    ...
    //
    rate -= 1.9e-26*nH*FUV_flux*exp(-1.9*FUV_extinction)/(1.0 +6.4*FUV_flux*exp(-1.9*FUV_extinction)/nH);
    //
    ...
    //
    rate -= 7.7e-32*nH*FUV_flux*exp(-0.05*FUV_extinction)*exp(-2.0*log(1.0+3.0e4/nH));
    //
    // Cosmic Ray Heating (Henney et al. 2009, eq. A7)
    // HACK ALERT!!! I AM INCREASING THIS BY 10X TO COMPENSATE FOR NO XRAY HEATING!!!
    //
    rate -= 5.0e-27*nH;
  ...
  else rep.error("Which Function? CoolingFn::CoolingRate",WhichFunction);
