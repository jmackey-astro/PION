    ...
    photo_xsections()  // photo_xsections(&Emin[0],&Emax[0],Nbins)
{
  ...
  // ---------
  // ---------       y_ion_num_elec (# electrons of y_ion_index_prim
  // species),---------
  ...
  for (int i = 0; i < len; i++) {
    ...
    // ================================================================
    if (s[0] == 'X') {
      X_mass_frac_index.push_back(
          ftr + N_elem);  ///< record primitive vector index of each element
      ...
      if (s.substr(2, 2) == "He") {
        ...
        X_elem_number_density.push_back(
            0);  // just to initialise the length of X_elem_number_density
      }
      //=======Hydrogen======
      else if (s[2] == 'H') {
  ...
  // lv_y_ion_index_offset = ftr + N_elem; // gives the index at which ions
  ...
  // NOTE \Maggie{or let's not assume that...} Assume metal content is low
  // enough to ignore it.
...
void MPv10::species_tracer_initialise(
    ...
    int el_index,  /// element index in N_species_by_elem, used in for loops for
                   /// densities etc
...
)
{
  ...
  y_ion_index_prim.push_back(
      ftr + N_elem + N_species);  // lv_y_ion_index_offset + N_species);
  ...
  ss_e >> num_elec_int;
  /// Record the electron number and He_ion_index vector (used to identify
  /// tracer index from string)
  ...
  /// Check if the next ion up / down exists in tracer list (or if it's neutral)
  /// to define y_ip1_index and y_im1_index.
  ///< due to ordering in tracer list, if the next ion exists, its index will be
...
double MPv10::get_n_elec(const pion_flt *P  ///< primitive state vector.
)
{
...
)
{
  ...
  /*cout << "temp=" <<
  gamma_minus_one*E/(k_B*get_ntot(y_ion_frac,X_number_density)); cout << ", ntot
  = " <<get_ntot(y_ion_frac,X_number_density) ; cout << ", E = " << E << "\n";*/
...
)
{
  ...
  for (int e = 0; e < N_elem; e++) {  // loop over every element
    ...
    pion_flt neutral_frac = 1;  // neutral frac, got by subtracting off the ion
                                // fractions in the next loop.

    for (int s = 0; s < N_elem_species;
         s++) {  // loop over every species in THIS element
      pion_flt number_density =
          X_number_density[e] * y_ion_frac[species_counter];
      int num_elec = y_ion_num_elec[species_counter];

      n_tot +=
          (1 + num_elec) * number_density;  // add on number of particles got by
                                            // electrons + ionised atom
...
)
{
  ...
  for (int i = 0; i < N_elem; i++) {  // loop over every element
    double n_X =
        p_in[RO] * (p_in[X_mass_frac_index[i]] / X_elem_atomic_mass[i]);
  ...
  // ==============================================================
  //
  // loop over every species and set the local vector, noting that
  // loc_vec[i]=prim_vec[i]/X_e
  int species_counter = 0;
  for (int e = 0; e < N_elem; e++) {  // loop over every element
    int N_elem_species = N_species_by_elem[e];
    ...
    for (int s = 0; s < N_elem_species;
      ...
      if (p_local[y_ion_index_local[species_counter]] < (-2.0 * MPv10_ABSTOL)) {
        alter_flag = 1;
          ...
          p_local[y_ion_index_local[species_counter]] > (1.0 + MPv10_ABSTOL)) {
        alter_flag = 1;
      };
      species_counter++;
    }
  ...
  if (p_local[lv_eint] <= 0.0) {
    ...
    p_local[lv_eint] = get_ntot(y_ion_frac, X_elem_number_density) * k_B
...
)
{
  ...
  for (int e = 0; e < N_elem; e++) {  // loop over every element
    ...
    for (int s = 0; s < N_elem_species;
      ...
      p_out[y_ion_index_prim[species_counter]] =
          p_local[y_ion_index_local[species_counter]]
          * p_in[X_mass_frac_index[e]];
      y_ion_frac[species_counter] = p_local[y_ion_index_local[species_counter]];
      species_counter++;
    }
  }

  // Set mass fraction tracers to be within the required range (not too close to
  // zero or 1)

  species_counter = 0;
  for (int e = 0; e < N_elem; e++) {  // loop over every element
    int N_elem_species          = N_species_by_elem[e];
    p_out[X_mass_frac_index[e]] = max(
        Min_NeutralFrac,
        min(Max_NeutralFrac, static_cast<double>(p_out[X_mass_frac_index[e]])));
    ...
    for (int s = 0; s < N_elem_species;
      ...
      p_out[y_ion_index_prim[species_counter]] = max(
          Min_NeutralFrac,
          min(static_cast<double>(p_out[X_mass_frac_index[e]])
                  * Max_NeutralFrac,
              static_cast<double>(p_out[y_ion_index_prim[species_counter]])));
      species_counter++;
    }
  ...
  double T =
      get_temperature(y_ion_frac, X_elem_number_density, p_local[lv_eint]);
  // cout <<"current temp = " << T<<"\n";
  ...
  for (int e = 0; e < N_elem; e++) {  // loop over every element
    ...
    for (int s = 0; s < N_elem_species;
      ...
      y_ion_frac[species_counter] =
          pv[y_ion_index_prim[species_counter]] / pv[X_mass_frac_index[e]];
      species_counter++;
    }
  }
...
)
{
  ...
  for (int e = 0; e < N_elem; e++) {  // loop over every element
    ...
    for (int s = 0; s < N_elem_species;
         s++) {  // loop over every species in THIS element
      y_ion_frac[species_counter] =
          p_pv[y_ion_index_prim[species_counter]] / p_pv[X_mass_frac_index[e]];
      species_counter++;
    }
  }
  ...
  P[lv_eint] =
      get_ntot(y_ion_frac, X_elem_number_density) * k_B * T / (gamma_minus_one);
...
)
{
  ...
  // Tests have shown this is good enough, and that a restriction on the energy
  // change (heating timescale) is not required for accurately tracking
  ...
  // criterion, rather than the default of absolute change in neutral fraction.
  //
  // LOOP OVER ALL THE IONS HERE, NOT JUST LV_H0, AND THAT SHOULD FIX YOUR
  // SEGFAULT PROBLEM.
...
)
{
  ...
  for (int e = 0; e < N_elem; e++) {  // loop over every element
    ...
    if (s_frac
        > ((p_in[X_mass_frac_index[e]] * e_correction) - Min_NeutralFrac)) {
      print_flagg = 1;
      double s_correction =
          ((p_in[X_mass_frac_index[e]] * e_correction) - Min_NeutralFrac)
          / s_frac;
...
)
{
  ...
  //  double Vshell;   ///< Shell volume for discrete photo-ionisation/-heating
  //  rates. double dS;       ///< Path length through cell. double
  //  strength[MAX_TAU]; ///< Luminosity (or flux if source at infinity). double
  //  Column[MAX_TAU];  ///< integral of quantities along LOS to near edge of
  //  cell. double DelCol[MAX_TAU];  ///< integral of quantities along LOS
  //  through cell. int id;   ///< source id. int type; ///< diffuse-radiation
  //  or a real source. short unsigned int NTau; ///< Number of LOS quantities
  //  traced for the source.
  //};
  ...
  for (int bin = 0; bin < get_nbins(); bin++) {
    ...
    for (int e = 0; e < N_elem; e++) {  // loop over every element
      ...
      for (int s = 0; s < N_elem_species; s++) {  // loop over every species
        double n_s =
            p_in[RO]
            * (p_in[y_ion_index_prim[species_counter]] / X_elem_atomic_mass[e]);
...
)
{
  ...
  for (int elem = 0; elem < N_elem; elem++) {  // loop over every element
    ...
    for (int s = 0; s < N_elem_species;
      ...
      // add to y_ion_frac
      y_ion_frac[species_counter] =
          NV_Ith_S(y_now, y_ion_index_local[species_counter]);
      // add to ne based on the number of electrons liberated to obtain this ion
      pion_flt number_density =
          X_elem_number_density[elem] * y_ion_frac[species_counter];
      int num_elec = y_ion_num_elec[species_counter];
      ne += num_elec * number_density;
      species_counter++;
    }
  ...
  /// ======== this_y_dot(ion) += ionise_rate(im1)*n_e*y(im1) <<< add ionisation
  /// from less ionised species to current species
  /// =================================================================================
  species_counter = 0;
  for (int elem = 0; elem < N_elem; elem++) {  // loop over every element
    int N_elem_species  = N_species_by_elem[elem];
    ...
    for (int s = 0; s < N_elem_species;
      ...
      if (y_im1_index_local[species_counter] != -1) {
        double lower_ion_rate =
            ionise_rate_table[y_im1_index_tables[species_counter]][temp_index];
        double upper_ion_rate_contrib =
            dT
            * ionise_slope_table[y_im1_index_tables[species_counter]]
                                [temp_index];
        ...
        if (y_im1_index_local[species_counter] != -2) {
          double this_y_dot =
              ionise_rate_im1
              * NV_Ith_S(y_now, y_im1_index_local[species_counter]) * ne;
          ...
          double ion_pot =
              ionisation_potentials[y_im1_index_tables[species_counter]];
  ...
  /// ============== Radiative recombination OUT OF this species and INTO NEXT
  /// species ===================== y_dot(ion) -= recomb_rate(ion)*n_e*y(ion)
  /// <<< subtract recombination to less ionised species
  /// ================================================================================
  species_counter = 0;
  for (int elem = 0; elem < N_elem; elem++) {  // loop over every element
    int N_elem_species = N_species_by_elem[elem];
    ...
    for (int s = 0; s < N_elem_species;
      ...
      if (y_im1_index_local[species_counter] != -1) {
        double lower_recomb_rate =
            recomb_rate_table[y_ion_index_tables[species_counter]][temp_index];
        double upper_recomb_rate_contrib =
            dT
            * ionise_slope_table[y_ion_index_tables[species_counter]]
                                [temp_index];
        double recomb_rate_ion = lower_recomb_rate + upper_recomb_rate_contrib;

        double this_y_dot =
            recomb_rate_ion
            * NV_Ith_S(y_now, y_ion_index_local[species_counter]) * ne;
        // Subtract this rate of change from the current species
        NV_Ith_S(y_dot, y_ion_index_local[species_counter]) -= this_y_dot;
        // add this rate of change to the recombination species (provided it
        // isn't neutral)
        if (y_im1_index_local[species_counter] != -2) {
          NV_Ith_S(y_dot, y_im1_index_local[species_counter]) += this_y_dot;
        }
        //         cout << "y_dot [" << species_counter << "] -=" << this_y_dot
        //         <<"\n"; for (int v=0;v<N_equations;v++) dydt[v] =
        //         NV_Ith_S(y_dot,v); rep.printVec("ydot",dydt,N_equations);
...
)
{
  ...
  // recombination for Nitrogen (from Raga, deColle, et al., 2007, A&A, 465,
  // 879).
  else if (ci->i == N1p) {
    a1 = 1.5e-12;
    a2 = 0.693;
...
double MPv10::rad_recomb(double T, enum species i)
{
...
double MPv10::dielec_recomb(double T, enum species i)
{
...
void MPv10::generate_lookup_tables()
{
  ...
  for (int i = 0; i < Num_temps; i++) {
    double T_i  = pow(10.0, (log10f(T_min) + i * delta_log_temp));
    Temp_arr[i] = T_i;
  }
  Temp_Table.insert(Temp_Table.end(), &Temp_arr[0], &Temp_arr[Num_temps]);

  //  Now make lookup tables for the recombination rates and ionisation rates
  //  respectively.
  const int number_of_species = 29;

  species species_list[number_of_species] = {
      H_0, H_1p, He0, He1p, He2p, C0,  C1p, C2p, C3p, C4p,
      C5p, C6p,  N0,  N1p,  N2p,  N3p, N4p, N5p, N6p, N7p,
      O0,  O1p,  O2p, O3p,  O4p,  O5p, O6p, O7p, O8p};
  string ion_names[number_of_species] = {
      "H0",  "H1+", "He0", "He1+", "He2+", "C0",  "C1+", "C2+", "C3+", "C4+",
      "C5+", "C6+", "N0",  "N1+",  "N2+",  "N3+", "N4+", "N5+", "N6+", "N7+",
  ...
  for (int i = 0; i < number_of_species; ++i)
    recomb_rate_table[i].resize(Num_temps);

  recomb_slope_table.resize(number_of_species);
  for (int i = 0; i < number_of_species; ++i)
    recomb_slope_table[i].resize(Num_temps);

  ionise_rate_table.resize(number_of_species);
  for (int i = 0; i < number_of_species; ++i)
    ionise_rate_table[i].resize(Num_temps);

  ionise_slope_table.resize(number_of_species);
  for (int i = 0; i < number_of_species; ++i)
    ionise_slope_table[i].resize(Num_temps);
  ...
  //  Now, generate the slopes tables for interpolating temperatures that don't
  //  exactly fit here.
  for (int s = 0; s < number_of_species; s++) {
    for (int T_i = 0; T_i < Num_temps - 1;
         T_i++) {  // note this is <Num_temps -1, b/c i+1
      float this_recombination_slope;
      float this_ionisation_slope;

      this_recombination_slope =
          (recomb_rate_table[s][T_i + 1] - recomb_rate_table[s][T_i])
          / (Temp_Table[T_i + 1] - Temp_Table[T_i]);

      this_ionisation_slope =
          (ionise_rate_table[s][T_i + 1] - ionise_rate_table[s][T_i])
          / (Temp_Table[T_i + 1] - Temp_Table[T_i]);
  ...
  /// ===================================================================
  double ionisation_pot_arr[number_of_species] = {
      13.59844, -1.0e99, 24.58741, 54.41778, -1.0e99, 11.3, 24.4, 47.9,
      64.5,     392.1,   490.0,    -1.0e99,  14.5,    29.6, 47.5, 77.5,
      97.9,     552.1,   667.0,    -1.e99,   13.6,    35.1, 54.9, 77.4,
      113.9,    138.1,   739.3,    871.4,    1.e99};  // energy (eV) required to
                                                      // raise ion from species
                                                      // i to species i+1
  for (int i = 0; i < number_of_species; i++)
    ionisation_pot_arr[i] *= erg_per_eV;  // convert eV to erg
  ionisation_potentials.insert(
      ionisation_potentials.end(), &ionisation_pot_arr[0],
      &ionisation_pot_arr[number_of_species]);
