      ...
      )
{
  if (rsi->effect!=RT_EFFECT_MFION)
	...
	// Create empty table to store temperature / flux values -- nb There are 65 temperatures recorded for logg=4
	double flux_table[65][12];
	double temps[65];
	
	// Grid of fluxes from Castelli, compiled into one text file of fractional luminosity / bin in a separate python script
	// Original data from: http://www.oact.inaf.it/castelli/castelli/grids/gridp00k2odfnew/fp00k2tab.html
	...
	for (int i=0; i<65; i++){ 			// Loop over remaining lines to populate table
		...
		for (int j=0; j<12; j++){			// record every other column as an entry in the flux table
			fscanf(pf, "\t%lf", &flux_table[i][j]);
		}
		fscanf(pf, "\n");							// progress to next line...
	}
	
	// To interpolate, first find which temperatures our star's temperature lies between 
	// Start by finding the first T in temps greater than our star's temp, i.e. upper value to interpolate from
	int i=0;
	while( rsi->Tstar > temps[i] && i < 65) i++;
	
	double Tlower = temps[i-1];
	double Tupper = temps[i];
	
	// Then linearly interpolate between flux bins above / below to determine flux bins for the given temperature
	for (int j=0; j<12; j++){
		output[j] = flux_table[i-1][j] * ( 1 - (rsi->Tstar - Tlower)/(Tupper - Tlower)) + flux_table[i][j]*(1 - (Tupper - rsi->Tstar)/(Tupper-Tlower));
