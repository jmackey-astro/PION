...
/// of the relevant column density range for Hot stars.  Error is up to 2 per cent
/// for a star with Teff=10000K, assuming it is a blackbody and emits ionising 
/// photons.  More realistic spectra could be added for such cool stars, since a
/// blackbody is not a good approximation for the UV emitted flux.  For hot stars
/// an integration up to 1000eV is recommended, although the results are not very
/// sensitive to this.  Cutting off at 500 eV gives almost indistinguishable results,
/// but at 250 eV the difference is noticeable.  For cool stars the cutoff could
/// probably be lower.
///
class hydrogen_photoion
{
  ...
  ///
  /// For given stellar parameters, set up the photoionisation and photoheating
  /// spline interpolation tables for the rates as a function of column density.
  /// Uses discretised photon-conserving formulation of Mellema et al (2006,NewA,11,374).
  ///
  ...
  ///
  /// Get DISCRETISED multifrequency photoionisation rate for a given column density
  /// of H0, local number density of H0, and Shell volume ~4.Pi.[(R+)^3-(R-)^3]/3
  /// Uses discretised photon-conserving formulation of Mellema et al (2006,NewA,11,374).
  ///
  /// N.B. The result is returned as the energy added per second per H nucleon.
  ///
  double Hi_discrete_multifreq_photoion_rate(
                const double, ///< Optical depth of H0 to front edge of cell (at 13.6eV).
  ...
  ///
  /// Get DISCRETISED multifrequency photoheating rate for a given optical depth
  /// of H0, local number density of H0, and Shell volume ~4.Pi.[(R+)^3-(R-)^3]/3
  /// Uses discretised photon-conserving formulation of Mellema et al (2006,NewA,11,374).
  ///
  /// N.B. The result is returned as the number of ionisations per second per H nucleon.
  ///
  ...
  ///
  /// Get multifrequency photoionisation rate for a given optical depth of H0, local
  /// number density of H0, and Shell volume ~4.Pi.[(R+)^3-(R-)^3]/3
  /// Uses discretised photon-conserving formulation of Mellema et al (2006,NewA,11,374).
  ///
  /// N.B. The result is returned as the number of ionisations per second per neutral H atom.
  ///
  ...
  ///
  /// Get multifrequency photoionisation heating rate for a given optical depth
  /// of H0, local number density of H0, and Shell volume ~4.Pi.[(R+)^3-(R-)^3]/3
  /// Uses discretised photon-conserving formulation of Mellema et al (2006,NewA,11,374).
  ///
  /// N.B. The result is returned as the energy added per second per neutral H atom.
  ///
