...
class MPv0 : public MicroPhysicsBase, public Integrator_Base {
 public:
  ...
  double Get_Temp(
      const double * ///< State vector to get Temperature from.
      );
  ...
  /// Returns Collisional Ionisation rate from ion passed in to next stage up, in [cm^3/s] units. 
  double Coll_Ion_rate(
      double,            ///< Precalculated Temperature.
      const ion_struct * ///< pointer to current ion data.
      );

  /// Returns Radiative Recombination rate from ion passed in to next stage down, in [cm^3/s] units. 
  double Rad_Recomb_rate(
      double,            ///< Precalculated Temperature.
      const ion_struct * ///< pointer to current ion data.
      );

  /// Some ions have dielectronic recombination; this returns the DR rate, calculated
