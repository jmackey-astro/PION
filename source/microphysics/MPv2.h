...
/// collisional excitation of neutral metals (eq.A10), and the SD93 CIE curve.
/// I think I will take the max of the SD93 curve and the Henney et al.
...
/// use a standard value, X-ray heating I'm not yet sure about.  UV heating due
/// to the interstellar radiation field (ISRF) will be according to the optical
/// depth from the edge of the domain to the cell in question, using e.g. HEA09
/// eq.A3.  UV heating from the star can use the same equation, but with the
/// optical depth from the source (using a total nucleon column density).
///
...
///
/// This class cannot be part of MPv2 because it cannot be the MP global
/// pointer. It needs a local initialisation (with different interface
/// functions) which CVodes can use.
///
...
public:
  ...
  ///
  /// This should be called before the microphysics integration, to set the
  ...
  ///
  /// returns the relative error tolerance required for this system of
...
///
/// This class will now just be a driver for CVodes, and using the dYdt()
/// function from mp_rates_ExpH_ImpMetals MPR.  It is essentially an interface
/// class like MPv9 which interfaces between my code and Harpreet's.
///
class MPv2 : public microphysics_base {
public:
  ///
  /// Constructor.  Sets index for H ion fraction in primitive variable state
  /// vector.
  ///
  MPv2(
  ...
  ///
  /// The main update function.  A primitive vector is input, and lots of
  /// optional extra data in the last argument, and the ion fraction(s) and
  /// internal energy are updated by the requested timestep.  Results are
  /// output to the destination vector, which can be the same pointer as the
  /// initial vector.
  ///
  /// The integration switch uses the following values:
  /// 0=adaptive-RK5, 1=adaptive-Euler, 2=single-step-RK4
  /// For strongly ionised cells an analytic solution is used, where the ion
...
private:
  ...
  ///
  /// This takes a step dt, returning a non-zero error code if the error
  /// fails.
  ///
  ...
  ///
  /// For each cell, dYdt() needs to know the local radiation field.  This
  ...
  int pv_Hp;  ///< index for element of Primitive vector that holds ionisation
              ///< pot.
