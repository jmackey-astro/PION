...
#define LOGSLP
//#define LINSLP ///< Use linear extrapolation of cooling function for low
// temperatures.
...
class CoolingFn {
  public:
     ...
     *
     * Input Temperature should be in Kelvin, number density in per c.c., and
     * ion fraction between 0 and 1.  The cooling rate is returned in cgs units,
     * (rate = erg cm^-3 s^-1).  This should be the actual cooling rate, so no
     * need to multiply it by number density or electron/ion densities.
     */
    double CoolingRate(
        const double,  ///< Input Temperature.
  ...
  private:
    ...
    double*
        Lamb;  ///< Normalised Cooling rate (erg cm^3 s^-1) = Lambda(T)/rho^2
