...
/// - 2011.04.18 JM: Added an ifdef for RT_TEST_PROBS to return 2.59e-13 as the
/// RRR.
...
Hummer94_Hrecomb::Hummer94_Hrecomb() : kB(1.381e-16)
{
    //
    ...
    double coolb[31]   = {8.287e-11, 7.821e-11, 7.356e-11, 6.892e-11, 6.430e-11,
                        ...
                        4.499e-14};
    double CoolTot[31] = {9.348e-11, 8.889e-11, 8.432e-11, 7.977e-11, 7.525e-11,
                          7.077e-11, 6.633e-11, 6.194e-11, 5.758e-11, 5.332e-11,
                          4.915e-11, 4.508e-11, 4.112e-11, 3.733e-11, 3.373e-11,
                          3.039e-11, 2.737e-11, 2.472e-11, 2.247e-11, 2.062e-11,
                          1.914e-11, 1.797e-11, 1.704e-11, 1.628e-11, 1.563e-11,
                          1.505e-11, 1.451e-11, 1.402e-11, 1.358e-11, 1.318e-11,
                          1.285e-11};
    //
    ...
    MaxSlope_alpha =
        (log10(hr_alpha[hr_Nspl - 1]) - log10(hr_alpha[hr_Nspl - 2]))
        / (log10(hr_t[hr_Nspl - 1]) - log10(hr_t[hr_Nspl - 2]));
    // cout << "\t\tAlpha min-slope="<<MinSlope_alpha<<"
    // max-slope="<<MaxSlope_alpha<<"\n";
    //
    ...
    MaxSlope_beta = (log10(hr_beta[hr_Nspl - 1]) - log10(hr_beta[hr_Nspl - 2]))
                    / (log10(hr_t[hr_Nspl - 1]) - log10(hr_t[hr_Nspl - 2]));
    // cout << "\t\tBeta  min-slope="<<MinSlope_beta<<"
    // max-slope="<<MaxSlope_beta<<"\n";
    //
...
double Hummer94_Hrecomb::Hii_rad_recomb_rate(const double T)
{
    ...
    //
    // Temperature, T, is in Kelvin.  We want to avoid running off the end of
    // the spline fit, so we limit T to be between [10,1e7].  Outside these
    // ranges we extrapolate the rates based on the logarithmic slope of the
    // function at the relevant end of the range.
    //
    double rate = 0.0;
    if (T > MaxTemp) {
...
// g++ -Wall -g Hummer94_Hrecomb.cc cooling_SD93_cie.cc ../global.cc
// ../cell_interface.cc -lreadline -o temp/testmp
int main()
{
