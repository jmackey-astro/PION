...
Integrator_Base::Integrator_Base()
{
  ...
  dc4 = c4-13525./55296.;
  dc5 = -277./14336.;
  dc6 = c6-0.25; ///< dc2=0.  These are the 5th order minus 4th order weights.
			       ...
			       )
{
  ...
  int err=0;
  for (int v=0;v<int_nvar;v++) pf[v]=p0[v];
      ...
      )
{
  ...
  if (errtol<0) rep.error("Int_DumbAdaptive_Euler() ErrTol is negative!",errtol);
  if (errtol<MACHINEACCURACY)
    rep.error("errtol beyond machine accuracy.  use more lenient value!\n",errtol);
  // Dumb adaptive integrator.  Start with 16 subpoints, keep doubling until values differ
  // by less than errtol
  int nsub=16;
  ...
  *tf = t+dt;
  ...
  for (int v=0; v<int_nvar; v++)
    if (fabs(p0[v])<1.e-100)
      cerr<<"Int_DumbAdaptive_Euler() WARNING: tiny values, so error unreliable!\n";
  ...
  do {
    ...
    for (int v=0; v<int_nvar; v++) {
      ...
      if (isnan(p2[v]) || isnan(p1[v])) maxerr=max(maxerr,1000.0);
      else {
  ...
  for (int v=0; v<int_nvar;v++) pf[v] = p1[v];
  if (iter>max_iter) cerr<<"Int_DumbAdaptive_Euler() WARNING: not converged!\n";
			      ...
			      )
{
  ...
  for (int v=0;v<int_nvar;v++) k4[v] *= dt;

  for (int v=0;v<int_nvar;v++) pf[v] = p0[v] +(k1[v] +2.0*k2[v] +2.0*k3[v] +k4[v])/6.0;
				...
				)
{
  ...
  */
  ...
  for (int v=0;v<int_nvar;v++) {
    k5[v] *= dt;
    ptemp[v] = p0[v] +b61*k1[v] +b62*k2[v] +b63*k3[v] +b64*k4[v] +b65*k5[v];
  }
				  ...
				  )
{
  ...
  for (int v=0;v<int_nvar;v++) {
    ...
    //  cerr <<"WARNING: stepper_RKCK() encountered tiny values, accuracy is unreliable.\n";
    //      return 1;
    //}
  }
  ...
  do {
    ...
    for (int v=0;v<int_nvar;v++){
      // if we get back NAN, make sure we try a shorter step!
      if (isnan(err[v]) || isnan(ptemp[v]) || isinf(err[v]) || isinf(ptemp[v])) {
	maxerr=max(maxerr,1000.0);
	//cout <<"err["<<v<<"]="<<err[v]<<" and ptemp["<<v<<"]="<<ptemp[v]<<"\n";
      }
      else {
    ...
    if (maxerr>1.) {
      ...
      //cout <<"\tSTEPPER:\t shrinking stepsize: h_old = "<<h<<" and h_new = "<<htemp<<"\n";
...
#define BISECTION_STEPPER
  ...
  *hnext = min(htemp, 5.0*h);
  ...
  //
  // Copy result vector into output vector, and check is has real numbers in it!
  //
  *hdid  = h;
  for (int v=0;v<int_nvar;v++) {
    p1[v] = ptemp[v];
...
#ifdef TESTING
  if (dp.c->id==2773) {
    cout <<"*********RKCK err estimate = "<<maxerr*errtol<<" after "<<ct<<" RKCK calls. rval="<<rval<<" and dt="<<h<<"\n";
    rep.printVec("p0      ",p0,int_nvar);
    rep.printVec("ptemp   ",ptemp,int_nvar);
  }    
#endif
				       ...
				       )
{
  ...
  for (int v=0;v<int_nvar;v++) p1[v] = p0[v];
  ...
  *tf = t0+dt;
  ...
  do {
    ...
    h = min(hnext, *tf-t);
    ct++;
    for (int v=0;v<int_nvar;v++) p1[v] = p2[v];
    //cout <<"ADAPTIVE INT:\t t_old="<<t-hdid<<" t_new="<<t<<" iter="<<ct<<"\n";
    //cout <<"ADAPTIVE INT:\t "; rep.printVec("pnew",p1,int_nvar);
  } while (t<*tf && (err==0) && (ct<ctmax));
  
  if (err || ct>ctmax) {
    if (err) {
      cout<<"Int_Adaptive_RKCK() errors encountered. nstep="<<ct<<" and err="<<err<<"\n";
...
int Integrator_Base::Int_Iterative_FC(const int nv,   ///< number of elements in P array.
				      const double *p0, ///< value of P at initial value of t.
				      const double t0,  ///< initial value of t.
				      const double dt,   ///< Total step dt to take.
				      double *pf ///< pointer to final P value.
				      )
{
   ...
   * This function is explicitly for solving the rate equations pdot=A*(1-p)-B*p
   * We linearise the equation by fixing A,B=constants, with parameters given
   * by some combination of p(0) and p(n-1), and put these into the analytic
   * solution p(n) = A/(A+B) + (p(0)-A/(A+B))exp(-(A+B)(t-t0))
   * And iterate until p(n)=p(n-1) to some tolerance.
   * We can't just use dPdt() for this, but need the actual parameters A,B, given
   * by the functions C_rate() = A and D_rate()=A+B.  So to use this function, the
   * derived class must have these defined right.
   */
  ...
  for (int v=0;v<int_nvar;v++) p2[v] = p0[v];
  do {
    for (int v=0;v<int_nvar;v++) p1[v] = p2[v];
    ...
    errmax = 0.0;
    for (int v=0;v<int_nvar;v++)
      errmax = max(errmax, fabs(p1[v]-p2[v])/(fabs(p1[v])+fabs(p2[v])+eps));
  } while ( (errmax>errtol) && nc<=Nmax);

  if (nc>Nmax) cerr <<"\t!!! failed to converge after "<<Nmax<<" steps. ";
  //  cout <<"\tAccuracy of solution = "<<errmax<<"\n";
  for (int v=0;v<int_nvar;v++) pf[v] = p2[v];
				      ...
				      )
{
  ...
  double subt;
  double t=0.0;
  for (int v=0;v<int_nvar;v++) pnow[v] = p0[v];
  ...
  //  cout <<"nsteps = "<<nc<<"\n";
  if (nc>Nmax) cerr <<"BDF_subcycling() took more than "<<Nmax<<" steps. ";
  for (int v=0;v<int_nvar;v++) pf[v] = pnow[v];
