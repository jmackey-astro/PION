...
Integrator_Base::Integrator_Base()
{
    ...
    dc5 = -277. / 14336.;
    dc6 = c6
          - 0.25;  ///< dc2=0.  These are the 5th order minus 4th order weights.
    ...
    do {
        ...
        for (int v = 0; v < int_nvar; v++) {
            ...
            if (isnan(p2[v]) || isnan(p1[v]))
                ...
                tmperr =
                    fabs(p2[v] - p1[v]) / (fabs(p2[v]) + fabs(p1[v]) + 1.e-100);
                //	cout <<"tmperr ="<<tmperr<<"\n";
...
)
{
    ...
    */
    ...
    for (int v = 0; v < int_nvar; v++) {
        k4[v] *= dt;
        ptemp[v] =
            p0[v] + b51 * k1[v] + b52 * k2[v] + b53 * k3[v] + b54 * k4[v];
    }
    ...
    for (int v = 0; v < int_nvar; v++) {
        ...
        // err is the difference between the 5th and 4th order result.
        dp[v] =
            dc1 * k1[v] + dc3 * k3[v] + dc4 * k4[v] + dc5 * k5[v] + dc6 * k6[v];
    }
    ...
    double* hnext)
{
    ...
    do {
        ...
        // cout <<"DDDD: "<<p0[4]<<"\n";
        rval +=
            Step_RK5CK(nv, p0, t0, h, ptemp, err);  // returns absolute error
        ...
        if (maxerr > 1.) {
            htemp =
                0.9 * h * exp(-0.25 * log(maxerr));  // NR stepsize shrinker.
...
)
{
    ...
    if (errtol < 0)
        rep.error("Int_Adaptive_RKCK() ErrTol is negative!", errtol);
    if (errtol < MACHINEACCURACY)
        rep.error(
            "errtol beyond machine accuracy.  use more lenient value!\n",
            errtol);
    ...
    *tf      = t0 + dt;
    ...
    if (err || ct > ctmax) {
        if (err) {
            ...
            err = ct;
            cout << "integration took too many steps!!! ct=err=" << ct << "\t"
                 << err << "\n";
...
)
{
    ...
    do {
        ...
        for (int v = 0; v < int_nvar; v++)
            errmax =
                max(errmax,
                    fabs(p1[v] - p2[v]) / (fabs(p1[v]) + fabs(p2[v]) + eps));
