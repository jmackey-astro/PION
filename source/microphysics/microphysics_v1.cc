...
///   Added in an ifdef in MP_Hydrogen for if the integration (not Photoionisation)
...
# else
    ...
    if      (s=="e-____")               {
      ...
      s="tr"; colour++; // do nothing with non-chemistry tracers.
    }
    else if (s=="H1+___" || s=="HII__") {
      s="H1+"; pvar[s] = ftr+i; lvar[s] = firstion+ct; ct++;
      lv_hp = lvar[s];
      ions.push_back(s);  nions++;
      els.push_back("H"); nels++;
    }
    else if (s=="He1+__" || s=="HeII_") {
      s="He1+"; pvar[s] = ftr+i; lvar[s] = firstion+ct; ct++;
      ions.push_back(s); nions++;
      els.push_back("He"); nels++;
    }
    else if (s=="He2+__" || s=="HEIII") {
      s="He2+"; pvar[s] = ftr+i; lvar[s] = firstion+ct; ct++;
      ions.push_back(s); nions++;
      bool fff=true; for (int j=0; j<nels; j++) if (els[j]=="He") fff=false;
      if (fff) {els.push_back("He"); nels++;} // unlikely, but you never know...
    }
    else if (s.substr(0,1)=="C") {
      bool fff=true; for (int j=0; j<nels; j++) if (els[j]=="C") fff=false;
      if (fff) {els.push_back("C"); nels++;}
      if (s.substr(0,2)=="C0") s="C0"; else s=s.substr(0,3);
      pvar[s] = ftr+i; lvar[s] = firstion+ct; ct++;
      ions.push_back(s); nions++;
    }
    else if (s.substr(0,1)=="N") {
      bool fff=true; for (int j=0; j<nels; j++) if (els[j]=="N") fff=false;
      if (fff) {els.push_back("N"); nels++;}
      if (s.substr(0,2)=="N0") s="N0"; else s=s.substr(0,3);
      pvar[s] = ftr+i; lvar[s] = firstion+ct; ct++;
      ions.push_back(s); nions++;
    }
    else if (s.substr(0,1)=="O") {
      bool fff=true; for (int j=0; j<nels; j++) if (els[j]=="O") fff=false;
      if (fff) {els.push_back("O"); nels++;}
      if (s.substr(0,2)=="O0") s="O0"; else s=s.substr(0,3);
      pvar[s] = ftr+i; lvar[s] = firstion+ct; ct++;
      ions.push_back(s); nions++;
    }
    else rep.error("Bad Tracer type!",s);
  }
  if ((ct+colour)!=len) rep.error("didn't get all tracers",len-ct-colour);
...
# else
  ...
  else rep.error("No known chemistry defined.",chemtype);
  ...
  for (int i=0;i<nels; i++) {
    ...
    ct=0;
    // count number of ions we are using which are associated with the current element.
    for (unsigned int v=0; v<ee[i]->ions.size(); v++) {
      for (int vv=0;vv<nions;vv++) {
	//	cout <<"ions: "<<ions[vv]<<" and els: "<<ee[i]->ions[v]<<"\n";
	...
	for (int vv=0;vv<nions;vv++) if (ions[vv]==ee[i]->ions[v]) {
	  cout <<"\t\tFor element "<<ee[i]->el<<" adding ion "<<ions[vv]<<" with index "<<ii[vv]->index<<"\n";
	  ee[i]->ion_indices[ct] = ii[vv]->index; ct++;
	}
      }
    }
    else {
  ...
  // At the moment I assume that im1 should always be tracked, as I don't explicitly
  // include the neutral stage as a variable usually, but I do want to track it.
  ...
  for (int i=0;i<nions;i++) {
    ...
    if (!higherstage) 
      {ii[i]->ip1 = ""; ii[i]->iip1 = 0;}
    else 
      {ii[i]->iip1 = &ion_props[ii[i]->ip1];}
    if (!lowerstage) {
      //ii[i]->im1 = ""; ii[i]->iim1 = 0;
      if (ii[i]->im1!="") ii[i]->iim1 = &ion_props[ii[i]->im1];
      else ii[i]->iim1 = 0;
    }
    else 
  ...
  for (int i=0;i<nions;i++) show_ion_struct(ii[i]);
  /*
			      ...
			      )
{
  ...
  // TimeUpdateMP does.
  double nf = P[lv_elec];
  for (int i=0;i<nels;i++) nf += ee[i]->numfrac;
  return P[lv_nh]*nf;
}
...
double MicroPhysics::neutral_fraction(const double *P, const ion_struct *i)
{
  ...
  for (int j=0; j<e->nspecies; j++) {
    //cout <<"j="<<j<<"\te->ion_indices[j] = "<< e->ion_indices[j]<<"\t: "<<P[e->ion_indices[j]]<<"\n";
				 ...
				 )
{
  ...
  else return p[PG]*m_p*0.61/kB/p[RO]; // assumes fully ionised gas with mu=1.22
}
				     ...
				     )
{
  ...
  if (ep.phot_ionisation) {
    p_local[lv_dtau] = 0.0;//p_local[lv_nh]*(1.0-p_local[lv_hp])*phot_xsection(ii[i])*path_length;
    tau_cell = 0.0;
  }
				     ...
				     )
{
  ...
  if (nions>0) {
    ...
    for (int i=0;i<nions;i++) p_out[ii[i]->pv_index] = max(p_local[ii[i]->index],min_if);
  }
  if (ep.phot_ionisation) {
					 ...
					 )
{
  ...
  *ttt = P[lv_eint]*(gamma-1.0)/kB/(2.0*P[lv_nh]); // fully ionised hydrogen
  // set output state vector -- only thing that changes is the pressure due to cooling.
  for (int v=0;v<nv_prim;v++) p_out[v] = p_in[v];
			       ...
			       )
{
  ...
  //cout <<"start  T= "<<Get_Temp(P) <<" and n_atoms= "<<Get_nTot(P)-P[lv_nh]*P[lv_elec]<<"\n";
  ...
  for (int i=0;i<nions;i++) {
    xi = ii[i]->index;
    if (P[xi]>=1.0001) {
      cout <<"ion: "<<ii[i]->ion<<" has i-frac="<<P[xi]<<"  ...setting to 1.\n";
      P[xi] = 1.0;
    }
  }
  if (P[lv_elec]>1.0001) {
  ...
  //  cout <<"finish T= "<<Get_Temp(P) <<" and n_atoms= "<<Get_nTot(P)-P[lv_nh]*P[lv_elec]<<"\n";
  // put updated state vector into p_out.
  *ttt = Get_Temp(P);
  //if (*ttt >1.e4) cout <<"\tT="<<*ttt<<" and n_atoms= "<<Get_nTot(P)-P[lv_nh]*P[lv_elec]<<"\n";
					 ...
					 )
{
  ...
  FUV_extinction        = 1.086*5.0e-22*tau2cell/6.3e-18;  // Extinction from Henney et al. 2009.
  ...
  *deltau = tau_cell/dt; // tau_cell is the integral of optical depth through cell over timestep.
				   ...
				   )
{
  ...
  double temp = (gamma-1.)*P[lv_eint]/kB/(2.0*P[lv_nh]); // for fully ionised hydrogen.
  R[lv_eint] = -cool->CoolingRate(temp,1.0,P[lv_nh],FUV_unattenuated_flux,FUV_extinction);
  if (R[lv_eint]>0) {
		       ...
		       )
{
  ...
  for (int i=0;i<nions;i++) {
    iip1 = ii[i]->iip1; iim1 = ii[i]->iim1;

    xi = ii[i]->index;
    temp=0.0;
    //        cout <<"ion :"<<ions[i]<<" temp="<<temperature<<" "<<lv_nh<<" "<<lv_eint<<" "<<lv_elec<<" "<<xi<<"\n";
	...
	if (iim1->index <0) temp *= neutral_fraction(P,iim1);
	else                temp *= P[iim1->index];
	R[xi] += temp; // coll.ion. to ion i adds to R[i]
	R[lv_eint] -= temp*iim1->ion_pot*ii[i]->e->numfrac; // reduces energy by the amount it took to ionise ion (i-1)
      }
      if (iip1) {
    ...
    // charge exchange
    if (iim1 && iim1->ch_ex) {
      // ch.ex. ionisation of iim1 adds to the rate R(ch-ex)*n_(H0)*x(iim1) [per sec]
      temp = charge_exchange_rate(temperature,iim1,1)*(1.0-P[lv_hp])*P[iim1->index]*P[lv_nh];
      R[xi] += temp;
    }
    if (ii[i]->ch_ex) {
      // can have recombination or ionisation exchange of current ion with H0 or H+
      R[xi] += charge_exchange_rate(temperature,ii[i],1)*(1.0-P[lv_hp])*P[xi]*P[lv_nh];
      R[xi] -= charge_exchange_rate(temperature,ii[i],0)*     P[lv_hp] *P[xi]*P[lv_nh];
    }
    if (iip1 && iip1->ch_ex) {
      // ch.ex. recombination of iip1 adds to the rate R(ch-ex)*n_(H+)*x(iip1) [per sec]
      R[xi] -= charge_exchange_rate(temperature,iip1,0)*      P[lv_hp] *P[iip1->index]*P[lv_nh];
    }
    */
	...
	temp  = Rad_Recomb_rate(temperature,ii[i])*P[lv_elec]*P[lv_nh]; // rate [1/s]
	R[xi] -= temp*P[xi]; // recomb from i to i-1 reduces fraction.
	//if (!ep.cooling || temperature<1.0e4) // avoid double counting!
	  //
	  // This is a bad approximation!!! UPDATE ME!!!
	  //
	R[lv_eint] -= temp*P[xi]*ii[i]->e->numfrac *kB*temperature/(gamma-1.0);
	...
	temp   = Rad_Recomb_rate(temperature,iip1 )*P[lv_elec]*P[lv_nh]; // rate [1/s]
	temp  *= P[iip1->index];
	...
	R[lv_dtau] = P[lv_nh]*(1.0-P[xi])*phot_xsection(ii[i])*path_length; // this is optical depth through cell.
	if (R[lv_dtau]>0.01)
	  temp = photons_in*(1.0 -exp(-R[lv_dtau]))/P[lv_nh]; // this is per sec.
	else 
	  temp = photons_in*phot_xsection(ii[i])*path_length*(1.0-P[xi]); // approx for 1-x<<1
	//  cout <<"\t\tphot rate: "<<temp<<"  and dtau_cell: "<<R[lv_dtau]<<" x="<<P[xi]<<"\n";
	//if (temp>1. && P[xi]>0.99) {
	//  cout <<"\t\tphot rate: "<<temp<<"  and dtau_cell: "<<R[lv_dtau]<<" x="<<P[xi]<<"\n";
	//  R[xi] = 1.0;
	//}
	R[xi] += temp;
	R[lv_eint] += temp*1.602e-12*2.4;// this adds in 2.4eV of energy per photo-ionisation.
	// have calculated the full rate for ionised hydrogen now, so the rate for dtau is simple:
  ...
  if (ep.cooling) {
    temp = cool->CoolingRate(temperature, P[lv_hp], P[lv_nh],FUV_unattenuated_flux,FUV_extinction);
    R[lv_eint] -= temp/P[lv_nh]; // (because we need to multiply by nH below...
  }
				   ...
				   )
{
  ...
  else rep.error("Bad ion in Coll_Ion_rate()",ii);

  //  if (ci->i == C3p) {
  //  cout <<"temp="<<ci->ion_pot/kB/temperature<<" and ci rate="<<A*(1.+PP*sqrt(temperature))*exp(K*log(temperature) -temperature)/(X+temperature)<<"  exp: "<<-temperature<<"\n";
  //}
  /*
  if    (ci->i ==C1p) 
				    ...
				    )
{
  ...
  if (T>0) err += Set_Eint(p_local,T);
  else T=Get_Temp(p_local);
  //  cout <<"Temp = "<<T<<"\n";
  if (nions<=0) {
  ...
  for (int i=0; i<nels; i++) {
    for (int j=0; j<ee[i]->nspecies; j++) {
      if (j==0) p_local[ee[i]->ion_indices[j]] = 1.0;
      else      p_local[ee[i]->ion_indices[j]] = 0.0;
    }
  }
  p_local[lv_elec] = 0.0;
  for (int i=0;i<nions; i++) {
    p_local[lv_elec] += p_local[ii[i]->index] *ii[i]->charge *ii[i]->e->numfrac;
  }
  ...
  for (int i=0; i<nels; i++) {
	...
	ifracs[e] *= (iii->g_stat/im1->g_stat) /p_local[lv_elec]/p_local[lv_nh];
	if (e>1) for (int eee=e; eee<ni; eee++) ifracs[eee] *= ifracs[eee-1];
      }
      double nfrac = 1.0;
...
void MicroPhysics::set_atomic_data()
{
  ...
  */
  ...
  i.i = C0; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["C"]); i.g_stat = 1; ion_props["C0" ] = i;
  i.ch_ex = true;

  i.ion = "C1+"; i.ip1 = "C2+"; i.im1 = "C0"; i.el  = "C";
  i.charge = 1; i.ion_pot = eV_per_Erg *24.4;
  i.i = C1p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["C"]); i.g_stat = 1; ion_props["C1+" ] = i;
  i.ch_ex = true;

  i.ion = "C2+"; i.ip1 = "C3+"; i.im1 = "C1+"; i.el  = "C";
  i.charge = 2; i.ion_pot = eV_per_Erg *47.9;
  i.i = C2p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["C"]); i.g_stat = 1; ion_props["C2+" ] = i;
  i.ch_ex = true;

  i.ion = "C3+"; i.ip1 = "C4+"; i.im1 = "C2+"; i.el  = "C";
  i.charge = 3; i.ion_pot = eV_per_Erg *64.5;
  i.i = C3p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["C"]); i.g_stat = 1; ion_props["C3+" ] = i;
  i.ch_ex = true;

  i.ion = "C4+"; i.ip1 = "C5+"; i.im1 = "C3+"; i.el  = "C";
  i.charge = 4; i.ion_pot = eV_per_Erg *392.1;
  i.i = C4p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["C"]); i.g_stat = 1; ion_props["C4+" ] = i;
  i.ch_ex = true;

  i.ion = "C5+"; i.ip1 = "C6+"; i.im1 = "C4+"; i.el  = "C";
  i.charge = 5; i.ion_pot = eV_per_Erg *490.0;
  i.i = C5p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["C"]); i.g_stat = 1; ion_props["C5+" ] = i;
  i.ch_ex = false;

  i.ion = "C6+"; i.ip1 = ""; i.im1 = "C5+"; i.el  = "C";
  i.charge = 6; i.ion_pot = -1.e99;
  i.i = C6p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["C"]); i.g_stat = 1; ion_props["C6+" ] = i;
  i.ch_ex = false;

  // nitrogen: N, N1+, N2+, N3+, N4+, N5+, N6+, N7+
  i.ion = "N0"; i.ip1 = "N1+"; i.im1 = ""; i.el  = "N";
  i.charge = 0; i.ion_pot = eV_per_Erg *14.5;
  i.i = N0; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N0" ] = i;
  i.ch_ex = true;

  i.ion = "N1+"; i.ip1 = "N2+"; i.im1 = "N0"; i.el  = "N";
  i.charge = 1; i.ion_pot = eV_per_Erg *29.6;
  i.i = N1p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N1+" ] = i;
  i.ch_ex = true;

  i.ion = "N2+"; i.ip1 = "N3+"; i.im1 = "N1+"; i.el  = "N";
  i.charge = 2; i.ion_pot = eV_per_Erg *47.5;
  i.i = N2p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N2+" ] = i;
  i.ch_ex = false;

  i.ion = "N3+"; i.ip1 = "N4+"; i.im1 = "N2+"; i.el  = "N";
  i.charge = 3; i.ion_pot = eV_per_Erg *77.5;
  i.i = N3p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N3+" ] = i;
  i.ch_ex = false;

  i.ion = "N4+"; i.ip1 = "N5+"; i.im1 = "N3+"; i.el  = "N";
  i.charge = 4; i.ion_pot = eV_per_Erg *97.9;
  i.i = N4p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N4+" ] = i;
  i.ch_ex = false;

  i.ion = "N5+"; i.ip1 = "N6+"; i.im1 = "N+4"; i.el  = "N";
  i.charge = 5; i.ion_pot = eV_per_Erg *552.1;
  i.i = N5p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N5+" ] = i;
  i.ch_ex = false;

  i.ion = "N6+"; i.ip1 = "N7+"; i.im1 = "N5+"; i.el  = "N";
  i.charge = 6; i.ion_pot = eV_per_Erg *667.0;
  i.i = N6p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N6+" ] = i;
  i.ch_ex = false;

  i.ion = "N7+"; i.ip1 = ""; i.im1 = "N6+"; i.el  = "N";
  i.charge = 7; i.ion_pot = -1.e99;
  i.i = N7p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["N"]); i.g_stat = 1; ion_props["N7+" ] = i;
  i.ch_ex = false;

  // oxygen: O, O1+, O2+, O3+, O4+, O5+, O6+, O7+, O8+, 
  i.ion = "O0"; i.ip1 = "O1+"; i.im1 = ""; i.el  = "O";
  i.charge = 0; i.ion_pot = eV_per_Erg *13.6;
  i.i = O0; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O0" ] = i;
  i.ch_ex = true;
  
  i.ion = "O1+"; i.ip1 = "O2+"; i.im1 = "O0"; i.el  = "O";
  i.charge = 1; i.ion_pot = eV_per_Erg *35.1;
  i.i = O1p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O1+" ] = i;
  i.ch_ex = true;

  i.ion = "O2+"; i.ip1 = "O3+"; i.im1 = "O1+"; i.el  = "O";
  i.charge = 2; i.ion_pot = eV_per_Erg *54.9;
  i.i = O2p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O2+" ] = i;
  i.ch_ex = false;

  i.ion = "O3+"; i.ip1 = "O4+"; i.im1 = "O2+"; i.el  = "O";
  i.charge = 3; i.ion_pot = eV_per_Erg *77.4;
  i.i = O3p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O3+" ] = i;
  i.ch_ex = false;

  i.ion = "O4+"; i.ip1 = "O5+"; i.im1 = "O3+"; i.el  = "O";
  i.charge = 4; i.ion_pot = eV_per_Erg *113.9;
  i.i = O4p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O4+" ] = i;
  i.ch_ex = false;

  i.ion = "O5+"; i.ip1 = "O6+"; i.im1 = "O4+"; i.el  = "O";
  i.charge = 5; i.ion_pot = eV_per_Erg *138.1;
  i.i = O5p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O5+" ] = i;
  i.ch_ex = false;

  i.ion = "O6+"; i.ip1 = "O7+"; i.im1 = "O5+"; i.el  = "O";
  i.charge = 6; i.ion_pot = eV_per_Erg *739.3;
  i.i = O6p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O6+" ] = i;
  i.ch_ex = false;

  i.ion = "O7+"; i.ip1 = "O8+"; i.im1 = "O6+"; i.el  = "O";
  i.charge = 7; i.ion_pot = eV_per_Erg *871.4;
  i.i = O7p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O7+" ] = i;
  i.ch_ex = false;

  i.ion = "O8+"; i.ip1 = ""; i.im1 = "O7+"; i.el  = "O";
  i.charge = 8; i.ion_pot = -1.e99;
  i.i = O8p; i.index = -1; i.pv_index = -1;
  i.e = &(el_props["O"]); i.g_stat = 1; ion_props["O8+" ] = i;
  i.ch_ex = false;
