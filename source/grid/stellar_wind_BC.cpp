...
)
{
    ...
    if (ndim == 2 && coordsys == COORD_CRT) {
        ...
        wc->p[PG] *= exp(
            (gamma - 1.0) * log(2.0 * M_PI * WS->Rstar * WS->Vinf / WS->Mdot));
        ...
        if (set_rho) {
            ...
            wc->p[PG] *=
                exp((gamma - 1.0)
                    * log(4.0 * M_PI * WS->Rstar * WS->Rstar * WS->Vinf
                          / WS->Mdot));
            wc->p[PG] *= exp((gamma)*log(wc->p[RO]));
        }
    }
    ...
    if (WS->Hplus >= 0) {
        if (WS->Tw < 1.0e4)
            ...
            WS->tracers[WS->iHplus] =
                1.0e-10 + (WS->Tw - 1.0e4) * (1.0 - 1.0e-10) / 0.5e4;
    }
...
)
{
    ...
    struct wind_source* WS = wlist[id];
#ifdef TESTING
    cout << "updating source " << id << " which has " << WS->ncell
         << " cells.\n";
#endif
...
)
{
    ...
    //  X_H  X_He  X_C  X_N  X_O  X_Z  X_D
    double time = 0.0, mass = 0.0, lumi = 0.0, teff = 0.0, radi = 0.0,
           mdot = 0.0, vrot = 0.0, vcrt = 0.0, vinf = 0.0;
    ...
    while ((rval = fgets(line, 512, wf)) != 0) {
        sscanf(
            ...
            &time, &mass, &lumi, &teff, &mdot, &vrot, &vcrt, &vinf, &xh, &xhe,
            &xc, &xn, &xo, &xz, &xd);
        // cout.precision(16);
#ifdef TESTING
    ...
    const double gamma)
{
    ...
    if (wd->i_XHe >= 0) {
        interpolate.root_find_linear_vec(
            wd->time_evo, wd->X_He_evo, t_now, xhe);
