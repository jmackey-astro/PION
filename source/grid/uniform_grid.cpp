...
// BC_nbc(Nbc)
{
    ...
    do {
        ...
        dom         = true;
        for (int v = 0; v < G_ndim; v++) {
            if (c->pos[v] < Sim_ixmin[v] || c->pos[v] > Sim_ixmax[v])
                dom = false;
        }
...
enum direction UniformGrid::OppDir(enum direction dir)
{
    ...
    if (dir == XP)
        ...
        rep.error("Bad direction given to OppDir", dir);
        return (NO);
    }
}
...
)
{
    ...
    for (int i = 0; i < G_ndim; i++)
        temp += pow(
            static_cast<double>(CI.get_ipos(c1, i) - CI.get_ipos(c2, i)), 2.0);
    return sqrt(temp);
}
...
)
{
    if (a == Zcyl)
        ...
        rep.error(
            "Bad axis for uniform_grid_cyl::idifference_vertex2cell()", a);
...
#endif
    return (
        (R_com(c, G_dx) - G_xmin[Rsph]) / CI.phys_per_int() + G_ixmin[Rsph]);
}
...
)
{
    if (a == Rsph)
        ...
        rep.error(
            "Bad axis for uniform_grid_sph::idifference_vertex2cell()", a);
