...
)
{
    ...
    if (SimPM.coord_sys == COORD_CRT)
        *grid = new UniformGrid(
            ...
            SimPM.Xmax, SimPM.NG, SimPM.Xmin, SimPM.Xmax, SimPM.Xmin,
            SimPM.Xmax);
    ...
    if (*grid == 0)
        rep.error(
            "(setup_fixed_grid::setup_grid) Couldn't assign data!", *grid);
#ifdef TESTING
...
)
{
    ...
    if (SimPM.EP.cooling && !SimPM.EP.chemistry) {
        ...
        if (mptype == "MPv1") {
            ...
            if (have_set_MP) rep.error("MP already initialised", mptype);
            MP = new MPv1(
                SimPM.nvar, SimPM.ntracer, SimPM.tracers, &(SimPM.EP),
                &(SimPM.RS));
            cout
...
#error "No timestep-limiting is defined in source/defines/functionality_flags.h"
    ...
    for (int isrc = 0; isrc < SimPM.RS.Nsources; isrc++) {
        if (SimPM.RS.sources[isrc].type == RT_SRC_SINGLE
            ...
            err = MP->set_multifreq_source_properties(
                &SimPM.RS.sources[isrc], data);
        }
    }
...
)
{
    ...
    for (int isrc = 0; isrc < Nevo; isrc++) {
        ...
        while ((rval = fgets(line, 512, infile)) != 0) {
            // printf("rval= %s\n",rval);
            sscanf(
                line, "   %lE   %lE %lE %lE %lE %lE %lE", &t1, &t2, &t3, &t4,
                &t5, &t6, &t7);
            // cout.precision(16);
            // cout <<t1 <<"  "<<t2  <<"  "<< t3  <<"  "<< t4 <<"  "<< t5 <<"
            // "<< t6
    ...
    const int)
{
    ...
    if (grid->BC_bd.size() == 0) {
        ...
        for (int b = 0; b < len; b++) {
            ...
            if (b < 2 * par.ndim)
                bd->depth =
                    grid->boundary_depth(static_cast<enum direction>(b));
    ...
    if (i < len) {
#ifdef TESTING
        cout << "Got " << i << " boundaries, but have " << len
             << " boundaries.\n";
