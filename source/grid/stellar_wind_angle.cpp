...
)
{
  ...
  //
  add_rotating_source(
      pos, rad, type, mdot, 0.0, vinf, vrot, vcrt, Twind, rstar, Bstar, trv,
...
)
{
  ...
  switch (type) {
    case WINDTYPE_ANGLE:
      spdlog::debug("\tAdding rotating wind source as id={}", ws->id);
      break;
    default:
      spdlog::error(
...
#ifndef NDEBUG
  spdlog::debug(
      "ws->dpos : {}", std::vector<double>(ws->dpos, ws->dpos + ndim));
#endif
