  ...
  for (int i = 0; i < npts_omega; i++) {
    for (int j = 0; j < npts_Teff; j++) {
      ...
      if (!isfinite(delta_vec[i][j])) {
        cout << "infinite delta!!! " << i << "  " << j << "  " << omega_vec[i]
             << "  " << Teff_vec[j] << delta_vec[i][j] << "\n";
...
{
  return fn_alpha(omega, theta, Teff) * pow_fast(1.0 - omega * sin(theta), c_xi)
         * sin(theta);
}
...
)
{
  ...
  if (set_rho) {
    wc->p[RO] = fn_density_interp(
        std::min(0.9999, WS->v_rot / WS->vcrit), WS->Vinf, WS->Mdot, wc->dist,
        wc->theta, WS->Tw);
    //
