...
// (also added fn to stellar_wind - probably already inherited from
// stellar_wind_evolution?)
double stellar_wind_angle::pow_fast(double a, double b)
{
    return exp(b * log(a));
}
...
void stellar_wind_angle::setup_tables()
{
    ...
    for (int k = 0; k < npts_theta; k++) {
        if (k <= 4)
            ...
            theta_vec[k] =
                (theta_mid
                 + (k - 4) * ((theta_max - theta_mid) / (npts_theta - 5)))
                * (pconst.pi() / 180.0);
    }
    ...
    for (int i = 0; i < npts_omega; i++) {
        for (int j = 0; j < npts_Teff; j++) {
            ...
            if (!isfinite(delta_vec[i][j])) {
                cout << "infinite delta!!! " << i << "  " << j << "  "
                     << omega_vec[i] << "  " << Teff_vec[j] << delta_vec[i][j]
                     << "\n";
    ...
    for (int i = 0; i < npts_omega; i++) {
        for (int j = 0; j < npts_theta; j++) {
            for (int k = 0; k < npts_Teff; k++) {
                alpha_vec[i][j][k] =
                    fn_alpha(omega_vec[i], theta_vec[j], Teff_vec[k]);
            }
        }
    }
...
)
{
    return fn_alpha(omega, theta, Teff)
           * pow_fast(1.0 - omega * sin(theta), c_xi) * sin(theta);
}
...
)
{
    double ans = (omega / (22.0 * pconst.sqrt2() * beta(Teff))) * sin(theta)
                 * pow_fast(1.0 - omega * sin(theta), -c_gamma);
...
)
{
    ...
    return std::max(0.5e5, v_inf * pow_fast(1.0 - omega * sin(theta), c_gamma));
}
...
)
{
    ...
    if (set_rho) {
        wc->p[RO] = fn_density_interp(
            std::min(0.9999, WS->v_rot / WS->vcrit), WS->Vinf, WS->Mdot,
            wc->dist, wc->theta, WS->Tw);
        //
...
)
{
    if (type != WINDTYPE_ANGLE) {
        rep.error(
            "Bad wind type for evolving stellar wind (rotating star)!", type);
    }
    //
    ...
    double mdot = 0.0, vinf = 0.0, Twind = 0.0, vrot = 0.0, rstar = 0.0,
           vcrt = 0.0;
    double xh = 0.0, xhe = 0.0, xc = 0.0, xn = 0.0, xo = 0.0, xz = 0.0,
           xd = 0.0;
    ...
    if (((t_now + temp->update_freq) > temp->tstart
        ...
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->R_evo, t_now, rstar);

        // get tracer values for elements.
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->X_H_evo, t_now, xh);
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->X_He_evo, t_now, xhe);
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->X_C_evo, t_now, xc);
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->X_N_evo, t_now, xn);
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->X_O_evo, t_now, xo);
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->X_Z_evo, t_now, xz);
        interpolate.root_find_linear_vec(
            temp->time_evo, temp->X_D_evo, t_now, xd);
    ...
    //
    add_rotating_source(
        pos, rad, type, mdot, vinf, vrot, vcrt, Twind, rstar, Bstar, trv);
    temp->ws = wlist.back();
...
)
{
    ...
    if (coordsys == COORD_SPH) {
        if (!pconst.equalD(ws->dpos[Rsph], 0.0))
            rep.error(
                "Spherical symmetry but source not at origin!", ws->dpos[Rsph]);
    }
