...
)
{
    ...
    for (int l = 0; l < SimPM.grid_nlevels; l++) {
        ...
        if (grid[l] == 0) rep.error("(setup_grid_NG_MPI::setup_grid)", grid[l]);
...
#ifdef TESTING
        cout << "NG-MPI l=" << l
             << ": updating evolving RT sources from setup_raytracing.\n";
#endif
...
)
{
    ...
    switch (typeOfFile) {
        ...
        case 5:  // Start from Silo snapshot.
            dataio =
                new dataio_silo_utility(par, "DOUBLE", &(par.levels[0].MCMD));
            break;
#endif  // if SILO
