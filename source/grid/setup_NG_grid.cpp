...
)
{
    ...
    for (int i = 0; i < SimPM.grid_nlevels; i++) {
        ...
        if (i == 0) {
            ...
            for (int v = 0; v < MAX_DIM; v++) {
                if (SimPM.NG_refine[v] == 1) {
                    SimPM.levels[i].Range[v] =
                        0.5 * SimPM.levels[i - 1].Range[v];
                    SimPM.levels[i].Xmin[v] =
                        0.5
                        * (SimPM.levels[i - 1].Xmin[v] + SimPM.NG_centre[v]);
                    SimPM.levels[i].Xmax[v] =
                        0.5
                        * (SimPM.levels[i - 1].Xmax[v] + SimPM.NG_centre[v]);
                }
                else {
    ...
    for (int l = 0; l < SimPM.grid_nlevels; l++) {
        ...
        if (SimPM.coord_sys == COORD_CRT)
            grid[l] = new UniformGrid(
                ...
                SimPM.levels[l].Xmin, SimPM.levels[l].Xmax, SimPM.Xmin,
                SimPM.Xmax);
        ...
        if (grid[l] == 0)
            rep.error(
                "(setup_NG_grid::setup_grid) Couldn't assign data!", grid[l]);
    ...
    for (int l = 0; l < par.grid_nlevels; l++) {
        ...
        do {
            ...
            for (int v = 0; v < par.ndim; v++) {
                if (c->pos[v] < sxmin[v] || c->pos[v] > sxmax[v])
                    c->isleaf = false;
            }
            if (l < par.grid_nlevels - 1) {
                ...
                for (int v = 0; v < par.ndim; v++) {
                    if (c->pos[v] > lxmax[v] || c->pos[v] < lxmin[v])
                        notleaf = false;
                }
...
)
{
    ...
    if (l > 0) {
        for (int i = 0; i < par.ndim; i++) {
            if (!pconst.equalD(
                    par.levels[l - 1].Xmin[i], par.levels[l].Xmin[i])) {
#ifdef TESTING
                cout << "reassigning neg. bc for axis " << i
                     << " to COARSE_TO_FINE\n";
#endif
            ...
            if (!pconst.equalD(
                    par.levels[l - 1].Xmax[i], par.levels[l].Xmax[i])) {
#ifdef TESTING
                cout << "reassigning pos. bc for axis " << i
                     << " to COARSE_TO_FINE\n";
#endif
    ...
    if (l < par.grid_nlevels - 1) {
        ...
        do {
            ...
            CI.get_dpos(c, cpos);
            for (int v = 0; v < par.ndim; v++) {
                if (cpos[v] < cxmin[v] || cpos[v] > cxmax[v])
                    within_child = false;
            }
            if (within_child) {
                c->isbd = true;
