    ...
    first_tr(ftr)
{
    ...
    if (XX != 0 || YY != 1 || ZZ != 2)
        rep.error(
            "axes enums not what is expected, bad stuff will happen.", XX);
...
)
{
    ...
    if (source->src_on_grid) {
        rep.error(
            "Can't have source on grid for parallel rays!", source->s->id);
    }
#endif  // RT_TESTING
...
)
{
    ...
    switch (source->s->opacity_src) {
        ...
        case RT_OPACITY_TRACER:
            ...
            cell_col[0] =
                ds * c->Ph[RO] * c->Ph[source->s->opacity_var + first_tr];
            break;
        ...
        case RT_OPACITY_HALPHA:
            ...
            cell_col[0] = cell_col[0]
                          + exp(-(c->Ph[RO] * mpptr->get_X_H() / pconst.m_p())
                                * 5.0e-22 * ds)
                                * (col2cell[0] - cell_col[0]);
            break;
...
)
{
    ...
    if (!source->src_on_grid) {
        for (int i = 0; i < ndim; i++) {
            ...
            //
            if (source->ipos[a] <= gridptr->iXmin_all(a))
                src_off_grid[a] = negdir;
            if (source->ipos[a] >= gridptr->iXmax_all(a))
                src_off_grid[a] = posdir;
            ...
            if (in_my_oct == false) {
...
#ifdef RT_TESTING
                cout << "octant " << oct
                     << ", source is off grid in this octant";
                ...
                if (oct == OCT1)
                    ...
                    if (startcell[OCT1] != 0) {
                        ...
                        rep.error(
                            "RayTracing3D::RayTraceSource logic error OCT2",
                            oct);
                    }
                }
                else if (oct == OCT4) {
                    if (startcell[OCT1] != 0) {
                        ...
                        rep.error(
                            "RayTracing3D::RayTraceSource logic error OCT4",
                            oct);
                    }
                }
                else if (oct == OCT3) {
                    ...
                    // plane must be (1,2,3,4)
                        ...
                        rep.error(
                            "RayTracing3D::RayTraceSource logic error OCT3",
                            oct);
                    }
                }
                    ...
                    if (startcell[OCT1] != 0) {
                        ...
                        rep.error(
                            "RayTracing3D::RayTraceSource logic error OCT5",
                            oct);
                    }
                }
                    ...
                    // plane must be (1,2,3,4)
                        ...
                        rep.error(
                            "RayTracing3D::RayTraceSource logic error OCT6",
                            oct);
                    }
                }
                    ...
                    // plane must be (1,2,3,4)
                        ...
                        rep.error(
                            "RayTracing3D::RayTraceSource logic error OCT8",
                            oct);
                    }
                }
                else if (oct == OCT7) {
                    if (startcell[OCT6] != 0) {
                        ...
                        rep.error(
                            "RayTracing3D::RayTraceSource logic error OCT7",
                            oct);
                    }
                }
                else
...
#ifdef RT_TESTING
        if (Nc[0] < 0.0) {
            cout << "column is negative:" << Nc[0]
                 << " coming from off grid???\n";
            CI.print_cell(c);
            CI.print_cell(ngb);
    ...
    const rad_source* src, cell* c, double Nc[], double* ds)
{
    ...
    //
    *ds = gridptr->DX()
          * sqrt(1. + deltas[0] * deltas[0] + deltas[1] * deltas[1]);
    double idx   = gridptr->idx();  // cell size in integer units.
    double idxo2 = 0.5 * idx;
    ...
    if (dx[o[0]] < idx) {
        ...
        if (dx[o[0]] < 5 * idx) {
            if (dx[o[0]] == 3 * idxo2) {
                ...
                double one_over_r2 =
                    1.0 / (dx[o[0]] * dx[o[0]] + dx[o[1]] * dx[o[1]]);
