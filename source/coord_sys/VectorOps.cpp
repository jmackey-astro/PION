    ...
    const cell* cpt, const int sv, const int var, class GridBaseClass* grid)
{
    ...
    for (int i = 0; i < 2 * VOnd; i++)
        if (!grid->NextPt(cpt, static_cast<direction>(i)))
            rep.error(
                "VectorOps_Cart::max_grad_abs: Some neighbour cells don't exist",
                i);
#endif  // TESTING
    ...
    pion_flt* grad)
{
    ...
    switch (sv) {
        case 0:  // Use vector c->P
            grad[0] =
                (grid->NextPt(c, XP)->P[var] - grid->NextPt(c, XN)->P[var])
                / (2. * VOdx);
            if (VOnd > 1)
                grad[1] =
                    (grid->NextPt(c, YP)->P[var] - grid->NextPt(c, YN)->P[var])
                    / (2. * VOdx);
            if (VOnd > 2)
                grad[2] =
                    (grid->NextPt(c, ZP)->P[var] - grid->NextPt(c, ZN)->P[var])
                    / (2. * VOdx);
            break;
        case 1:  // Use Vector c-Ph
            grad[0] =
                (grid->NextPt(c, XP)->Ph[var] - grid->NextPt(c, XN)->Ph[var])
                / (2. * VOdx);
            if (VOnd > 1)
                grad[1] = (grid->NextPt(c, YP)->Ph[var]
                           - grid->NextPt(c, YN)->Ph[var])
                          / (2. * VOdx);
            if (VOnd > 2)
                grad[2] = (grid->NextPt(c, ZP)->Ph[var]
                           - grid->NextPt(c, ZN)->Ph[var])
                          / (2. * VOdx);
            break;
        default:
            rep.error(
                "Don't know what state vector to use for calculating gradiend.",
                sv);
    }
    return;
}
    ...
    switch (sv) {
        ...
        case 1:  // Use Vector c-Ph
            for (int v = 0; v < VOnd; v++) {
                ...
                if (!isfinite(divv)) {
                    cout << "divv[" << v << "] = ";
                    cout << (ngb[pdir]->Ph[var[v]] - ngb[ndir]->Ph[var[v]])
                                / dx[v];
                    cout << ": dx=" << dx[v] << " cn=" << ngb[ndir]
                         << ", cp=" << ngb[pdir];
    ...
    pion_flt* ans)
{
    ...
    for (int i = 0; i < 2 * VOnd; i++)
        if (!grid->NextPt(c, static_cast<direction>(i)))
            rep.error(
                "VectorOps_Cart::Curl: Some neighbour cells don't exist", i);
#endif  // TESTING
    if (!c->isgd) {
    ...
    switch (sv) {
        case 0:  // Use vector c->P
            ...
            if (VOnd > 2) {  // Theta-dir, need scale factor 1/R
                dT = (CI.get_dpos(grid->NextPt(c, TPcyl), Tcyl)
                      - CI.get_dpos(c, Tcyl))
                     * R_com(c, dR);
                temp = fabs(grid->NextPt(c, TPcyl)->P[var] - c->P[var]) / dT;
                if (temp > grad) grad = temp;
                dT = (CI.get_dpos(c, Tcyl)
    ...
    switch (sv) {
        case 0:  // Use vector c->P
            ...
            if (VOnd > 1) {
                ...
                grad[1] =
                    (cp->P[var] - cn->P[var]) / (R_com(cp, dR) - R_com(cn, dR));
            }
            if (VOnd > 2) {
        ...
        case 1:  // Use Vector c-Ph
            ...
            if (VOnd > 1) {
                cn      = grid->NextPt(c, RNcyl);
                cp      = grid->NextPt(c, RPcyl);
                grad[1] = (cp->Ph[var] - cn->Ph[var])
                          / (R_com(cp, dR) - R_com(cn, dR));
            }
            if (VOnd > 2) {
                cn      = grid->NextPt(c, TNcyl);
                cp      = grid->NextPt(c, TPcyl);
    ...
    switch (sv) {
        case 0:  // Use vector c->P
            ...
            if (VOnd > 2) {  // d(V_theta)/(R*dtheta)
                cn = ngb[TNcyl];
                cp = ngb[TPcyl];
                dT = (CI.get_dpos(cp, Tcyl) - CI.get_dpos(cn, Tcyl))
                     * R_com(c, dR);
    ...
    class GridBaseClass* grid)
{
    if (OA == OA1) {  // 1st order, constant data.
        ...
        rep.error("SetEdgeState OOA -- only 1st and 2nd order", OA);
