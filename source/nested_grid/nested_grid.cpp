...
#ifdef TESTING
  cout <<"nested_grid Destructor:\tdone.\n";
#endif
} // Destructor
...
int nested_grid::assign_grid_structure()
{
#ifdef TESTING
...
int nested_grid::allocate_grid_data()
{
...
enum direction nested_grid::OppDir(enum direction dir)
{
...
class cell* nested_grid::FirstPt()
{
  ///
...
class cell* nested_grid::FirstPt_All()
{
  ///
  /// First point is always the at the negative corner of the grid in
  /// all directions.
...
class cell* nested_grid::LastPt_All()
{
  return(G_lpt_all);
} // LastPt
...
int nested_grid::SetupBCs(
      class SimParams &par  ///< List of simulation params (including BCs)
      )
{
  if (BC_setBCtypes(par) !=0)
    rep.error("nested_grid::setupBCs() Failed to set types of BC",1);
  //
...
int nested_grid::TimeUpdateInternalBCs(
        const double simtime,   ///< current simulation time
        const int cstep,
        const int maxstep
        )
...
int nested_grid::TimeUpdateExternalBCs(
      const double simtime,   ///< current simulation time
      const int cstep,
      const int maxstep
      )
...
int nested_grid::BC_assign_PERIODIC(  boundary_data *b)
{
...
int nested_grid::BC_update_PERIODIC(
      struct boundary_data *b,
      const int cstep,
      const int maxstep
      )
...
int nested_grid::BC_assign_OUTFLOW(   boundary_data *b)
{
  //
  ...
  do{
    ...
    if (G_eqntype==EQGLM) {
      ...
      if ((*bpt)->isedge == -1) {
        ...
        //
        (*bpt)->P[SI]  = -NextPt(temp,ondir)->P[SI];
        (*bpt)->Ph[SI] = -NextPt(temp,ondir)->Ph[SI];
      }
...
int nested_grid::BC_update_OUTFLOW(
      struct boundary_data *b,
      const int cstep,
      const int maxstep
      )
{
  ...
  for (c=b->data.begin(); c!=b->data.end(); ++c) {
    ...
    //
    gc = (*c)->npt;
    for (int v=0;v<G_nvar;v++) (*c)->Ph[v] = gc->Ph[v];
    for (int v=0;v<G_nvar;v++) (*c)->dU[v] = 0.;
    if (cstep==maxstep) {
      for (int v=0;v<G_nvar;v++) (*c)->P[v] = gc->P[v];
    }
...
int nested_grid::BC_assign_ONEWAY_OUT(boundary_data *b)
{
  //
...
int nested_grid::BC_update_ONEWAY_OUT(
      struct boundary_data *b,
      const int cstep,
      const int maxstep
      )
...
int nested_grid::BC_assign_INFLOW(    boundary_data *b)
{
  enum direction ondir  = b->ondir;
...
int nested_grid::BC_update_INFLOW(
      struct boundary_data *b,
...
int nested_grid::BC_assign_REFLECTING(boundary_data *b)
{
  enum direction offdir = b->dir;
  enum direction ondir  = b->ondir;
...
int nested_grid::BC_update_REFLECTING(
      struct boundary_data *b,
      const int cstep,
      const int maxstep
      )
...
int nested_grid::BC_assign_FIXED(     boundary_data *b)
{
  cout <<" nested_grid::BC_assign_FIXED starting\n";
  enum direction ondir  = b->ondir;
  if (b->data.empty()) {
...
int nested_grid::BC_update_FIXED(
      struct boundary_data *b,
...
int nested_grid::BC_assign_JETBC(     boundary_data *b)
{
  if (!JP.jetic) {
...
int nested_grid::BC_update_JETBC(      struct boundary_data *b,
               const int,
               const int
               )
{
...
int nested_grid::BC_assign_JETREFLECT(boundary_data *b)
{
  enum direction offdir = b->dir;
  enum direction ondir  = b->ondir;
  if (b->data.empty()) {
...
int nested_grid::BC_update_JETREFLECT(
      struct boundary_data *b,
      const int cstep,
      const int maxstep
      )
...
int nested_grid::BC_assign_DMACH(
...
int nested_grid::BC_update_DMACH(
      const double simtime,   ///< current simulation time
      struct boundary_data *b,
      const int cstep,
      const int maxstep
...
int nested_grid::BC_assign_DMACH2(    boundary_data *b) 
{
#ifdef TESTING
  ...
  do {
    ...
    // rewrite the function in the parallel nested grid.
    //
...
int nested_grid::BC_update_DMACH2(
      struct boundary_data *b,
      const int,
      const int
      )
...
int nested_grid::BC_assign_RADSHOCK(  boundary_data *b)
{
  //
...
int nested_grid::BC_update_RADSHOCK(   struct boundary_data *b,
               const int cstep,
               const int maxstep
               )
{
...
int nested_grid::BC_assign_RADSH2(     boundary_data *b)
{
  //
  // Initial checks and memory allocation.
  //
...
int nested_grid::BC_update_RADSH2(   struct boundary_data *b,
             const int cstep,
             const int maxstep
             )
{
...
//
int nested_grid::BC_assign_STWIND(
      const double simtime,   ///< current simulation time
...
int nested_grid::BC_assign_STWIND_add_cells2src(
...
//
int nested_grid::BC_update_STWIND(
        const double simtime,   ///< current simulation time
...
int nested_grid::BC_assign_STARBENCH1(boundary_data *b)
{
  //
...
int nested_grid::BC_update_STARBENCH1(
        struct boundary_data *b,
        const int cstep,
        const int maxstep
        )
...
int nested_grid::BC_printBCdata(boundary_data *b)
{
  list<cell*>::iterator c=b->data.begin();
  for (c=b->data.begin(); c!=b->data.end(); ++c) {  
    CI.print_cell(*c);
...
void nested_grid::BC_deleteBoundaryData()
{
#ifdef TESTING
...
///
double nested_grid::distance_cell2cell(
...
/// two units).
///
double nested_grid::idistance_cell2cell(
      const cell *c1, ///< cell 1
      const cell *c2  ///< cell 2
      )
{
...
///
double nested_grid::distance_vertex2cell(
...
///
double nested_grid::difference_vertex2cell(
...
///
double nested_grid::idistance_vertex2cell(
...
/// the *cell* coordinate minus the *vertex* coordinate.
///
double nested_grid::idifference_vertex2cell(
      const int *v,  ///< vertex (integer)
      const cell *c, ///< cell
      const axes a   ///< Axis to calculate.
      )
...
///
double nested_grid::idifference_cell2cell(
              const cell *c1, ///< cell 1
              const cell *c2, ///< cell 2
              const axes a    ///< Axis.
              )
...
/// Constructor
///
nested_grid_cyl::uniform_grid_cyl(
...
{
#ifdef TESTING
  cout <<"Setting up cylindrical nested grid with";
...
double nested_grid_cyl::iR_cov(const cell *c)
{
  //
...
/// physical units.
///
double nested_grid_cyl::distance(
          const double *p1, ///< position 1 (physical)
          const double *p2  ///< position 2 (physical)
          )
{
...
/// integer units (but obviously the answer is not an integer).
///
double nested_grid_cyl::idistance(
          const int *p1, ///< position 1 (integer)
          const int *p2  ///< position 2 (integer)
          )
{
...
/// Result returned in physical units (e.g. centimetres).
///
double nested_grid_cyl::distance_cell2cell(
      const cell *c1, ///< cell 1
      const cell *c2  ///< cell 2
      )
{
...
/// two units).
///
double nested_grid_cyl::idistance_cell2cell(
      const cell *c1, ///< cell 1
      const cell *c2  ///< cell 2
      )
{
...
/// geometry).  Here both input and output are physical units.
///
double nested_grid_cyl::distance_vertex2cell(
          const double *v, ///< vertex (physical)
          const cell *c    ///< cell
          )
{
...
/// the *cell* coordinate minus the *vertex* coordinate.
///
double nested_grid_cyl::difference_vertex2cell(
      const double *v,  ///< vertex (double)
      const cell *c, ///< cell
      const axes a   ///< Axis to calculate.
      )
...
/// geometry).  Here both input and output are code-integer units.
///
double nested_grid_cyl::idistance_vertex2cell(
          const int *v, ///< vertex (integer)
          const cell *c ///< cell
          )
{
...
/// the *cell* coordinate minus the *vertex* coordinate.
///
double nested_grid_cyl::idifference_vertex2cell(const int *v,  ///< vertex (integer)
             const cell *c, ///< cell
             const axes a   ///< Axis to calculate.
             )
{
  if      (a==Zcyl)
    ...
    rep.error("Bad axis for nested_grid_cyl::idifference_vertex2cell()",a);
...
/// It returns *cell2* coordinate minus *cell1* coordinate.
///
double nested_grid_cyl::idifference_cell2cell(
              const cell *c1, ///< cell 1
              const cell *c2, ///< cell 2
              const axes a    ///< Axis.
              )
{
  if      (a==Zcyl)
    ...
    rep.error("Bad axis for nested_grid_cyl::idifference_cell2cell()",a);
  return -1.0;
}
...
int nested_grid_cyl::BC_assign_STWIND_add_cells2src(
        const int id, ///< source id
        struct boundary_data *b
        )
{
...
nested_grid_sph::uniform_grid_sph(
    int nd,         ///< ndim, length of position vector.
    int nv,         ///< nvar, length of state vectors.
    int eqt,        ///< eqntype, which equations we are using (needed by BCs).
    int Nbc,        ///< Number of boundary cells to use.
...
{
#ifdef TESTING
  cout <<"Setting up spherical nested grid with";
  cout <<" G_ndim="<<G_ndim<<" and G_nvar="<<G_nvar<<"\n";
#endif
  if (G_ndim!=1)
...
double nested_grid_sph::iR_cov(const cell *c)
{
  //
  // integer and physical units have different origins, so I need a
  // function to get R_com() in integer units, measured from the
...
/// physical units.
///
double nested_grid_sph::distance(const double *p1, ///< position 1 (physical)
          const double *p2  ///< position 2 (physical)
          )
{
...
/// integer units (but obviously the answer is not an integer).
///
double nested_grid_sph::idistance(const int *p1, ///< position 1 (integer)
           const int *p2  ///< position 2 (integer)
           )
{
...
/// Result returned in physical units (e.g. centimetres).
///
double nested_grid_sph::distance_cell2cell(const cell *c1, ///< cell 1
              const cell *c2  ///< cell 2
              )
{
...
/// two units).
///
double nested_grid_sph::idistance_cell2cell(const cell *c1, ///< cell 1
               const cell *c2  ///< cell 2
               )
{
...
/// geometry).  Here both input and output are physical units.
///
double nested_grid_sph::distance_vertex2cell(const double *v, ///< vertex (physical)
                const cell *c    ///< cell
                )
{
...
/// the *cell* coordinate minus the *vertex* coordinate.
///
double nested_grid_sph::difference_vertex2cell(
      const double *v,  ///< vertex (double)
      const cell *c, ///< cell
      const axes a   ///< Axis to calculate.
      )
...
/// geometry).  Here both input and output are code-integer units.
///
double nested_grid_sph::idistance_vertex2cell(const int *v, ///< vertex (integer)
                 const cell *c ///< cell
                 )
{
...
/// the *cell* coordinate minus the *vertex* coordinate.
///
double nested_grid_sph::idifference_vertex2cell(const int *v,  ///< vertex (integer)
             const cell *c, ///< cell
             const axes a   ///< Axis to calculate.
             )
{
  if (a==Rsph)
    ...
    rep.error("Bad axis for nested_grid_sph::idifference_vertex2cell()",a);
...
/// It returns *cell2* coordinate minus *cell1* coordinate.
///
double nested_grid_sph::idifference_cell2cell(
              const cell *c1, ///< cell 1
              const cell *c2, ///< cell 2
              const axes a    ///< Axis.
              )
{
  if (a==Rsph)
    ...
    rep.error("Bad axis for nested_grid_sph::idifference_cell2cell()",a);
  return -1.0;
}
...
int nested_grid_sph::BC_assign_STWIND_add_cells2src(
        const int id, ///< source id
        struct boundary_data *b
        )
{
