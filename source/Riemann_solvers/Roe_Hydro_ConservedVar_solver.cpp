    ...
    pion_flt* out_flux)
{
    ...
    if (fabs(out_flux[1]) > 1.0e50) {
        ...
        rep.printVec("\tRCV_meanp", RCV_meanp, 5);
        cout << "\tRCV_a_mean=" << RCV_a_mean
             << ",  RCV_v2_mean=" << RCV_v2_mean << "\n";
        rep.printVec("\tRCV_eval", RCV_eval, 5);
        rep.printVec("\tRCV_udiff", RCV_udiff, 5);
        rep.printVec("\tRCV_strength", RCV_strength, 5);
        rep.printVec("\tout_flux", out_flux, 5);
...
void Riemann_Roe_Hydro_CV::set_wave_strengths()
{
    ...
    RCV_strength[1] =
        (RCV_udiff[eqRHO]
             * (RCV_meanp[eqHH] - RCV_meanp[eqVX] * RCV_meanp[eqVX])
...
void Riemann_Roe_Hydro_CV::calculate_symmetric_flux(pion_flt* out_flux)
{
    ...
    for (int iwave = 0; iwave < 5; iwave++) {
        out_flux[eqRHO] -= RCV_strength[iwave] * fabs(RCV_eval[iwave])
                           * RCV_evec[iwave][eqRHO];
        out_flux[eqMMX] -= RCV_strength[iwave] * fabs(RCV_eval[iwave])
                           * RCV_evec[iwave][eqMMX];
        out_flux[eqMMY] -= RCV_strength[iwave] * fabs(RCV_eval[iwave])
                           * RCV_evec[iwave][eqMMY];
        out_flux[eqMMZ] -= RCV_strength[iwave] * fabs(RCV_eval[iwave])
                           * RCV_evec[iwave][eqMMZ];
        out_flux[eqERG] -= RCV_strength[iwave] * fabs(RCV_eval[iwave])
                           * RCV_evec[iwave][eqERG];
    }
