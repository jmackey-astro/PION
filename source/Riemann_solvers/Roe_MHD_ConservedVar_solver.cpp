...
#ifdef FUNCTION_ID
    cout
        << "Riemann_Roe_MHD_CV::MHD_Roe_CV_flux_solver_symmetric ...starting.\n";
#endif  // FUNCTION_ID
...
int Riemann_Roe_MHD_CV::Roe_get_wavespeeds()
{
    ...
    if ((cf2diff = Roe_cf * Roe_cf - Roe_cs * Roe_cs) > MACHINEACCURACY) {
        if ((Roe_alphaf = Roe_a * Roe_a - Roe_cs * Roe_cs) < 0.0)
            Roe_alphaf = 0.;
        if ((Roe_alphas = Roe_cf * Roe_cf - Roe_a * Roe_a) < 0.0)
            Roe_alphas = 0.;
    ...
    pion_flt* out_flux)
{
    ...
    switch (err) {
        ...
        case 1:
            negPGct++;
            if (negPGct < 1000) {
                cout << "Got negative pressure in Roe starred state! err="
                     << err << "\n";
