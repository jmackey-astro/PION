      ...
      )
{
  ...
  // are then return the flux of the left state:
  //
  err = test_left_right_equality(left,right);
  if (err) {
    for (int v=0;v<rs_nvar;v++)
  ...
  //
  // see Roe_flux_solver_symmetric() for descriptions of what all of
  // these functions do.
  //
  set_Roe_mean_state(left,right);
  set_eigenvalues();
  ...
  //
  // Here we differ from the symmetric solver.  If the smallest eval
  // is positive or the largest eval is negative, then the solution is
  // the left/right state and the flux can be calculated with no more
  // work.  This results in a much more efficient solver.
  //
  if       (RCV_eval[0] >= 0.0) {
    //
    ...
    //
    // The intermediate state required us to calculated the
    // eigenvectors and wavestrengths.
    //
    set_eigenvectors();
    set_wave_strengths();

    //
    // Here we further differ from the symmetric solver by calculating
    // the one-sided flux.  Note that this produces asymmetric results
    // in symmetric simulations, so it should be used with caution!
    // Especially for problems where symmetry may be important.
    //
    calculate_asymmetric_flux(left,right,out_flux);
    //
    // Here Pstar is obtained from the mean state vector.
    //
    set_pstar_from_meanp(out_pstar);
  }
...
#ifdef FUNCTION_ID
  cout <<"Riemann_Roe_Hydro_CV::Roe_flux_solver_onesided ...returning.\n";
#endif //FUNCTION_ID
  return 0;
}
...
void Riemann_Roe_Hydro_CV::set_wave_strengths()
{
  ...
  double u5bar = RCV_udiff[eqERG] -RCV_strength[2]*RCV_meanp[eqVY] -RCV_strength[3]*RCV_meanp[eqVZ];
  
  RCV_strength[1] = (RCV_udiff[eqRHO]*(RCV_meanp[eqHH]-RCV_meanp[eqVX]*RCV_meanp[eqVX])
	    +RCV_meanp[eqVX]*RCV_udiff[eqMMX] -u5bar)*(eq_gamma-1.0)/RCV_a_mean/RCV_a_mean;
  RCV_strength[0] = 0.5*(RCV_udiff[eqRHO]*(RCV_meanp[eqVX]+RCV_a_mean)
		-RCV_udiff[eqMMX] -RCV_a_mean*RCV_strength[1])/RCV_a_mean;
      ...
      )
{
  ...
  for (int iwave=0;iwave<5;iwave++) {
    out_flux[eqRHO] -= RCV_strength[iwave]*fabs(RCV_eval[iwave])*RCV_evec[iwave][eqRHO];
    out_flux[eqMMX] -= RCV_strength[iwave]*fabs(RCV_eval[iwave])*RCV_evec[iwave][eqMMX];
    out_flux[eqMMY] -= RCV_strength[iwave]*fabs(RCV_eval[iwave])*RCV_evec[iwave][eqMMY];
    out_flux[eqMMZ] -= RCV_strength[iwave]*fabs(RCV_eval[iwave])*RCV_evec[iwave][eqMMZ];
    out_flux[eqERG] -= RCV_strength[iwave]*fabs(RCV_eval[iwave])*RCV_evec[iwave][eqERG];
  }
      ...
      )
{
  ...
  cout <<"Riemann_Roe_Hydro_CV::calculate_asymmetric_flux ...starting.\n";
    ...
    if (diff>1e-3) {
      cout <<"*** FLUX CALCULATION ERROR IN flux_solver_hydro_adi::Roe_flux_solver(): diff = "<<diff<<"\n";
      rep.printVec("left  flux:",out_flux,5);
	...
	cout << "]" << "\n";
      ...
      cout <<"*** FLUX CALCULATION ERROR IN flux_solver_hydro_adi::Roe_flux_solver()\n\n";
    } // if large difference
