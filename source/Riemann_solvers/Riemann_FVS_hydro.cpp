      ...
      )
{
  ...
  //
  double 
    cl = chydro(pl,eq_gamma),
    cr = chydro(pr,eq_gamma),
    Ml = pl[eqVX]/cl,
    Mr = pr[eqVX]/cr,
    f1=0.0,
    f2=0.0;
      ...
      )
{
  ...
  //
  double
    RoeAvg_rl = sqrt(p1[eqRO]),
    RoeAvg_rr = sqrt(p2[eqRO]),
  ...
  //
  ans[eqPG] = RoeAvg_denom*(RoeAvg_rl*Enthalpy(p1,eq_gamma) +
		     RoeAvg_rr*Enthalpy(p2,eq_gamma));
  ans[eqPG] = (eq_gamma-1.0)*(ans[eqPG]-0.5*(ans[eqVX]*ans[eqVX]
					     +ans[eqVY]*ans[eqVY]
					     +ans[eqVZ]*ans[eqVZ]));
