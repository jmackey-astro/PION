  ...
  virtual public eqns_mhd_ideal
{
  public:
    ...
    * Also receives mean values of problems it is likely to encounter, for reference
    * purposes, when deciding whether a quantity is 'small' or not.
    * */
   riemann_MHD(const int,      ///< Length of state vectors, nvar.
    ...
    * pointers to the left and right state vectors, and a pointer to the resulting
    * state vector.
    * 
    * \retval 0 success
  ...
  private:
   int RS_mode; ///< Mode of solution, only mode=1 works so far, for the linear solver.
   int RS_nvar; ///< The number of active variables/waves in the Riemann problem. Hard-coded to seven for simplicity.
   double ansBX;  ///< B_x is just a parameter, so set it at the start of the sovle function, and assign to this variable.
   double ch,///< Hydrodynamical sound speed (in average state).
     ...
     bx, ///< Normalised x-component of magnetic field,\f$=B_x/\sqrt{\rho}\f$.
     bt, ///< Normalised tangential component of magnetic field,\f$=\sqrt{(B_y^2+B_z^2)/\rho}\f$.
    ...
    * */
   void getPdiff();
   /** \brief Gets the appropriate value of P* from left and right and makes sure they match.
    * 
    * 
    * \retval 0 success
    * \retval 1 failure
    ...
    * 
    * The riemann solver orders the variables according to the enum rsvars, while
    * the main code orders them according to the enum primitive, so this function
