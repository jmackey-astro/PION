      ...
      )
{
  ...
  // 
  double BX = left[eqBX]; // Bx is constant
  ...
  // 
  double 
    gp_l = gamma * left[eqPG];
    BB_l = pow(BX,2) + pow(left[eqBY],2)  + pow(left[eqBZ],2);
    temp = gp_l + BB_l;
    cf_l = sqrt((temp + sqrt(pow(temp,2) - 4*gp_l * pow(BX,2)))/(2*left[eqRO]));

  double 
    gp_r = gamma * right[eqPG];
    BB_r = pow(BX,2) + pow(right[eqBY],2) + pow(right[eqBZ],2);
    temp = gp_r + BB_r;
    cf_r = sqrt((temp + sqrt(pow(temp,2) - 4*gp_r * pow(BX,2)))/(2*right[eqRO]));
  ...
  double 
    sl_vl = HD_lambda[0] - left[eqVX];
    sr_vr = HD_lambda[4] - right[eqVX];
    tp_r  = right[eqPG]  + 0.5 * BB_r; // total pressure
    tp_l  = left[eqPG]   + 0.5 * BB_l;
  ...
  //
  double 
    sl_sm = HD_lambda[0] - HD_lambda[2];
    sr_sm = HD_lambda[4] - HD_lambda[2];


  HD_ULs[eqRHO] = left[eqRO]  * sl_vl/sl_sm; // d* (m05 eq 43)
  HD_URs[eqRHO] = right[eqRO] * sr_vr/sl_sm;
  ...
  double 
    temp_l1 = HD_lambda[2] - left[eqVX];
    temp_l2 = left[eqRO]   * sl_vl * sl_sm - pow(BX,2);
    temp_r1 = HD_lambda[2] - right[eqVX];
    temp_r2 = right[eqRO] * sr_vr * sr_sm - pow(BX,2);

  double 
    vys_l  = left[eqVY]  - BX * left[eqBY]  * temp_l1/temp_l2; // Vy* (m05 eq. 44)
    vys_r  = right[eqVY] - BX * right[eqBY] * temp_r1/temp_r2;
  
  HD_ULs[eqMMY] = vys_l * HD_ULs[eqRHO]; 
  HD_URs[eqMMY] = vys_r * HD_URs[eqRHO]; 

  double 
    vzs_l  = left[eqVZ]  - BX  * left[eqBZ]  * temp_l1/temp_l2; // Vz* (m05 eq. 46)
    vzs_r  = right[eqVZ] - BX  * right[eqBZ] * temp_r1/temp_r2;
  ...
  temp_l2 = HD_lambda[3] * HD_ULs[eqBBX] + vys_l * HD_ULs[eqBBY] + vzs_l * HD_ULs[eqBBZ]; // B* dot v*
  temp_r2 = HD_lambda[3] * HD_URs[eqBBX] + vys_r * HD_URs[eqBBY] + vzs_r * HD_URs[eqBBZ];
