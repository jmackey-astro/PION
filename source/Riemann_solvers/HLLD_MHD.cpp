    ...
    eqns_mhd_ideal(nv)
{
    ...
    HD_lambda =
        mem.myalloc(HD_lambda, 5);  // wave speeds (one entropy, two Alfvén)
...
)
{
    ...
    HD_lambda[2] = (sr_vr * HD_UR[eqMMX] - sl_vl * HD_UL[eqMMX] - tp_r + tp_l)
                   / temp;  // S_m (m05 eq 38)
    double tp_s =
        (sr_vr * Pr[eqRO] * tp_l - sl_vl * Pl[eqRO] * tp_r
         + Pl[eqRO] * Pr[eqRO] * sr_vr * sl_vl * (Pr[eqVX] - Pl[eqVX]))
        / temp;  // total pressure* (m05 eq 41)
    ...
    if (isfinite(temp_l1 / temp_l2)) {
        vys_l =
            Pl[eqVY] - BX * Pl[eqBY] * temp_l1 / temp_l2;  // Vy* (m05 eq. 44)
        vzs_l =
            Pl[eqVZ] - BX * Pl[eqBZ] * temp_l1 / temp_l2;  // Vz* (m05 eq. 46)
    }
    ...
    temp_l1 =
        Pl[eqVX] * BX + Pl[eqVY] * Pl[eqBY] + Pl[eqVZ] * Pl[eqBZ];  // B dot v
    temp_r1 = Pr[eqVX] * BX + Pr[eqVY] * Pr[eqBY] + Pr[eqVZ] * Pr[eqBZ];
    temp_l2 = HD_lambda[2] * HD_ULs[eqBBX] + vys_l * HD_ULs[eqBBY]
              + vzs_l * HD_ULs[eqBBZ];  // B* dot v*
    temp_r2 = HD_lambda[2] * HD_URs[eqBBX] + vys_r * HD_URs[eqBBY]
              + vzs_r * HD_URs[eqBBZ];

    HD_ULs[eqERG] = (sl_vl * HD_UL[eqERG] - tp_l * Pl[eqVX]
                     + tp_s * HD_lambda[2] + BX * (temp_l1 - temp_l2))
                    / sl_sm;  // e* (m05 eq 48)
    HD_URs[eqERG] = (sr_vr * HD_UR[eqERG] - tp_r * Pr[eqVX]
                     + tp_s * HD_lambda[2] + BX * (temp_r1 - temp_r2))
                    / sr_sm;

    HD_lambda[1] =
        HD_lambda[2] - abs(BX) / sqrt(HD_ULs[eqRHO]);  // S_l* (m05 eq 51)
    ...
    if (BX == 0) {
        ...
        double vy_ss = (temp_l1 * vys_l + temp_r1 * vys_r
                        + (HD_URs[eqBBY] - HD_ULs[eqBBY]) * sgn)
                       / temp;
        HD_ULss[eqMMY] = vy_ss * HD_ULss[eqRHO];  // Vy** (m05 eq 59)
        HD_URss[eqMMY] = vy_ss * HD_URss[eqRHO];

        double vz_ss = (temp_l1 * vzs_l + temp_r1 * vzs_r
                        + (HD_URs[eqBBZ] - HD_ULs[eqBBZ]) * sgn)
                       / temp;
        HD_ULss[eqMMZ] = vz_ss * HD_ULss[eqRHO];  // Vz** (m05 eq 60)
        HD_URss[eqMMZ] = vz_ss * HD_URss[eqRHO];

        HD_ULss[eqBBX] = HD_URss[eqBBX] = BX;
        HD_ULss[eqBBY]                  = HD_URss[eqBBY] =
            (temp_l1 * HD_URs[eqBBY] + temp_r1 * HD_ULs[eqBBY]
             + temp_l1 * temp_r1 * (vys_r - vys_l) * sgn)
            / temp;  // By** eq 61
        HD_ULss[eqBBZ] = HD_URss[eqBBZ] =
            (temp_l1 * HD_URs[eqBBZ] + temp_r1 * HD_ULs[eqBBZ]
             + temp_l1 * temp_r1 * (vzs_r - vzs_l) * sgn)
            / temp;  // Bz** eq 62

        temp = HD_lambda[2] * HD_ULss[eqBBX] + vy_ss * HD_ULss[eqBBY]
               + vz_ss * HD_ULss[eqBBZ];  // B** dot v**

        HD_ULss[eqERG] = HD_ULs[eqERG]
                         - temp_l1 * (temp_l2 - temp) * sgn;  // E** (m05 eq 63)
    ...
    double& Sr)
{
    ...
    double BX = 0.5 * (Pl[eqBX] + Pr[eqBX]);
    double cf_l =
        cfast_components(Pl[eqRO], Pl[eqPG], BX, Pl[eqBY], Pl[eqBZ], eq_gamma);

    double cf_r =
        cfast_components(Pr[eqRO], Pr[eqPG], BX, Pr[eqBY], Pr[eqBZ], eq_gamma);
