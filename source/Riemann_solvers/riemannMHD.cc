  ...
  : eqns_base(nv), eqns_mhd_ideal(nv)
{
  ...
  for (int v=0;v<RS_nvar;v++) {
    ...
    for (int w=0;w<RS_nvar;w++) RS_leftevec[v][w] = RS_rightevec[v][w] = 0.;
  }
  ...
  samestate=0;
  totalsolve = 0;
  smallB = MACHINEACCURACY;     // This is a small number to use when roundoff errors happen.
				   ...
				   const double g)
{
  ...
  case (FLUX_RSlinear):
    ...
    break;
  default:
    rep.error("\tMODE i: Don't know what to do.",RS_mode); break;
  }
  totalsolve++;
  ...
  if (diff <1.e-6) {
    ...
    if (RS_pstar[RPG]<0.) {
      ...
      cerr <<"pstar[] = ["; for (int v=0;v<(eq_nvar-1);v++) cerr << RS_pstar[v] <<", "; cout << RS_pstar[eq_nvar-1] << " ]" << "\n";
    ...
    for (int v=0; v<eq_nvar; v++) ans[v] = RS_pstar[v];
  ...
  // ******************************************************************
  switch (mode) {
   case (1): // Should push this out into a function called LinearSolve() if I ever add a second mode.
    ...
    if (err!=0){
      rep.error("riemann_MHD::(RoeBalsara_evectors) returned with error",err);
    }
...
void riemann_MHD::assign_data(const double* l, const double* r)
{
  ...
  if (l[eqRO]<TINYVALUE || l[eqPG]<TINYVALUE ||
    ...
    rep.error("riemann_MHD::assign_data() Density/Pressure too small",
	      min(r[eqRO],r[eqPG]));
#endif // RS_TESTING
...
int riemann_MHD::get_sound_speeds()
{
   ...
   *     \f$b_t\f$ is very small and \f$b_x^2 \simeq a^2\f$.  So we need to check
   *     that this is positive.
   *   - Secondly for the slow speed, the term \f$[(a^2+b^2)-\sqrt{\ldots}]\f$ can 
   *     be very small.  This happens when \f$a \ll 1\f$ or when \f$b_x \ll1\f$.
   *     So we also need to check that this is positive.
   * The question is what to do when these turn out to be negative.  This means that
   * the computer can't calculate it accurately.  I can give it an approximation to
   * calculate it accurately, or I can arbitrarily set it to be a value slightly
   * larger than the machine precision.  I decided the simplest thing is the latter.
   * 
   * In two cases where the slow speed goes to zero (namely \f$a\rightarrow 0\f$
   * and/or \f$b_x\rightarrow 0\f$), it is well approximated by
   * \f[ 
   * c_s^2 \simeq \frac{a^2b_x^2}{a^2+b^2} + \mbox{h.o.t.}
   * \f]
   * so I can use this.  There is also the hydrodynamic limit, where \f$b\rightarrow 0\f$.
   * In this case \f$ c_s^2 \simeq b_x^2 +\mbox{h.o.t.}\f$.
   * 
   * In cases where \f$ [(a^2+b^2)^2 - 4a^2b_x^2]\rightarrow 0 \f$, the fast and
   * slow speed become the same.
   * 
   * So at this point I will determine whether we have any numerical instability 
   * present, and will get rid of it.
   * 
   * Three quantities determine the stability: \f$ \{b_x,b_t,a\}\f$.  
   * - if \f$a\rightarrow 0\f$ Then we are in trouble.  In this case MHD probably 
   *   isn't applicable, because the magnetic field energy is \f$ 10^{15} \f$ times stronger
   *   than the gas energy.  This is not likely to happen in the problems we are 
   *   considering.  So I think if this is encountered we should bug out of the code,
   *   with an explanation of why, of course.  So for the rest of the cases we assume
   *   that \f$a\f$ is of order unity.
   * - if \f$b_x\rightarrow 0\f$ but \f$b_t\simeq 1\f$, then the Alfv\'en and slow
   *   waves disappear into the contact discontinuity.  The fast and slow speeds are 
   *   given by \f$c_f^2 \simeq a^2+b_t^2\f$ and \f$c_s^2 \simeq  \frac{a^2b_x^2}{a^2+b^2}\f$
   *   respectively.  The normalisation quantities \f$\alpha_{f,s}\f$ will be calculated
   *   without difficulty.  So in this case I may want to approximate \f$c_s\f$.  Or,
   *   what would be better would be to have a pseudo-hydro solver, as Falle points out
   *   that this is ok.  Certainly simpler than solving the MHD problem.
   * - if \f$b_x\rightarrow 0 \f$ and \f$b_t\rightarrow 0\f$ we get the hydrodynamic
   *   limit, where the magnetic field is irrelevant.  I can just call the 
   *   hydro solver in this case, and leave the magnetic field unchanged.
   * - The remaining case is where \f$b_t\rightarrow 0\f$ and \f$b_x\simeq 1\f$.
   *   This is really three subcases.  Obviously the Alfv\'en waves need attention 
   *   regardless.  Falle suggests setting \f$\beta_{f,s} = 1/\sqrt{2}\f$, saying that
   *   it really doesn't matter what you choose.  I wonder is it better to give them
   *   random signs, so you don't have any chance to add up error???  For the slow and
   *   fast waves here are the three cases:
   *   - When \f$b_x^2<a^2\f$, there is no problem as long as \f$a^2-b_x^2 > \epsilon\f$
   *     where \f$\epsilon\f$ is a small number close to the machine precision.
   *   - When \f$b_x^2>a^2\f$, there is no problem as long as \f$b_x^2-a^2 > \epsilon\f$
   *     where \f$\epsilon\f$ is a small number close to the machine precision.
   *   - When \f$b_x^2 \simeq a^2\f$, things become difficult.  The slow and fast speeds
   *     become the same.  The fast and slow normalisations can vary from zero to one
   *     very sensitively, based on the small difference in the sound speeds.  Falle, and
   *     Roe and Balsara both claim that it doesn't matter what you choose for this case,
   *     that the flux you get out at the end is insensitive to what you choose.  I should
   *     test this.
   * 
   * So, basically, if the slow or fast speeds are the same as the sound speed, then 
   * I set them to be chydro+-smallB.  If the tangential field is less than smallB^3,
   * I set it to be tinyB.  If alpha_f,s cannot be determined accurately, it means
   * that the speeds are very similar, so I set them to both to be equal at 1/root2.
   * */
   ...
   * \f$b_t > \mbox{\tt tinyB}\f$, and \f$\beta_{y,z} = 1/\sqrt{2}\f$ otherwise.
   * */
  riemann_MHD::bt = sqrt((RS_meanp[RBY]*RS_meanp[RBY]+
			  RS_meanp[RBZ]*RS_meanp[RBZ])/RS_meanp[RRO]);
  if(bt>tinyB) {
   ...
   * We check that the hydro sound speed is not insignificantly small compared to 
   * the magnetic field strength.  If this is the case, then MHD is probably no
   * good anyway, and nothing will be calculated accurately.\n
   * The condition is that if \f$a/\max{(|b_x|,b_t)} < \sqrt{\mbox{machine precision}}\f$
   * then we bug out and complain.
   * */
   ...
   * If I am clever, I have already ensured that the sound speeds are all finite,
   * and as a byproduct, they are all greater than zero.  So I shouldn't have 
   * any trouble with machine accuracy here.  I check anyway.\n
   * Where I might have trouble is that I have set cf=ch+smallB, cs=ch-smallB, so 
   * this enforces alphaf^2=alphax^2=1/2.  But this is only enforced sometimes.
   * */
  double cf2diff;
  ...
  if( (cf<=0.) || (cs<0.) || (ca<0.) || (ch<=0.) ) {
    cerr<<"(riemann_MHD::get_sound_speeds) Error... sound speeds are negative.\n";
    cout <<"Speeds: (ch2,ca2,ct2,cs2,cf2): ("<<ch*ch
      << " ,"<<ca*ca<<" ,"<< bt*bt<< " ,"
      <<cs*cs<< " ,"<<cf*cf<<" )\n";
    return(1);
...
{
  cout << "Normalising evectors the hard way" << "\n";
  double dp;
  for (waves i=FN; i<=FP; ++i) {
    dp = dot_product(RS_leftevec[i],RS_leftevec[i], RS_nvar);
    // cout << "(norm_ev) Dot product: " << dp << "\n";
    for (waves j=FN; j<=FP; ++j) {
      RS_leftevec[i][j] /= sqrt(dp);
      // cout  << "leftevec[" << i << "][" << j << "]: " << RS_leftevec[i][j] << "\n";
    }
  } 
  return(0);
}
...
int riemann_MHD::get_pstar()
{
  ...
  for (i=0; i<RS_nvar; i++) {
    //cout <<"RS_TESTING!\n";
    if( fabs(RS_pstar[i]-RS_pdiff[i])/(fabs(RS_pstar[i])+fabs(RS_pdiff[i])+SMALLVALUE)> 5.e-2) {
      cout.width(10);
      cout  <<"(riemann_MHD::get_pstar) left-ans["<<i<<"] - right-ans["<<i<<"] = ";
      cout.width(10); cout<< RS_pstar[i]-RS_pdiff[i] <<"\n";
      cout <<"Evalues[] = [";
      for (int j=0;j<(RS_nvar-1);j++)
	cout <<RS_evalue[j] <<", "; cout <<RS_evalue[RS_nvar-1] << " ]\n";
      cout <<"left[]    = [";
      for (int j=0;j<(RS_nvar-1);j++)
	cout <<  RS_left[j] <<", "; cout <<  RS_left[RS_nvar-1] << " ]" << "\n";
      cout <<"right[]   = [";
      for (int j=0;j<(RS_nvar-1);j++)
	cout << RS_right[j] <<", "; cout << RS_right[RS_nvar-1] << " ]" << "\n";
      cout <<"pstar_l[] = [";
      for (int j=0;j<(RS_nvar-1);j++)
	cout << RS_pstar[j] <<", "; cout << RS_pstar[RS_nvar-1] << " ]" << "\n";
      cout <<"pstar_r[] = [";
      for (int j=0;j<(RS_nvar-1);j++)
	cout << RS_pdiff[j] <<", "; cout << RS_pdiff[RS_nvar-1] << " ]" << "\n";
      cout <<"Bx = "<<ansBX<<"\t alphaf,s = "<<alphaf<<" "<<alphas<<"\n";
...
int riemann_MHD::RoeBalsara_evectors()
{
  ...
  // Now multiply left fast and slow evectors by 1/(2a^2) to make them normalised.
...
{
  // Warning!!
  cout <<"WARNING: FALLE EVECTORS IS NOT FINISHED CODE, ";
  cout <<"AND IS TESTED BUT FOUND WANTING SO FAR...\n";
  cout <<"Use at your own risk!!!\n";
  rep.error("DON'T USE FALLE EVECTORS -- NOT STABLE, AND CAUSES TROUBLE",999);
  if (RS_nvar !=8) {
    cerr << "(riemann_MHD::construct_evector) RS_nvar !=8 !!!";
    cerr <<" Don't know how to contruct non-MHD evectors." << "\n";
    return(1);
  }

  double Bt = sqrt(RS_meanp[RBY]*RS_meanp[RBY] + RS_meanp[RBZ]*RS_meanp[RBZ]);
  double deltaF, deltaS, normA, normF, normS;
  deltaF = RS_meanp[RRO]*cf*cf - ansBX*ansBX;
  deltaS = RS_meanp[RRO]*cs*cs - ansBX*ansBX;

  if (Bt>smallB)  normA = 1./sqrt(2.)/Bt;
  else normA = 0.0;

  normF = deltaF /(cf*sqrt(2.*(deltaF*deltaF + ansBX*ansBX*Bt*Bt)));
  normS = deltaS /(cs*sqrt(2.*(deltaS*deltaS + ansBX*ansBX*Bt*Bt)));
  //  cout << "(riemann_MHD::construct_eigenvectors) some values... " << "\n";
  //  cout << "Bt: "<<Bt<< " delF,S: "<<deltaF<<", "<<deltaS;
  //  cout <<" normF,S: "<<normF<<", "<<normS << "\n";
  int sBx;
  if (ansBX<0) sBx = -1;
  else if (ansBX>0) sBx = 1;
  else {
  ...
  // ****LEFT EIGENVECTORS****
  // Left Fast Magnetosonic Wave
  RS_leftevec[FN][0] = 0.0;
  RS_leftevec[FN][1] = -cf;
  RS_leftevec[FN][2] = cf*ansBX*RS_meanp[RBY]/deltaF;
  RS_leftevec[FN][3] = cf*ansBX*RS_meanp[RBZ]/deltaF;
  RS_leftevec[FN][4] = 1./RS_meanp[RRO];
  RS_leftevec[FN][5] = cf*cf*RS_meanp[RBY]/deltaF;
  RS_leftevec[FN][6] = cf*cf*RS_meanp[RBZ]/deltaF;
  // Left Alfven Wave
  RS_leftevec[AN][0] = 0.;
  RS_leftevec[AN][1] = 0.;
  RS_leftevec[AN][2] =  sBx*RS_meanp[RBZ];
  RS_leftevec[AN][3] = -sBx*RS_meanp[RBY];
  RS_leftevec[AN][4] = 0.;
  RS_leftevec[AN][5] =  RS_meanp[RBZ] /sqrt(RS_meanp[RRO]);
  RS_leftevec[AN][6] = -RS_meanp[RBY] /sqrt(RS_meanp[RRO]);
  // Left Slow Magnetosonic Wave
  RS_leftevec[SN][0] = 0.0;
  RS_leftevec[SN][1] = -cs;
  RS_leftevec[SN][2] = cs*ansBX*RS_meanp[RBY]/deltaS;
  RS_leftevec[SN][3] = cs*ansBX*RS_meanp[RBZ]/deltaS;
  RS_leftevec[SN][4] = 1./RS_meanp[RRO];
  RS_leftevec[SN][5] = cs*cs*RS_meanp[RBY]/deltaS;
  RS_leftevec[SN][6] = cs*cs*RS_meanp[RBZ]/deltaS;
  // Contact Discontinuity
  RS_leftevec[CT][0] = 1.;
  RS_leftevec[CT][1] = 0.;
  RS_leftevec[CT][2] = 0.;
  RS_leftevec[CT][3] = 0.;
  RS_leftevec[CT][4] = -1/ch/ch;
  RS_leftevec[CT][5] = 0.;
  RS_leftevec[CT][6] = 0.;
  // Right Slow Magnetosonic Wave
  RS_leftevec[SP][0] = 0.0;
  RS_leftevec[SP][1] = -RS_leftevec[SN][1];
  RS_leftevec[SP][2] = -RS_leftevec[SN][2];
  RS_leftevec[SP][3] = -RS_leftevec[SN][3];
  RS_leftevec[SP][4] = RS_leftevec[SN][4];
  RS_leftevec[SP][5] = RS_leftevec[SN][5];
  RS_leftevec[SP][6] = RS_leftevec[SN][6];
  // Right Alfven Wave
  RS_leftevec[AP][0] = 0.;
  RS_leftevec[AP][1] = 0.;
  RS_leftevec[AP][2] = RS_leftevec[AN][2];
  RS_leftevec[AP][3] = RS_leftevec[AN][3];
  RS_leftevec[AP][4] = 0.;
  RS_leftevec[AP][5] = -RS_leftevec[AN][5];
  RS_leftevec[AP][6] = -RS_leftevec[AN][6];
  // Right Fast Magnetosonic wave
  RS_leftevec[FP][0] = 0.0;
  RS_leftevec[FP][1] = -RS_leftevec[FN][1];
  RS_leftevec[FP][2] = -RS_leftevec[FN][2];
  RS_leftevec[FP][3] = -RS_leftevec[FN][3];
  RS_leftevec[FP][4] = RS_leftevec[FN][4];
  RS_leftevec[FP][5] = RS_leftevec[FN][5];
  RS_leftevec[FP][6] = RS_leftevec[FN][6];
  // ****RIGHT EIGENVECTORS****
  // Left Fast Magnetosonic Wave
  RS_rightevec[FN][0] = RS_meanp[RRO];
  RS_rightevec[FN][1] = -cf;
  RS_rightevec[FN][2] = RS_leftevec[FN][2];
  RS_rightevec[FN][3] = RS_leftevec[FN][3];
  RS_rightevec[FN][4] = RS_meanp[RRO]*ch*ch;
  RS_rightevec[FN][5] = RS_meanp[RRO]*RS_leftevec[FN][5];
  RS_rightevec[FN][6] = RS_meanp[RRO]*RS_leftevec[FN][6];
  // Left Alfven Wave
  RS_rightevec[AN][0] = 0.;
  RS_rightevec[AN][1] = 0.;
  RS_rightevec[AN][2] = RS_leftevec[AN][2];
  RS_rightevec[AN][3] = RS_leftevec[AN][3];
  RS_rightevec[AN][4] = 0.;
  RS_rightevec[AN][5] = RS_leftevec[AN][5]*RS_meanp[RRO];
  RS_rightevec[AN][6] = RS_leftevec[AN][6]*RS_meanp[RRO];
  // Left Slow Magnetosonic Wave
  RS_rightevec[SN][0] = RS_meanp[RRO];
  RS_rightevec[SN][1] = RS_leftevec[SN][1];
  RS_rightevec[SN][2] = RS_leftevec[SN][2];
  RS_rightevec[SN][3] = RS_leftevec[SN][3];
  RS_rightevec[SN][4] = RS_meanp[RRO]*ch*ch;
  RS_rightevec[SN][5] = RS_meanp[RRO]*RS_leftevec[SN][5];
  RS_rightevec[SN][6] = RS_meanp[RRO]*RS_leftevec[SN][6];
  // Contact Discontinuity
  RS_rightevec[CT][0] = 1.0;
  RS_rightevec[CT][1] = 0.;
  RS_rightevec[CT][2] = 0.;
  RS_rightevec[CT][3] = 0.;
  RS_rightevec[CT][4] = 0.;
  ...
  RS_rightevec[CT][6] = 0.;
  // Right Slow Magnetosonic Wave
  RS_rightevec[SP][0] = RS_meanp[RRO];
  RS_rightevec[SP][1] = -RS_rightevec[SN][1];
  RS_rightevec[SP][2] = -RS_rightevec[SN][2];
  RS_rightevec[SP][3] = -RS_rightevec[SN][3];
  RS_rightevec[SP][4] = RS_rightevec[SN][4];
  RS_rightevec[SP][5] = RS_rightevec[SN][5];
  RS_rightevec[SP][6] = RS_rightevec[SN][6];
  // Right Alfven Wave
  RS_rightevec[AP][0] = 0.;
  RS_rightevec[AP][1] = 0.;
  RS_rightevec[AP][2] = RS_leftevec[AP][2];
  RS_rightevec[AP][3] = RS_leftevec[AP][3];
  RS_rightevec[AP][4] = 0.;
  RS_rightevec[AP][5] = RS_leftevec[AP][5]*RS_meanp[RRO];
  RS_rightevec[AP][6] = RS_leftevec[AP][6]*RS_meanp[RRO];
  // Right Fast Magnetosonic wave
  RS_rightevec[FP][0] = RS_meanp[RRO];
  RS_rightevec[FP][1] = -RS_rightevec[FN][1];
  RS_rightevec[FP][2] = -RS_rightevec[FN][2];
  RS_rightevec[FP][3] = -RS_rightevec[FN][3];
  RS_rightevec[FP][4] = RS_rightevec[FN][4];
  RS_rightevec[FP][5] = RS_rightevec[FN][5];
  RS_rightevec[FP][6] = RS_rightevec[FN][6];
  // This function normalises by explicitly calculating dot products and dividing.
  //  riemann_MHD::normalise_evectors();
  // Better to do it as in Falle et al.(1998), as follows
//  cout << "Normalising...";
  for (int i=0; i<RS_nvar; i++) {
    RS_leftevec[FN][i] *= normF;
    RS_leftevec[FP][i] *= normF;
    RS_leftevec[AN][i] *= normA;
    RS_leftevec[AP][i] *= normA;
    RS_leftevec[SN][i] *= normS;
    RS_leftevec[SP][i] *= normS;
    RS_rightevec[FN][i] *= normF;
    RS_rightevec[FP][i] *= normF;
    RS_rightevec[AN][i] *= normA;
    RS_rightevec[AP][i] *= normA;
    RS_rightevec[SN][i] *= normS;
    RS_rightevec[SP][i] *= normS;
    // Contact wave e-vector is already normalised.
  }
//  cout << "  done." << "\n";
  return(0);
}
...
int riemann_MHD::check_evectors()
{
  ...
  double errtol = 1.e-8;
  for (waves i=FN; i<=FP; ++i) {
    if ( (test=fabs(dot_product(RS_leftevec[i],RS_rightevec[i],RS_nvar)-1.)) > errtol) {
  ...
  for (waves i=FN; i<=FP; ++i) {
    for (waves j=FN; j<=FP; ++j) {
      if(j!=i){
	if ((test=fabs(dot_product(RS_leftevec[i],RS_rightevec[j],RS_nvar))) >= errtol) {
	  cout << "evectors not properly normalised: ";
