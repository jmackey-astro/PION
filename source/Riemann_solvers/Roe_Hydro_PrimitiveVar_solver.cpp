    ...
    pion_flt* rpv_pstar)
{
    ...
    for (int v = 0; v < rs_nvar; v++) {
        if (!isfinite(rpv_left[v]) || !isfinite(rpv_right[v])
            || !isfinite(rpv_g)) {
    ...
    double rl = sqrt(rpv_left[eqRO]), rr = sqrt(rpv_right[eqRO]),
           lH = Enthalpy(rpv_left, rpv_g), rH = Enthalpy(rpv_right, rpv_g),
           denom = 1.0 / (rl + rr), a_mean = 0.0, v2_mean = 0.0;
    ...
    if (rpv_meanp[eqVX] - a_mean >= 0.) {
        ...
        rpv_pstar[eqPG] =
            0.5
            * (rpv_left[eqPG] + rpv_right[eqPG]
               - rpv_meanp[eqRO] * a_mean * (rpv_right[eqVX] - rpv_left[eqVX]));
        rpv_pstar[eqVX] =
            0.5
            * (rpv_left[eqVX] + rpv_right[eqVX]
