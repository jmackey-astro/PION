...
{
public:
  Riemann_FVS_Euler(const int,      ///< Length of State Vectors, nvar
  ...
  ///  
  int FVS_flux(const double *, ///< Left Primitive var. state vector.
	       const double *, ///< Right Primitive var. state vector.
	       double *,       ///< Result Flux vector.
	       double *,    ///< Interface state (for viscosity)
	       const double    ///< Gas constant gamma (unused b/c g=constant)
	       );
private:
  const int rs_nvar; ///< length of state vectors in solver (ignore tracers!).
  double *fpos, *fneg; ///< split flux vectors.
  void Roe_average_state(const double *, ///< state 1
			 const double *, ///< state 2
			 const double,   ///< gamma
			 double *        ///< Roe-averaged state
			 );
};
