...
void sim_init::print_command_line_options(int argc, char** argv)
{
    ...
    cout << "\t redirect=string : filename with path to redirect stdout/stderr "
            "to\n";
    cout
        << "\t op_criterion=N  : 0=output every I steps, 1=output every D time "
           "units.\n";
    ...
    cout << "\t EtaVisc=D     : modify viscosity parameter to the given double "
            "precision value.\n";
    cout
        << "\t cfl=D         : change the CFL no. for the simulation, in range "
           "(0,1).\n";
    ...
    cout
        << "\t wind_radius_N=D : change radius of boundary for wind source N to "
           "value D (in cm)\n";
    cout << "\n";
    cout << "     *********************************************\n\n";
    return;
        ...
        if (args[i].find("ooa=") != string::npos) {
            ...
            if (v == 0) {
                ...
                cout << "\t\tUsing Colella and Woodward (1984) AV prescription "
                        "(Lapidus).\n";
                cout
                    << "\t\t****** WARNING, THIS NEEDS TESTING, EXPERIMENTAL CODE!! "
                       "****\n";
                ...
                SimPM.etav =
                    0.1;  // This parameter is redundant for the H-correction.
            }
            else if (v == 4) {
                cout << "\t\tUsing the H-correction of Sanders et al. "
                        "(1998,JCP,145,511)\n";
                cout
                    << "\t\twith the 1D viscosity of Falle, Komissarov, Joarder "
                       "(1998)\n";
            ...
            cout
                << "\tOVERRIDE PARAMS: already redirecting stdout, continuing...\n";
        }
            ...
            if (c < 0. || !isfinite(c))
                ...
                cout
                    << "\tWARNING: CFL no. >1, so results will be unstable!!!\n";
            SimPM.CFL = c;
            ...
            if (c < 0 || c > 100)
                rep.error("Bad cooling flag (only 0-11 allowed", c);
            ...
            if (c < 0 || c > 1)
                rep.error("Bad dynamics flag (only 0,1, allowed", c);
            ...
            if (c < 0 || c > 1)
                rep.error("Bad chemistry flag (only 0,1, allowed", c);
            ...
            cout
                << "0=LF,1=RSlin,2=RSexact,3=RShybrid,4=RSroe,5=RSroePV,6=FVS,7="
                   "HLLD,8=HLL:";
...
)
{
    ...
    /// I'm going to set it to output every nth timestep.  Can modify this if
    /// needed. This is a parameter in the parameterfile.  If opfreq is set to a
    /// negative number then only output the final state. We can also set it so
    /// that it outputs every D time units, and there is automatic checkpointing
    /// to filebase.9999999.[txt/fits/silo] every 100 steps.
    ///
    int err = 0;
    //
    ...
    if ((SimPM.timestep != 0) && ((SimPM.timestep % checkpoint_freq) == 0)) {
        ...
        if (textio)
            err += textio->OutputData(
                SimPM.outFileBase, grid, SimPM, checkpoint_id);
        if (err) {
    ...
    if (textio)
        err +=
            textio->OutputData(SimPM.outFileBase, grid, SimPM, SimPM.timestep);
    if (err) {
