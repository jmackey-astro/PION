...
public:
  ...
  /// minimum of all calculated steps.  This function calls two functions, one
  /// to get the microphysics timestep (if needed), and another to get the
  /// dynamics timestep.
  ///
...
protected:
  ///
  /// Calculate the microphysics timestep, based on heating/cooling and
  /// reaction rates.  Returns the minimum timestep of the local grid
  /// (negative if error).
  ///
  ...
  /// also calls the solver function set_thermal_conduction_Edot() so it knows
