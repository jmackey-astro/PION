...
)
{
            ...
            for (size_t i = 0; i < grid[l]->BC_bd.size(); i++) {
                if (grid[l]->BC_bd[i]->itype == COARSE_TO_FINE_SEND) {
                    err += BC_update_COARSE_TO_FINE_SEND(
                        SimPM, grid[l], spatial_solver, l, grid[l]->BC_bd[i], 2,
                        2);
                }
            }
        }
    ...
    if (SimPM.op_criterion == 1) {
        if (SimPM.opfreq_time < TINYVALUE)
            rep.error(
                "opfreq_time not set right and is needed!", SimPM.opfreq_time);
...
#endif
        err += TimeUpdateInternalBCs(
            SimPM, l, grid[l], spatial_solver, SimPM.levels[l].simtime,
    ...
    while (SimPM.maxtime == false) {
        ...
        for (int l = SimPM.grid_nlevels - 1; l >= 0; l--) {
            ...
            if (!restart && !first_step) {
                ...
                SimPM.levels[l].last_dt =
                    SimPM.last_dt / SimPM.levels[l].multiplier;
            }
    ...
    if (SimPM.levels[l].step % 2 == 0) {
        ...
        for (size_t i = 0; i < grid->BC_bd.size(); i++) {
            if (grid->BC_bd[i]->itype == COARSE_TO_FINE_RECV) {
                err += BC_update_COARSE_TO_FINE_RECV(
                    SimPM, spatial_solver, l, grid->BC_bd[i],
                    SimPM.levels[l].step);
            }
        }
#ifdef TEST_INT
...
#endif
    err +=
        TimeUpdateExternalBCs(SimPM, l, grid, spatial_solver, ctime, OA2, OA2);
    // --------------------------------------------------------
    ...
    for (int l = 0; l < SimPM.grid_nlevels; l++) {
        ...
        do {
            if (c->isdomain && c->isleaf) {
                ...
                nowMASS += u[RHO] * dv;
                totmom +=
                    sqrt(u[MMX] * u[MMX] + u[MMY] * u[MMY] + u[MMZ] * u[MMZ])
                    * dv;
            }
        } while ((c = grid[l]->NextPt(c)) != 0);
    }
