...
#ifdef TESTING
    if (t_mp < t_dyn)
        cout << "Limiting timestep by MP: mp_t=" << t_mp
             << "\thydro_t=" << t_dyn << "\n";
        ...
        if (c->timestep && !c->isbd) {
            ...
            if (tempdt <= 0.0) {
                ...
                cout << "celltimestep=" << tempdt << ", gamma=" << par.gamma
                     << ", dx=" << dx << "\n";
                rep.error(
                    "CellTimeStep function returned failing value", c->id);
            }
        ...
        if (c->isbd || !c->isleaf) {
            ...
            switch (par.EP.MP_timestep_limit) {
                case 1:  // cooling time only.
                    tempdt =
                        MP->timescales(c->Ph, par.gamma, true, false, false);
                    break;
                case 4:  // recomb only
                    tempdt =
                        MP->timescales(c->Ph, par.gamma, false, true, false);
                    break;
                case 2:  // cooling+recomb
                    tempdt =
                        MP->timescales(c->Ph, par.gamma, true, true, false);
                    break;
                ...
                default:
                    rep.error(
                        "Bad MP_timestep_limit", par.EP.MP_timestep_limit);
            }
            dt = min(dt, tempdt);
    ...
    do {
        ...
        if (c->isbd || !c->isleaf) {
            ...
            if (tempdt <= 0.0) {
                ...
                rep.error(
                    "Negative timestep from microphysics with RT!", tempdt);
            }
            // cout <<"c->id="<<c->id<<"\tdt="<<tempdt<<"  ";
            // rep.printVec("Ph",c->Ph,par.nvar);
            dt = min(dt, tempdt);
