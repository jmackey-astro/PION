...
)
{
    ...
    int err = inviscid_flux(
        par, grid, dx, Cl, Cr, lp, rp, f, pstar, par.solverType, g);
    ...
    pion_flt* flux)
{
    ...
    if (FV_ntr > 0) {
        ...
        if (flux[eqRHO] > 0.0) {
            ...
            for (int t = 0; t < FV_ntr; t++) {
                flux[eqTR[t]] =
                    left[eqTR[t]] * flux[eqRHO] * corrector[eqTR[t]];
            }
        }
            ...
            for (int t = 0; t < FV_ntr; t++) {
                flux[eqTR[t]] =
                    right[eqTR[t]] * flux[eqRHO] * corrector[eqTR[t]];
            }
        }
        else {
            for (int t = 0; t < FV_ntr; t++)
    ...
    for (int idim = 0; idim < SimPM.ndim; idim++) {
        ...
        do {
            ...
            do {
                ...
                do {
                    err += SetEdgeState(
                        cpt, posdirs[idim], SimPM.nvar, slope_cpt, edgeL, csp,
                        grid);
                    err += SetSlope(
                        npt, axis[idim], SimPM.nvar, slope_npt, csp, grid);
                    err += SetEdgeState(
                        npt, negdirs[idim], SimPM.nvar, slope_npt, edgeR, csp,
                        grid);
                ...
                if (SimPM.ndim == 1)
                    ...
                    start =
                        grid->NextPt(start, posdirs[(idim + 1) % SimPM.ndim]);
    ...
    for (int idim = 0; idim < SimPM.ndim; idim++) {
        ...
        do {
            ...
            do {
                ...
                q_neg =
                    0.0;  // no flux coming in from non-existent boundary data.
                q_pos = 0.0;
                ...
                do {
                    ...
                    if (T < 4.2e5)
                        ...
                        Qclassical =
                            29.7
                            + log(T / (1.0e6 * sqrt(c->Ph[RO] * 4.2735e23)));
                    ...
                    if (SimPM.coord_sys == COORD_CYL && axis[idim] == Rcyl) {
                        ...
                        double rn = rp - dx;
                        cpt->dU[ERG] += 2.0 * (rn * q_neg - rp * q_pos)
                                        / (rp * rp - rn * rn);
                    }
                    else if (
                        SimPM.coord_sys == COORD_SPH && axis[idim] == Rsph) {
                        double rc = CI.get_dpos(c, Rsph);
                        double rp = rc + 0.5 * dx;
                        double rn = rp - dx;
                        rc        = (pow(rp, 3.0) - pow(rn, 3.0)) / 3.0;
                        cpt->dU[ERG] +=
                            (rn * rn * q_neg - rp * rp * q_pos) / rc;
                    }
                    else {
