...
)
{
    ...
    if (s == "MAX") {
        ...
        global = HUGEVALUE;
        err += MPI_Allreduce(
            &local, &global, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
    }
    else if (s == "SUM") {
        global = 0.0;
        err += MPI_Allreduce(
    ...
    if (err)
        rep.error(
            "comm_mpi:global_operation_double: MPI call returned abnormally",
            err);
...
)
{
    ...
    if (err)
        rep.error(
            "comm_mpi:global_op_double_array: MPI call returned abnormally",
            err);
...
)
{
    ...
    if (type == "DOUBLE") {
        err += MPI_Bcast(
            static_cast<double*>(data), n_el, MPI_DOUBLE, sender,
            MPI_COMM_WORLD);
    }
    else if (type == "INT") {
        err += MPI_Bcast(
...
#error "MUST define either PION_DATATYPE_FLOAT or PION_DATATYPE_DOUBLE"
    ...
    //
    err += MPI_Isend(
        si->data, position, MPI_PACKED, si->to_rank, si->comm_tag,
        MPI_COMM_WORLD, &(si->request));
...
)
{
    ...
    if (*from_rank >= 0) {
        ...
        err =
            MPI_Probe(MPI_ANY_SOURCE, comm_tag, MPI_COMM_WORLD, &(ri->status));
    }
...
#endif  // TEST_COMMS
        err +=
            MPI_Unpack(buf, ct, &position, &c_id, 1, MPI_LONG, MPI_COMM_WORLD);
#ifdef TEST_COMMS
...
#endif  // TEST_COMMS
        err +=
            MPI_Unpack(buf, ct, &position, ipos, ndim, MPI_INT, MPI_COMM_WORLD);
#ifdef TEST_COMMS
        // cout <<"position="<<position<<" : ";
...
#if defined PION_DATATYPE_DOUBLE
        err +=
            MPI_Unpack(buf, ct, &position, p, nvar, MPI_DOUBLE, MPI_COMM_WORLD);
#elif defined PION_DATATYPE_FLOAT
        err +=
            MPI_Unpack(buf, ct, &position, p, nvar, MPI_FLOAT, MPI_COMM_WORLD);
#else
#error "MUST define either PION_DATATYPE_FLOAT or PION_DATATYPE_DOUBLE"
#endif
#ifdef TEST_COMMS
    ...
    if (position != ct)
        rep.error("length of data doesn't match", position - ct);
...
)
{
    ...
    if (err) rep.error("getting size of message to receive", err);
#ifdef TEST_COMMS
    cout << "comm_mpi::receive_double_data: buffer is " << ct
         << " elements.\n.";
#endif  // TEST_COMMS
    if (ct < 0) rep.error("bad buffer size count", ct);
    if (ct != nel) {
...
#ifdef TEST_COMMS
    cout
        << "comm_mpi::receive_double_data: receiving buffer of data from rank: "
        << from_rank << "\n";
#endif  // TEST_COMMS
    void* buf = data;
    err += MPI_Recv(
