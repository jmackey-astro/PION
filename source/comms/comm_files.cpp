...
#ifdef TESTING
            cout << "global_operation_double() " << s << " got value "
                 << vals[r] << " from file " << f.str() << "\n";
#endif
            infile.close();
            fs.release_lock(f.str());
            remove(f.str().c_str());
...
)
{
    ...
    do {
        ...
        for (int v = 0; v < nvar; v++)
            outfile.write(
                reinterpret_cast<char*>(&((*c)->Ph[v])), sizeof(double));
        ct++;
        ++c;  // next cell in list.
    } while (c != l->end());
...
#ifdef TESTING
    cout << "rank: " << myrank
         << "  comm_files::send_cell_data() comm_tag=" << comm_tag
         << " nc=" << nc << " \n";
...
)
{
...
#ifdef TESTING
                    cout
                        << "comm_files::look_for_data_to_receive: found data file : "
                        << f.str() << "\n";
#endif
                    found      = true;
                    *from_rank = rank;
    ...
    ifstream infile(f.str().c_str(), ios_base::binary);
    if (!infile.is_open())
        rep.error("failed to open file for reading", f.str());
...
)
{
    ...
    if (!infile.is_open())
        rep.error("failed to open file for reading", info->id);
...
)
{
    ...
    //
    ostringstream f;
    f << dir << doubledata << SimPM.timestep << "_rank_" << myrank
      << "_to_rank_" << to_rank;
    id = f.str();
    ...
    for (int i = 0; i < n_el; i++)
        outfile.write(
            reinterpret_cast<const char*>(&(data[i])), sizeof(double));
    // outfile << data[i];
    outfile.close();
    fs.release_lock(id);
...
)
{
    ...
    //
#ifdef TESTING
    cout << "comm_files::receive_double_data: recv_list size="
         << recv_list.size() << "\n";
#endif  // TESTING
    if (recv_list.empty())
        rep.error("Call look4data before receive_data", recv_list.size());
    ...
    if (nel != ct)
        rep.error(
            "comm_files::receive_cell_data: n_el has unexpected value",
            nel - ct);
    for (int v = 0; v < nel; v++)
...
{
#ifdef TESTING
    cout << "comm_files::silo_pllel_wait_for_file() opening file: "
         << s_filename << " into directory: " << s_dir << "\n";
#endif
    ...
    if (rank_in_grp == 0) {
        ...
        //
        ostringstream f;
        f << dir << silo_id << "_grp_" << grp_rank << "_rank_"
          << rank_in_grp - 1 << finished_with_file;
        wait_for_file(f.str());
        fs.acquire_lock(f.str());
#ifdef TESTING
        cout << "comm_files::silo_pllel_wait_for_file(): found file: "
             << f.str() << ", so my turn to write file.\n";
#endif
        remove(f.str().c_str());
        fs.release_lock(f.str());
...
#ifdef TESTING
        cout
            << "comm_files::silo_pllel_finish_with_file() I'm last proc in group, "
               "returning.\n";
#endif
    }
    else {
...
#ifdef TESTING
        cout
            << "comm_files::silo_pllel_finish_with_file() closed file and created "
...
#ifdef TESTING
            cout << "   rank " << myrank << ": ...still waiting for file "
                 << filename << "\n";
#endif
        }
        else {
...
)
{
    ...
    for (int rank = 1; rank < nproc; rank++) {
        ...
        wait_for_file(tmp.str());
#ifdef TESTING
        cout << "master_wait_on_slave_files:" << msg
             << " got file:" << tmp.str() << "\n";
#endif
    }
