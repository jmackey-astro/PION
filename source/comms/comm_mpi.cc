...
double comm_mpi::global_operation_double(const string s, ///< Either Max, Min, or Sum
...
int comm_mpi::broadcast_data(const int sender,       ///< rank of sender.
...
int comm_mpi::send_cell_data(const int to_rank,    ///< rank to send to.
			     ...
			     )
{
  ...
  temp <<"from "<<mpiPM.myrank<<" to "<<to_rank<<" tag="<<comm_tag;
...
int comm_mpi::wait_for_send_to_finish(string &id ///< identifier for the send we are waiting on.
				      )
{
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::wait_for_send_to_finish() starting\n";
#endif //TESTING
  //
  ...
  struct sent_info *si=0;
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::wait_for_send_to_finish() more than one send, so finding in list.\n";
  cout <<"\t\tsend id="<<id<<"\n";
#endif //TESTING
  ...
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::wait_for_send_to_finish() found this send.\n";
#endif //TESTING
  ...
  MPI_Status status;
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::wait_for_send_to_finish() waiting for send.\n";
...
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::wait_for_send_to_finish() send has finished\n";
#endif //TESTING
  ...
  //
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::wait_for_send_to_finish() deleting send record\n";
#endif //TESTING
...
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::wait_for_send_to_finish() returning\n";
#endif //TESTING
  return 0;
}
...
int comm_mpi::look_for_data_to_receive(int *from_rank, ///< rank of sender
...
{
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::look_for_data_to_receive() starting\n";
#endif //TESTING
  //
...
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::look_for_data_to_receive() looking for source\n";
#endif //TESTING
  //
...
#ifdef TESTING
  cout <<"rank: "<<mpiPM.myrank<<"  comm_mpi::look_for_data_to_receive() found data, parsing...\n";
#endif //TESTING
  ...
  *comm_tag  = ri->status.MPI_TAG;
  ostringstream temp; temp.str("");
  temp <<"from "<<*from_rank<<" to "<<mpiPM.myrank<<" tag="<<*comm_tag;
  id = temp.str(); temp.str("");
#ifdef TESTING
  cout <<"comm_mpi::look_for_data_to_receive: found data from "<<*from_rank<<" with tag:"<<*comm_tag<<"\n";
#endif //TESTING
...
int comm_mpi::receive_cell_data(const int from_rank,  ///< rank of process we are receiving from.
				...
				)
{
...
#endif //TESTING
  int position=0; long int ncell=0;
  ...
  //
  //  cout <<mpiPM.myrank<<"\tcomm_unpack_data: length of data in boundary = "<<b->data.size()<<"\n";
  //  cout <<mpiPM.myrank<<"\tcomm_unpack_data: cells received for boundary= "<<ncell<<"\n";
#ifdef TESTING
			       ...
			       )
{
  ...
  //
  err += MPI_Isend(si->data, n_el, MPI_DOUBLE, si->to_rank, si->comm_tag, MPI_COMM_WORLD, &(si->request));
...
int comm_mpi::receive_double_data(const int from_rank, ///< rank of process we are receiving from.
				  const int comm_tag,  ///< comm_tag: what sort of comm we are looking for (PER,MPI,etc.)
				  const string &id,    ///< identifier for receive, for any book-keeping that might be needed.
			      ...
			      )
{
  ...
  bat = PMPIO_Init(num_files, iomode, MPI_COMM_WORLD, tag, PMPIO_DefaultCreate, PMPIO_DefaultOpen, PMPIO_DefaultClose, 0);
  if (!bat) rep.error("failed to init files",mpiPM.myrank);

  // Get the index of the group I am in.
  *group_rank = PMPIO_GroupRank(bat,mpiPM.myrank);
  // Get the rank of my processor within the group:
  *rank_in_group = PMPIO_RankInGroup(bat,mpiPM.myrank);
...
int comm_mpi::silo_pllel_wait_for_file(const std::string id, ///< identifier for this read/write.
...
{
#ifdef TESTING
  cout <<"comm_mpi::silo_pllel_wait_for_file() opening file: "<<filename<<" into directory: "<<dir<<"\n";
#endif
  ...
  *dbfile = static_cast<DBfile *>(PMPIO_WaitForBaton(bat, filename.c_str(), dir.c_str()));
  if (!(*dbfile)) rep.error("wait for baton failed to return file pointer.",*dbfile);

  return 0;
}

int comm_mpi::silo_pllel_finish_with_file(const std::string id, ///< identifier for this read/write.
