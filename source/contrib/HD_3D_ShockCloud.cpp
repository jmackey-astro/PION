      ...
      )
{
  ...
  cout <<"Rmin/Rmax="<<Rmin<<"/"<<Rmax<<", Rclump="<<Rclump<<", Rshock="<<Rshock<<"\n";
  ...
  if (radius.size()==2) rep.error("Need interior data, sampling not fine enough.",Rmax);
  ...
  if (!GS.equalD(SimPM.Range[XX],Rmax-Rmin))
    rep.error("Sim range doesn't match Rmax-Rmin",SimPM.Range[XX]);
  if (!GS.equalD(SimPM.Xmin[XX],Rmin-Rclump))
    rep.error("Sim Rmin doesn't match Rmin",SimPM.Xmin[XX]);
  if (!GS.equalD(SimPM.Xmax[XX],Rmax-Rclump))
    rep.error("Sim Rmax doesn't match Rmax",SimPM.Xmax[XX]);
  if (SimPM.coord_sys!=COORD_CRT)
    rep.error("Wrong coordinate system, use Cartesian!",SimPM.coord_sys);
  ...
  //  the spherical symmetry value and make it a planar slab at that value of z.
  ...
  do {
    ...
    for (int v=0;v<SimPM.nvar; v++) c->P[v] = data_vals[v];
          ...
          )
{
  ...
  //cout <<"z="<<seek<<", min,max="<<radius.front()<<", "<<radius.back()<<"\n";
  //
  ...
  for (int v=0;v<nvar;v++) {
    out[v] = data[v][imin] + (data[v][imax]-data[v][imin])*
                    (seek-radius[imin])/(radius[imax]-radius[imin]);
  }
  ...
  int ilo=0, ihi=0;
  while (radius[ihi]<seek2) ihi++;
  ilo=ihi-1;
  double bulk_V = data[VX][ilo] + (data[VX][ihi]-data[VX][ilo])*
                      (seek2-radius[ilo])/(radius[ihi]-radius[ilo]);
  ...
  if (dpos[XX]>Rshock) {
    ...
    //
    out[VZ] = -(out[VX]-bulk_V)*dpos[ZZ]/
        sqrt(dpos[ZZ]*dpos[ZZ]+dpos[YY]*dpos[YY]+dpos[XX]*dpos[XX]);
    out[VY] = -(out[VX]-bulk_V)*dpos[YY]/
        sqrt(dpos[ZZ]*dpos[ZZ]+dpos[YY]*dpos[YY]+dpos[XX]*dpos[XX]);
    out[VX] = bulk_V -(out[VX]-bulk_V)*dpos[XX]/
        sqrt(dpos[ZZ]*dpos[ZZ]+dpos[YY]*dpos[YY]+dpos[XX]*dpos[XX]);
  }
