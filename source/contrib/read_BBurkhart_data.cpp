    ...
    for (int v = 0; v < (SimPM.nvar - SimPM.ntracer); v++) {
        ...
        switch (v) {
            ...
            case PG:
                ...
                //
                scaling =
                    pconst.kB() * Tism / (mass_per_particle * CodePDratio);
                break;
            ...
            case BZ:
                ...
                //
                scaling = sqrt(ISMdensity / (4.0 * M_PI))
                          * sqrt(
                                pconst.kB() * Tism
                                / (mass_per_particle * CodePDratio));
                break;
...
)
{
    ...
    err = fio->read_fits_image_to_data(
        ff, HDU_NAME, ndim, l_xmin, g_xmin, pix_size, NG, ncell, TDOUBLE,
        &data);
