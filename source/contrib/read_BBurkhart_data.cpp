      ...
      )
{
  ...
  str = rp->find_parameter(seek);
  if (str=="") VXoffset = 0.0;
  else         VXoffset = atof(str.c_str());
  seek = "BB_offset_VY";
  str = rp->find_parameter(seek);
  if (str=="") VYoffset = 0.0;
  else         VYoffset = atof(str.c_str());
  seek = "BB_offset_VZ";
  str = rp->find_parameter(seek);
  if (str=="") VZoffset = 0.0;
  else         VZoffset = atof(str.c_str());
  ...
  //
  double
    CodePDratio=0.0, ISMdensity=0.0, Tism=0.0, mass_per_particle=0.0;
  ...
  for (int v=0; v<(SimPM.nvar-SimPM.ntracer); v++) {
    ...
    switch (v) {
      ...
      case SI:
      // The glm-mhd psi variable has no scaling, and should be set to zero.
      scaling=0.0;
      break;
  ...
  for (int t=0; t<SimPM.ntracer; t++) {
    ttt.str("");
    ttt << "BB_TR"; ttt.width(2); ttt.fill('0'); ttt<< t;
    temp = ttt.str();
    cout <<" looking for tracer "<<temp<<";";
    seek = rp->find_parameter(temp);
    if (seek!="") data_vals[t] = atof(seek.c_str());
    else          data_vals[t] = 0.0;
          ...
          )
{
  ...
  if(status) {fits_report_error(stderr,status); rep.error("open fits file",err);}
