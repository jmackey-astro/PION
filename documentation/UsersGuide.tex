...
\title{User's Guide for JM's code}
...
Installation is reasonably straightforward, but there is on library which needs to be manually downloaded because it does not have a free licence.
...
It is quite rare that the code needs to recompiled to run a different simulation -- a guiding design principle of the code is that all options should be set at run-time and not compile-time.
Here is how to compile the code:
\begin{enumerate}
\item
  If you have a copy of JM's code as jmcode.tar.gz, unzip/untar it into any directory you want.  The cd into this directory and the directory \verb|uniform_grid_code/| should have been created there.  cd into this directory.
\item
  The Sundials numerical integration library is used for some of the microphysics classes, so the file \verb|sundials-2.4.0.tar.gz| needs to be downloaded from \url{https://computation.llnl.gov/casc/sundials/download/download.html} and moved into the directory \verb|uniform_grid_code/trunk/extra_libraries/|.
\item
  Then from \verb|uniform_grid_code/| with a standard linux workstation you should be able to run the command \verb|./install.sh|.
  If this is successful then the \textsc{Silo}, \textsc{Fits}, and \textsc{Sundials} libraries should be installed to \verb|uniform_grid_code/trunk/extra_libraries/| and code executables should have been created in the current directory.
  The exectuables are \verb|gridcode_serial|, \verb|gridcode_parallel|, \verb|icgen_serial|, and \verb|icgen_parallel|.
\end{enumerate}

The compile scripts will automatically detect if it is being compiled on JUROPA at J\"ulich Supercomputing Centre or PHALANX at UCL astrophysics dept., and if neither of these machines are identified then it is assumed that compilation is on a standard linux workstation.
...
It is relatively straightforward to modify the compile scripts for other machines -- it is just a matter of locating the standard libraries, setting the MPI wrapper commands, and setting the \texttt{CC, CXX} commands (Fortran is not needed, but one or other of the libraries may also need \texttt{FC} to be defined if not using gfortran).
As you can see from \verb|install.sh|, you need to edit the files
\verb|uniform_grid_code/trunk/extra_libraries/install_all_libs.sh|,
\verb|uniform_grid_code/trunk/bin_parallel/compile_code.sh|,and
\verb|uniform_grid_code/trunk/bin_serial/compile_code.sh|.

The library install script \verb|uniform_grid_code/trunk/extra_libraries/install_all_libs.sh| detects machines by hostname, so you can add a loop for your host.
...
The serial and parallel \verb|compile_code.sh| scripts have a similar structure, but here \texttt{MAKE\_UNAME} is important.
A new section needs to be created for the new machine in the Makefiles: \texttt{Makefile.serial.code}, \texttt{Makefile.serial.icgenerator}, \texttt{Makefile.pllel.code}, and \texttt{Makefile.pllel.icgenerator}.
This section 
...
It then writes all of this to a standard simulation output file, in either FITS\footnote{See \url{http://www.nasa.gov/} for details.} or SILO\footnote{See \url{http://www.llnl.gov/} for details} format.
