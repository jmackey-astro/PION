  ...
  for (ifile=0; ifile<nfiles; ifile++) {
    ...
    do {
      ...
      if (SimPM.coord_sys==COORD_CYL) {
        //cout <<"rad="<< cpos[Rcyl] <<",  ";
        //cout <<"diff="<< grid->idifference_vertex2cell(iSrcPos,c,Rcyl)*CI.phys_per_int();
        //cout <<", ratio = ";
        //cout << grid->idifference_vertex2cell(iSrcPos,c,Rcyl)*CI.phys_per_int()/cpos[Rcyl];
        //cout <<"\n";
        cpos[Rcyl] = grid->idifference_vertex2cell(iSrcPos,c,Rcyl)*CI.phys_per_int();
      }
      else if (SimPM.coord_sys==COORD_SPH) {
        //cout <<"rad="<< cpos[Rsph] <<",  ";
        //cout <<"diff="<< grid->idifference_vertex2cell(iSrcPos,c,Rsph)*CI.phys_per_int();
        //cout <<", ratio = ";
        //cout << grid->idifference_vertex2cell(iSrcPos,c,Rsph)*CI.phys_per_int()/cpos[Rsph];
        //cout <<"\n";
      ...
      if ( (c->P[RO] > 7.79e-22*1.1) && (vrad>1.0e4) ) {
    ...
    if (SimPM.coord_sys==COORD_SPH) {
      c=grid->FirstPt();
      while (grid->NextPt(c)->P[tr_Hp]>0.5) {
        c=grid->NextPt(c);
      }
      dist = grid->distance_vertex2cell(SrcPos,c);
