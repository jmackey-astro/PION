 ...
 * 
 * Compile with:\n
 * g++ -Wall -g -DSERIAL -DSILO    plot_radius.cc \
   ${SRCDIR}/global.cc ${SRCDIR}/uniformGrid.cc dataio.o readparams.o global.o \
	dataio_silo.o -lreadline -lcfitsio
 * 
 * Run with (example):\n
...
int main(int argc, char **argv)
{
  ...
  //
  int rt_flag = SimPM.EP.raytracing;
  int hc_flag = 0;
  // (IGNORE ARTIFICIAL VISCOSITY VARIABLES SINCE NOT DOING ANY DYNAMICS)
  // if (SimPM.artviscosity==AV_LAPIDUS)
  //   hc_flag += 1;  // Just need one var. for Div(v)
  // else if (SimPM.artviscosity==AV_HCORRECTION ||
  // 	   SimPM.artviscosity==AV_HCORR_FKJ98)
  //   hc_flag += SimPM.ndim; // need one for each dimension.
  ...
  outf << "# Columns are [time, I-front position, err, err2, posn from Nions, Nions, Nphotons], and should be in cgs units (s,cm).\n\n";
  ...
  do {
     ...
     do {
       ...
       if (SimPM.coord_sys==COORD_CYL) cvol=2.*M_PI*cpos[Rcyl]*dx*dx;
     ...
     fit_radius = a;
     double src_str=0.0;  RSP.src_strength(0,&src_str);
     ...
     //err += dataio->OutputData(outfile_img);
			   ...
			   )
{
  ...
  double src_str=0.0; RSP.src_strength(0,&src_str);
  double rad = src_str*SimPM.simtime*GS.m_p()/M_PI/c->P[RO];
