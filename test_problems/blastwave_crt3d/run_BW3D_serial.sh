...
#
# 2016.0317 JM: program written.
#

# Test command-line arguments
if test "$#" -ne 4; then
    echo "UASGE: run_BW3D_serial.sh test_dir code_dir data_dir resolution"
    exit
fi


# call with ./run_BW3D_serial.sh $test_dir $code_dir $data_dir $resolution
...
rm ${data_dir}/BWcrt3Dser_Octant_NR${resolution}_*.*.silo

# 
# First run some short simulations to make sure the early expansion is ok.
#
echo "Running"  ${PION} IC_BWcrt3D_Octant_NR${resolution}.silo 5 1
${PION} IC_BWcrt3D_Octant_NR${resolution}.silo 5 1 \
  outfile=${data_dir}/BWcrt3Dser_Octant_NR${resolution}_HYB_FKJav01 \
  redirect=${data_dir}/msg_BWcrt3Dser_Octant_NR${resolution}_HYB_FKJav01 \
  cfl=0.3 AVtype=1 EtaVisc=0.1 solver=3 \
  opfreq_time=1.578e11 finishtime=3.156e10 &
#
${PION} IC_BWcrt3D_Octant_NR${resolution}.silo 5 1 \
  outfile=${data_dir}/BWcrt3Dser_Octant_NR${resolution}_RPV_FKJav01 \
  redirect=${data_dir}/msg_BWcrt3Dser_Octant_NR${resolution}_RPV_FKJav01 \
  cfl=0.3 AVtype=1 EtaVisc=0.1 solver=5 \
  opfreq_time=1.578e11 finishtime=3.156e10 &
#
${PION} IC_BWcrt3D_Octant_NR${resolution}.silo 5 1 \
  outfile=${data_dir}/BWcrt3Dser_Octant_NR${resolution}_FVS_FKJav01 \
  redirect=${data_dir}/msg_BWcrt3Dser_Octant_NR${resolution}_FVS_FKJav01 \
  cfl=0.3 AVtype=1 EtaVisc=0.1 solver=6 \
  opfreq_time=1.578e11 finishtime=3.156e10 &
#
${PION} IC_BWcrt3D_Octant_NR${resolution}.silo 5 1 \
  outfile=${data_dir}/BWcrt3Dser_Octant_NR${resolution}_RCV_FKJav01 \
  redirect=${data_dir}/msg_BWcrt3Dser_Octant_NR${resolution}_RCV_FKJav01 \
  cfl=0.3 AVtype=1 EtaVisc=0.1 solver=4 \
  opfreq_time=1.578e11 finishtime=3.156e10 &
#
${PION} IC_BWcrt3D_Octant_NR${resolution}.silo 5 1 \
  outfile=${data_dir}/BWcrt3Dser_Octant_NR${resolution}_RCV_Hcorr \
  redirect=${data_dir}/msg_BWcrt3Dser_Octant_NR${resolution}_RCV_Hcorr   \
...
#
rm ${data_dir}/BWcrt3D_Octant*.999999*.silo
#
